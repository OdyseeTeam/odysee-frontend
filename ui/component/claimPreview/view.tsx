...
const AbandonedChannelPreview = lazyImport(() => import('component/abandonedChannelPreview'
...
const ClaimPreview = forwardRef<any, {}>((props: Props, ref: any) => {
  ...
  const shouldFetch = claim === undefined || claim !== null && claim.value_type === 'channel' && isEmpty(claim.meta) && !pending;
  ...
  const backgroundImage = thumbnailFromClaim ? 'https://thumbnails.odycdn.com/optimize/s:390:0/quality:85/plain/' + thumbnailFromClaim : undefined;
  ...
  const channelSubscribers = React.useMemo(() => {
    ...
    return <div className="media__subtitle">
      ...
      </div>; // eslint-disable-next-line react-hooks/exhaustive-deps -- @see TODO_NEED_VERIFICATION
  }, [channelSubCount]);
  ...
  const repostedChannelUri = claim && claim.repost_channel_url && claim.value_type === 'channel' ? claim.permanent_url || claim.canonical_url : undefined;
  ...
  const navigateUrl = isCollection && listId && defaultCollectionAction === COLLECTIONS_CONSTS.DEFAULT_ACTION_VIEW ? `/$/${PAGES.PLAYLIST}/${listId}` : formatLbryUrlForWeb(claim && claim.canonical_url || uri || '/');
  ...
  let shouldHide = placeholder !== 'loading' && (abandoned && !showUnresolvedClaim || !claimIsMine && obscureNsfw && nsfw && !showUserBlocked);
  ...
  const JoinButton = React.useMemo(() => () => isChannelUri && !claimIsMine && !hideJoin && (!banState.muted || showUserBlocked) && <div className={'membership-button-wrapper' + (type ? ' ' + type : '')}>
          <JoinMembershipButton uri={uri} />
        </div>, [banState.muted, claimIsMine, hideJoin, isChannelUri, showUserBlocked, type, uri]);
  ...
  if (placeholder === 'loading' || uri && claim === undefined) {
    return <ClaimPreviewLoading isChannel={isChannelUri} type={type} WrapperElement={WrapperElement} xsmall={smallThumbnail} />;
  }

  if (claim && showNullPlaceholder && shouldHide && nsfw && obscureNsfw) {
    return <ClaimPreviewHidden message={__('Mature content hidden by your preferences')} isChannel={isChannelUri} type={type} />;
  }

  if (claim && showNullPlaceholder && shouldHide || !claim && playlistPreviewItem) {
    return <WrapperElement ref={ref} className={classnames('claim-preview__wrapper', {
      ...
      'non-clickable': !playlistPreviewItem || nonClickable
        ...
        <ClaimPreviewHidden message={!claim && playlistPreviewItem ? __('Deleted content') : __('This content is hidden')} isChannel={isChannelUri} type={type} uri={uri} collectionId={!claim && playlistPreviewItem && collectionId ? collectionId : undefined} />
  ...
  return <WrapperElement ref={ref} role="link" onClick={pending || type === 'inline' ? undefined : handleOnClick} className={classnames('claim-preview__wrapper', {
    'claim-preview__wrapper--row': !type,
    ...
    'non-clickable': nonClickable
      ...
      <>
        {!type && <div className="claim-preview__background" style={backgroundImage && {
        ...
        'claim-preview--collection-editing': isMyCollection && showEdit
          ...
          {showIndexes && <span className="card__subtitle card__subtitle--small-no-margin claim-preview__list-index">
              {indexInContainer + 1}
            </span>}

          {isMyCollection && showEdit && !isCollectionOnPublicView && <CollectionEditButtons uri={uri} collectionId={listId} isEditPreview={isEditPreview} dragHandleProps={dragHandleProps} doDisablePlayerDrag={doDisablePlayerDrag} />}

          {isChannelUri && claim ? <UriIndicator focusable={false} uri={uri} link external={isEmbed}>
              <ChannelThumbnail uri={uri} small={type === 'inline'} checkMembership={false} />
            </UriIndicator> : <>
              {!pending ? <NavLink aria-hidden tabIndex={-1} {...navLinkProps} target={isEmbed && '_blank'}>
                  <FileThumbnail thumbnail={thumbnailUrl} small={smallThumbnail} uri={uri} secondaryUri={firstCollectionItemUrl}>
                    {showCollectionContext && !smallThumbnail && <div className="claim-preview__hover-actions-grid">
          ...
          <div className="claim-preview__text">
            <div className="claim-preview-metadata">
              <div className="claim-preview-info">
                {pending ? <ClaimPreviewTitle uri={uri} /> : <NavLink aria-label={ariaLabelData} aria-current={active ? 'page' : null} {...navLinkProps} target={isEmbed && '_blank'}>
                    <ClaimPreviewTitle uri={uri} />
                  </NavLink>}
                {banState.blacklisted && claimIsMine && <a href="https://help.odysee.tv/category-uploading/dmca-content/#receiving-a-dmca-notice" target="_blank" rel="noreferrer" onClick={e => {
                e.stopPropagation();
              }}>
                    <div className="dmca-info">{__('DMCA flagged')}</div>
                  </a>}
                {banState.filtered && claimIsMine && <a href="https://help.odysee.tv/category-uploading/dmca-content/#receiving-a-dmca-notice" target="_blank" rel="noreferrer" onClick={e => {
                e.stopPropagation();
              }}>
              ...
              <div className="claim-tile__info">
                {!isChannelUri && signingChannel && <div className="claim-preview__channel-staked">
                ...
                {type !== 'small' && <>
                    <div className="claim-preview__tags">
                      {claim && <React.Fragment>
                          {typeof properties === 'function' ? properties(claim) : properties !== undefined ? properties : !isMobile && <ClaimTags uri={uri} type={type} />}
                        </React.Fragment>}
                    </div>
              ...
              {!type && <div className="description__wrapper">
                  <div className="description">{claim?.value?.description || __('No description available.')}</div>
                </div>}
            </div>

            {type !== 'small' && (!pending || !type) && isChannelUri && <div className="claim-preview__actions">
                {!hideJoin && <JoinButton uri={uri} />}
                {!pending && <>
                    {shouldHideActions || renderActions ? null : actions !== undefined ? actions : <>
                        {isChannelUri && !claimIsMine && (!banState.muted || showUserBlocked) && <SubscribeButton uri={repostedChannelUri || (uri.startsWith('lbry://') ? uri : `lbry://${uri}`)} />}
        ...
        {inWatchHistory && <div onClick={e => removeFromHistory(e, uri)} className="claim-preview__history-remove">
