...
function TagSearch(props: Props) {
  ...
  const handleChangeDebounced = React.useCallback(debounce(v => handleChange({
    key: CS.TAGS_KEY,
    value: v
  }), 500), []);
  return <div className={classnames('clh-tag-search', {
    'clh-tag-search--standalone': standalone
      ...
      <div className="clh-tag-search__input_group">
        <FormField ref={inputRef} placeholder={__('Search tags')} type="text" className="clh-tag-search__input" name="tag_query" value={tagSearchQuery} onChange={e => {
        setTagSearchQuery(e.target.value);
        handleChangeDebounced(e.target.value);
      }} />
        <Icon icon={ICONS.TAG} />
        {tagSearchQuery && <Button icon={ICONS.REMOVE} aria-label={__('Clear')} button="alt" className="clh-tag-search__clear" onClick={() => {
        setTagSearchQuery('');
        handleChange({
          key: CS.TAGS_KEY,
          value: ''
