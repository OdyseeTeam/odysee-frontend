...
function ClaimListHeader(props: Props) {
  ...
  const streamTypeParam = streamType || CS.FILE_TYPES.includes(contentTypeParam) && contentTypeParam || defaultStreamType || null;
  ...
  const setMinDurationMinutesDebounced = React.useCallback(debounce(m => setMinDurationMinutes(m), 750), []);
  const setMaxDurationMinutesDebounced = React.useCallback(debounce(m => setMaxDurationMinutes(m), 750), []);

  const isFiltered = () => Boolean(contentType || urlParams.get(CS.FRESH_KEY) || urlParams.get(CS.CONTENT_KEY) || !filterCtx?.liftUpTagSearch && urlParams.get(CS.TAGS_KEY) || urlParams.get(CS.DURATION_KEY) || urlParams.get(CS.FEE_AMOUNT_KEY) || urlParams.get(CS.LANGUAGE_KEY) || filterCtx?.repost?.hideReposts || filterCtx?.membersOnly?.hideMembersOnly);

  const languageValue = searchInLanguage ? languageParam === null ? languageSetting : languageParam : languageParam === null ? CS.LANGUAGES_ALL : languageParam;
  const shouldHighlight = searchInLanguage ? languageParam !== languageSetting && languageParam !== null : languageParam !== CS.LANGUAGES_ALL && languageParam !== null;
  const orderParam = usePersistentUserParam([orderBy, urlParams.get(CS.ORDER_BY_KEY), defaultOrderBy], 'orderUser', CS.ORDER_BY_TRENDING);
  React.useEffect(() => {
      ...
      <div className="claim-search__wrapper clh__wrapper">
        <div className="claim-search__header">
          <div className="claim-search__top">
            {!hideFilters && <div className="claim-search__menu-group">
                {CS.ORDER_BY_TYPES.map(type => <Button key={type} button="alt" onClick={e => handleChange({
              key: CS.ORDER_BY_KEY,
              value: type
            })} className={classnames(`button-toggle button-toggle--${type}`, {
              'button-toggle--active': orderParam === type
            })} disabled={orderBy} icon={toCapitalCase(type)} iconSize={toCapitalCase(type) === ICONS.NEW ? 20 : undefined} label={__(toCapitalCase(type))} />)}
              </div>}
            <div className="claim-search__menu-group">
              {tileLayout !== undefined && !hideLayoutButton && <>
                  <Button onClick={() => {
                doSetClientSetting(SETTINGS.TILE_LAYOUT, true);
              }} button="alt" className={classnames(`button-toggle button-toggle--top`, {
                'button-toggle--active': tileLayout
              })} aria-label={__('Change to tile layout')} icon={ICONS.VIEW_TILES} />
                  <Button onClick={() => {
                doSetClientSetting(SETTINGS.TILE_LAYOUT, false);
              }} button="alt" className={classnames(`button-toggle button-toggle--top`, {
                'button-toggle--active': !tileLayout
              })} aria-label={__('Change to list layout')} icon={ICONS.VIEW_LIST} />
                </>}
            </div>
            <>
              {showHideAnonymous && <div className="claim-search__menu-group hide-anonymous-checkbox">
                  <FormField label={__('Hide anonymous')} name="hide_anonymous" type="checkbox" checked={hideAnonymous} onChange={() => setHideAnonymous(!hideAnonymous)} />
                </div>}
            </>
            <div className="claim-search__menu-group stretch">
              {!hideAdvancedFilter && <Button button="alt" aria-label={__('More')} className={classnames(`button-toggle button-toggle--top button-toggle--more`, {
              'button-toggle--custom': isFiltered(),
              'button-toggle--active button-toggle--bottom-arrow': expanded
            })} icon={ICONS.SLIDERS} onClick={() => setExpanded(!expanded)} />}
        ...
        <div className={classnames('claim-search__filters-wrapper', {
        'claim-search__filters-wrapper-expanded': expanded
          ...
          <div className="claim-search__filters">
            <div className="claim-search__menus">
              ...
              {orderParam === CS.ORDER_BY_TOP && <div className="claim-search__input-container">
                  <FormField className={classnames('claim-search__dropdown', {
                'claim-search__dropdown--selected': freshnessParam !== defaultFreshness
              })} type="select" name="trending_time" label={__('How Fresh')} value={freshnessParam} onChange={e => handleChange({
                key: CS.FRESH_KEY,
                value: e.target.value
              })}>
                    {CS.FRESH_TYPES.map(time => <option key={time} value={time}>
                        {
                    /* i18fixme */
                  }
                        {time === CS.FRESH_DAY && __('Today')}
                        {time !== CS.FRESH_ALL && time !== CS.FRESH_DEFAULT && time !== CS.FRESH_DAY && __('This ' + toCapitalCase(time))
              ...
              {showDuration && <div className={classnames('claim-search__input-container', {
              'claim-search__input-container--selected': contentTypeParam
            })}>
                  <FormField className={classnames('claim-search__dropdown', {
                'claim-search__dropdown--selected': contentTypeParam
              })} type="select" name="claimType" label={__('Content Type')} value={contentTypeParam || CS.CONTENT_ALL} onChange={e => handleChange({
                key: CS.CONTENT_KEY,
                value: e.target.value
              })}>
              ...
              {showDuration && <>
                ...
                })} label={__('Duration --[length of audio or video]--')} type="select" name="duration" disabled={!(contentTypeParam === null || contentTypeParam === CS.FILE_AUDIO || contentTypeParam === CS.FILE_VIDEO || streamTypeParam === CS.FILE_AUDIO || streamTypeParam === CS.FILE_VIDEO)} value={durationParam || CS.DURATION.ALL} onChange={e => handleChange({
                  key: CS.DURATION_KEY,
                  value: e.target.value
                })}>
                      {CS.DURATION_TYPES.map(dur => <option key={String(dur)} value={dur}>
                          {
                      /* i18fixme */
                    }
                  ...
                  {durationParam === CS.DURATION.CUSTOM && <div className="claim-search__duration-inputs-container">
                      <div className="claim-search__input-container">
                        <FormField label={__('Min Minutes')} type="number" name="min_duration__minutes" value={minMinutes} onChange={e => {
                    setMinMinutes(e.target.value);
                    setMinDurationMinutesDebounced(e.target.value);
                  }} />
                      </div>
                      <div className="claim-search__input-container">
                        <FormField label={__('Max Minutes')} type="number" name="max_duration__minutes" value={maxMinutes} onChange={e => {
              ...
              {false && !claimType && <div className={classnames('claim-search__input-container', {
              'claim-search__input-container--selected': shouldHighlight
            })}>
                  <FormField className={classnames('claim-search__dropdown', {
                'claim-search__dropdown--selected': shouldHighlight
              })} type="select" name="claimType" label={__('Language')} value={languageValue || CS.LANGUAGES_ALL} onChange={e => handleChange({
                key: CS.LANGUAGE_KEY,
                value: e.target.value
              })}>
              ...
              <div className={'claim-search__input-container'}>
                <FormField className={classnames('claim-search__dropdown', {
                'claim-search__dropdown--selected': feeAmountParam === CS.FEE_AMOUNT_ONLY_FREE || feeAmountParam === CS.FEE_AMOUNT_ONLY_PAID
              })} label={__('Price')} type="select" name="paidcontent" disabled={filterCtx.isChannelSearch} value={feeAmountParam} onChange={e => handleChange({
              ...
              {orderParam === CS.ORDER_BY_NEW && <div className={'claim-search__input-container'}>
                  <FormField className={classnames('claim-search__dropdown', {
                'claim-search__dropdown--selected': sortByParam
              })} label={__('Sort By')} type="select" name="sort_by" value={sortByParam || CS.SORT_BY.NEWEST.key} onChange={e => handleChange({
                key: CS.SORT_BY_KEY,
                value: e.target.value
              })}>
                    {Object.entries(CS.SORT_BY).map(([key, value]) => {
                  return (// $FlowFixMe https://github.com/facebook/flow/issues/2221
              ...
              {channelIdsInUrl && <div className={'claim-search__input-container'}>
                  <label>{__('Advanced Filters from URL')}</label>
                  <Button button="alt" className="claim-search__filter-button" label={__('Clear')} onClick={handleAdvancedReset} />
                </div>}
            </div>
            <div className="claim-search__menus">
              {filterCtx.repost && <div className="claim-search__input-container">
                  <AdditionalFilters filterCtx={filterCtx} contentType={contentTypeParam} />
                </div>}
              {!filterCtx.isChannelSearch && <div className="claim-search__input-container">
