...
const PublishPending = (props: Props) => {
  ...
  if (stalled) {
    return <Button button="link" label={__('Upload stalled. Retry?')} onClick={() => Lbry.file_reflect({
      sd_hash: sdHash
    }).then(() => checkReflecting())} />;
  } else if (reflecting) {
    return <span>{__('Uploading (%progress%%) ', {
        progress: progress
      })}</span>;
  } else {
    return <div className="confirming-change">
