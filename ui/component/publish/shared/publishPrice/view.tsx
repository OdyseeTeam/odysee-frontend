...
function PublishPrice(props: Props) {
  ...
  function sanitizeDuration() {
    if (Number.isFinite(fiatRentalExpiration.value)) {
      updatePublishForm({
        fiatRentalExpiration: { ...fiatRentalExpiration,
  ...
  function getRestrictionWarningRow() {
    return <div className={classnames('publish-price__row', {})}>
    ...
    return <div className={classnames('publish-price__row', {
      'publish-price__row--disabled': restrictedWithoutPrice
        ...
        <div className="publish-price__grp-1">
          <fieldset-section>
            <React.Fragment>
              <FormField type="radio" name="content_free" label={__('Free')} checked={paywall === PAYWALL.FREE} disabled={disabled} onChange={() => updatePublishForm({
              paywall: PAYWALL.FREE
            })} />

              <FormField type="radio" name="content_fiat" label={`${__('Purchase / Rent')} \u{0024}`} checked={paywall === PAYWALL.FIAT} disabled={disabled || !monetizationStatus} onChange={() => updatePublishForm({
              ...
              {hadSDKPaywallSelected && <>
                  <FormField type="radio" name="content_sdk" label={<LbcSymbol prefix={__('Purchase with Credits')} />} checked={paywall === PAYWALL.SDK} disabled={disabled} onChange={() => updatePublishForm({
                paywall: PAYWALL.SDK
              })} />
                  {paywall === PAYWALL.SDK && <p className="help--warning" style={{
                'margin-top': '10px'
              }}>
  ...
  function getTncRow() {
    return <div className="publish-price__row">
        <div className="publish-price__grp-1 publish-price__tnc">
          <I18nMessage tokens={{
          paid_content_terms_and_conditions: <Button button="link" href="https://help.odysee.tv/category-monetization/" label={__('paid-content terms and conditions')} />
        }}>
  ...
  function getPurchaseRow() {
    return <div className={classnames('publish-price__row', {
      'publish-price__row--disabled': (noBankAccount || restrictedWithoutPrice) && !ENABLE_ARCONNECT
        ...
        <div className="publish-price__grp-1">
          <FormField label={__('Purchase')} name="purchase" type="checkbox" checked={fiatPurchaseEnabled} onChange={() => updatePublishForm({
          fiatPurchaseEnabled: !fiatPurchaseEnabled
        })} />
        </div>
        <div className={classnames('publish-price__grp-2', {
        'publish-price__grp-2--disabled': !fiatPurchaseEnabled
      })}>
          <FormFieldPrice name="fiat_purchase_fee" min={0.01} price={fiatPurchaseFee} onChange={fee => updatePublishForm({
          fiatPurchaseFee: fee
        })} onBlur={() => sanitizeFee('fiatPurchaseFee')} currencies={CURRENCY_OPTIONS} />
          <div className="publish-price__fees">
            <FeeBreakdown amount={fiatPurchaseFee.amount} currency={fiatPurchaseFee.currency} doCustomerPurchaseCost={doCustomerPurchaseCost} />
          </div>
        </div>
      </div>;
  }
  ...
  function getRentalRow() {
    return <div className={classnames('publish-price__row', {
      'publish-price__row--disabled': (noBankAccount || restrictedWithoutPrice) && !ENABLE_ARCONNECT
        ...
        <div className="publish-price__grp-1">
          <FormField label={__('Rent')} name="rent" type="checkbox" checked={fiatRentalEnabled} onChange={() => updatePublishForm({
        ...
        'publish-price__grp-2--disabled': !fiatRentalEnabled
          ...
          <FormFieldPrice name="fiat_rental_fee" min={0.01} price={fiatRentalFee} onChange={fee => updatePublishForm({
          fiatRentalFee: fee
        })} onBlur={() => sanitizeFee('fiatRentalFee')} currencies={CURRENCY_OPTIONS} />
          <FormFieldDurationCombo label={__('Duration')} name="fiat_rental_expiration" min={1} duration={fiatRentalExpiration} onChange={duration => updatePublishForm({
          fiatRentalExpiration: duration
        })} onBlur={() => sanitizeDuration()} units={['months', 'weeks', 'days', 'hours']} />
          <div className="publish-price__fees">
            <FeeBreakdown amount={fiatRentalFee.amount} currency={fiatRentalFee.currency} doCustomerPurchaseCost={doCustomerPurchaseCost} />
          </div>
        </div>
      </div>;
  }
  ...
  function getLbcPurchaseRow() {
    return <div className={classnames('publish-price__row', {
      'publish-price__row--disabled': restrictedWithoutPrice
        ...
        <div className="publish-price__grp-1">
          <FormFieldPrice name="lbc_purchase_fee" min={1} price={fee} onChange={newFee => updatePublishForm({
          fee: newFee
        })} />
          {fee && fee.currency !== 'LBC' && <p className="publish-price__subtitle">{__('All content fees are charged in Credits.')}</p>}
        </div>
      </div>;
  }
  ...
  if (paymentDisallowed) {
    return <div className="publish-price">
        <Card background isBodyList className="card--enable-overflows" title={__('Price')} body={<div className="publish-price__reason">
  ...
  return <div className="publish-price">
      <Card background isBodyList className="card--enable-overflows" title={__('Price')} body={<>
            {expanded && <div className={classnames('settings-row', {
        'settings-row--disabled': paymentDisallowed
      })}>
                {restrictedWithoutPrice && getRestrictionWarningRow()}
                {getPaywallOptionsRow()}
                {paywall === PAYWALL.FIAT && <div className="publish-price__group">
                    {getPurchaseRow()}
                    {getRentalRow()}
                    {getTncRow()}
                  </div>}
            ...
            <div className="publish-row publish-row--more">
              <Button label={__(expanded ? 'Hide' : 'Show')} button="link" onClick={() => setExpanded(prev => !prev)} />
            </div>
          </>} />
    </div>;
}
