...
function PublishAdditionalOptions(props: Props) {
  ...
  return <>
      <Card background className="card--enable-overflow" title={__('Additional Options')} body={<React.Fragment>
            {!hideSection && !disabled && <div className="settings-row">
                <div className={classnames({
          'card--disabled': !name
        })}>
                  <div className="section">
                    {showReleaseDate && <PublishReleaseDate />}
                    <FormField className={!showReleaseDate && 'publish-row--no-margin-select'} label={__('Language')} type="select" name="content_language" value={language} // $FlowFixMe @see FIX_LANGUAGE_STATE
            onChange={event => updatePublishForm({
              languages: [event.target.value]
                    ...
                    <LicenseType licenseType={licenseType} otherLicenseDescription={otherLicenseDescription} licenseUrl={licenseUrl} handleLicenseChange={(newLicenseType, newLicenseUrl) => updatePublishForm({
              licenseType: newLicenseType,
              licenseUrl: newLicenseUrl
            })} handleLicenseDescriptionChange={event => updatePublishForm({
              otherLicenseDescription: event.target.value
            })} handleLicenseUrlChange={event => updatePublishForm({
