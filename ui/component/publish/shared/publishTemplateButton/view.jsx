...
const DEFAULT_FEE = { amount: 1, currency: 'LBC' };
const DEFAULT_FIAT_FEE = { amount: 1, currency: 'USD' };
const DEFAULT_RENTAL_DURATION = { value: 1, unit: 'weeks' };
...
export default function PublishTemplateButton(props: Props) {
  ...
  const [selectedTemplateKey, setSelectedTemplateKey] = React.useState<?string>(null);
  const [selectedTemplateBaselineData, setSelectedTemplateBaselineData] = React.useState<?UploadTemplateData>(null);
  ...
  const getTemplatesForChannelId = React.useCallback(
    (channelId: ?string): Array<UploadTemplate> => {
      if (!channelId) return [];
  ...
  function persistTemplatesForChannel(channelClaim: ?ChannelClaim, templatesToSave: Array<UploadTemplate>) {
    if (!channelClaim || !channelClaim.claim_id) return;
    const channelId = channelClaim.claim_id;

    setOptimisticTemplatesByChannelId((prev) => ({
      ...prev,
  ...
  function handleSaveTemplate() {
    ...
    if (existingTemplateByNameForSave) {
      ...
      doToast({ message: __('Template "%name%" updated', { name: normalizedTemplateName }) });
      closeSaveInput();
      return;
    }
  ...
  function handleApplyTemplate(template: TemplateEntry) {
    ...
    setSelectedTemplateBaselineData({
      ...currentDataBeforeApply,
      ...template.data,
    });
    ...
    doToast({
      message: __('Template "%name%" applied', { name: template.name }),
      actionText: __('Undo'),
  ...
  function handleToggleTemplatePin(template: TemplateEntry) {
    const ownerChannelClaim = orderedChannelClaims.find((channelClaim) => channelClaim.claim_id === template.channelId);
    if (!ownerChannelClaim) {
      doToast({ message: __('Unable to find template owner channel.'), isError: true });
      return;
    }
  ...
  function handleUpdateSelectedTemplate() {
    ...
    openModal(MODALS.CONFIRM, {
      title: __('Update template "%name%"?', { name: selectedTemplate.name }),
      ...
      onConfirm: (closeModal) => {
        ...
        const templateData = extractTemplateDataFromPublishForm(publishFormValues);
        if (Object.keys(templateData).length === 0) {
          doToast({ message: __('Cannot update with an empty publish form.'), isError: true });
          closeModal();
          return;
        }
        ...
        doToast({ message: __('Template "%name%" updated', { name: selectedTemplate.name }) });
        closeModal();
      },
    });
  }
      ...
      {showSaveInput && (
        <div className="publish-template-save">
          ...
          <span className="publish-template-save__target">
            {__('Saving to %channel%', { channel: saveTargetChannelName })}
          </span>
