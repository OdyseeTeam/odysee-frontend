...
const TEMPLATE_FIELDS = ['title', 'description', 'tags', 'thumbnail', 'language', 'languages', 'licenseType', 'licenseUrl', 'otherLicenseDescription', 'nsfw', 'paywall', 'fee', 'fiatPurchaseFee', 'fiatPurchaseEnabled', 'fiatRentalFee', 'fiatRentalExpiration', 'fiatRentalEnabled', 'visibility', 'memberRestrictionOn', 'memberRestrictionTierIds'];
...
function getTemplateSortTimestamp(template: {
  createdAt: number;
  lastUsedAt?: number;
}): number {
...
function getTemplateSearchText(template: TemplateEntry): string {
  ...
  const tags = Array.isArray(data.tags) ? data.tags.map(tag => tag && typeof tag === 'object' ? tag.name : tag).filter(Boolean).join(' ') : '';
  return `${template.name || ''} ${template.channelName || ''} ${data.title || ''} ${data.description || ''} ${tags}`.toLowerCase().trim();
}
...
function getDefaultTemplateDataForComparison(publishFormValues: any): UploadTemplateData {
  ...
  const defaultLanguages = Array.isArray(publishFormValues?.languages) ? publishFormValues.languages : defaultLanguage ? [defaultLanguage] : [];
  return {
    title: '',
    description: '',
    tags: [],
...
export default function PublishTemplateButton(props: Props) {
  ...
  const [selectedTemplateBaselineData, setSelectedTemplateBaselineData] = React.useState<UploadTemplateData | null | undefined>(null);
  const [requestedSettingsByChannelId, setRequestedSettingsByChannelId] = React.useState<Record<string, boolean>>({});
  const [optimisticTemplatesByChannelId, setOptimisticTemplatesByChannelId] = React.useState<Record<string, Array<UploadTemplate>>>({});
  const previousSettingsByChannelIdRef = React.useRef<Record<string, PerChannelSettings | null | undefined>>(settingsByChannelId || {});
  ...
  const getTemplatesForChannelId = React.useCallback((channelId: string | null | undefined): Array<UploadTemplate> => {
    if (!channelId) return [];
  ...
  const ensureChannelSettingsLoaded = React.useCallback((channelId: string | null | undefined) => {
    if (!channelId) return;
    if (settingsByChannelId && settingsByChannelId[channelId]) return;
    if (requestedSettingsByChannelId[channelId]) return;
    setRequestedSettingsByChannelId(prev => ({ ...prev,
      [channelId]: true
    }));
  ...
  const sortedTemplates = React.useMemo(() => [...allTemplates].sort((a, b) => {
  ...
  const filteredTemplates = React.useMemo(() => normalizedTemplateSearch ? sortedTemplates.filter(template => getTemplateSearchText(template).includes(normalizedTemplateSearch)) : sortedTemplates, [sortedTemplates, normalizedTemplateSearch]);
  const pinnedTemplates = React.useMemo(() => filteredTemplates.filter(template => Boolean(template.isPinned)), [filteredTemplates]);
  const recentTemplates = React.useMemo(() => filteredTemplates.filter(template => !template.isPinned), [filteredTemplates]);
  ...
  const visibleRecentTemplates = React.useMemo(() => showAllRecentTemplates ? recentTemplates : recentTemplates.slice(0, MAX_VISIBLE_TEMPLATES), [showAllRecentTemplates, recentTemplates]);
  const hiddenTemplateCount = Math.max(0, recentTemplates.length - visibleRecentTemplates.length);
  const selectedTemplate = React.useMemo(() => sortedTemplates.find(template => getTemplateKey(template.channelId, template.id) === selectedTemplateKey), [sortedTemplates, selectedTemplateKey]);
  const saveTargetChannelId = publishFormValues.channelId || defaultChannelId || activeChannelClaim && activeChannelClaim.claim_id;
  const saveTargetChannelClaim = orderedChannelClaims.find(channelClaim => channelClaim.claim_id === saveTargetChannelId) || activeChannelClaim;
  ...
  const currentTemplateData = React.useMemo(() => extractTemplateDataFromPublishForm(publishFormValues), [publishFormValues]);
  const hasPublishFormTemplateChanges = React.useMemo(() => {
    const defaultTemplateData = extractTemplateDataFromPublishForm(getDefaultTemplateDataForComparison(publishFormValues));
  ...
  React.useEffect(() => {
    ...
    const selectedTemplateStillExists = sortedTemplates.some(template => getTemplateKey(template.channelId, template.id) === selectedTemplateKey);
  ...
  function persistTemplatesForChannel(channelClaim: ChannelClaim | null | undefined, templatesToSave: Array<UploadTemplate>) {
    if (!channelClaim || !channelClaim.claim_id) return;
    const channelId = channelClaim.claim_id;
    setOptimisticTemplatesByChannelId(prev => ({ ...prev,
      [channelId]: templatesToSave
    }));
  ...
  function handleSaveTemplate() {
    ...
    if (existingTemplateByNameForSave) {
      const updatedTemplates = saveTargetTemplates.map(template => template.id === existingTemplateByNameForSave.id ? { ...template,
  ...
  function handleApplyTemplate(template: TemplateEntry) {
    ...
    setSelectedTemplateBaselineData({ ...currentDataBeforeApply,
      ...template.data
    });
    ...
    if (ownerChannelClaim) {
      const now = Date.now();
      const updatedTemplates = ownerChannelTemplates.map(existingTemplate => existingTemplate.id === template.id ? { ...existingTemplate,
        lastUsedAt: now
      } : existingTemplate);
  ...
  function handleToggleTemplatePin(template: TemplateEntry) {
    ...
    persistTemplatesForChannel(ownerChannelClaim, updatedTemplates);
    doToast({
      message: template.isPinned ? __('Template "%name%" unpinned', {
        name: template.name
  ...
  function handleUpdateSelectedTemplate() {
    ...
    openModal(MODALS.CONFIRM, {
      ...
      onConfirm: closeModal => {
        const ownerChannelClaim = orderedChannelClaims.find(channelClaim => channelClaim.claim_id === selectedTemplate.channelId);
        ...
        const updatedTemplates = ownerChannelTemplates.map(template => template.id === selectedTemplate.id ? { ...template,
          data: templateData,
          lastUsedAt: now
        } : template);
        persistTemplatesForChannel(ownerChannelClaim, updatedTemplates);
  ...
  function renderTemplateMenuItem(template: TemplateEntry) {
    return <MenuItem key={getTemplateKey(template.channelId, template.id)} className="publish-template-menu__item" onSelect={() => handleApplyTemplate(template)}>
  ...
  return <div className="publish-template-wrapper">
      <Menu>
        ...
        <MenuList className="menu__list publish-template-menu__list">
          ...
          {shouldShowTemplateSearch && <div className="publish-template-menu__search" onClick={e => e.stopPropagation()} onMouseDown={e => e.stopPropagation()}>
              <Icon icon={ICONS.SEARCH} />
              <input type="text" name="upload_template_menu_search" value={templateSearchQuery} placeholder={__('Search templates...')} autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck={false} onChange={e => setTemplateSearchQuery(e.target.value)} onClick={e => e.stopPropagation()} onMouseDown={e => e.stopPropagation()} onKeyDown={e => e.stopPropagation()} />
            </div>}

          {filteredTemplates.length > 0 ? <>
              {pinnedTemplates.length > 0 && <>{pinnedTemplates.map(template => renderTemplateMenuItem(template))}</>}

              {recentTemplates.length > 0 && <>{visibleRecentTemplates.map(template => renderTemplateMenuItem(template))}</>}

              {!showAllRecentTemplates && hiddenTemplateCount > 0 && !normalizedTemplateSearch && <MenuItem className="publish-template-menu__item" onSelect={() => setShowAllTemplates(true)}>
                  <div className="menu__link">
              ...
              {showAllTemplates && !normalizedTemplateSearch && recentTemplates.length > MAX_VISIBLE_TEMPLATES && <MenuItem className="publish-template-menu__item" onSelect={() => setShowAllTemplates(false)}>
                  <div className="menu__link">
              ...
              {normalizedTemplateSearch ? __('No templates match your search.') : __('No templates found across your channels.')}
            </div>}

          <hr className="publish-template-menu__separator" />

          <MenuItem className={classnames('publish-template-menu__item', {
          'publish-template-menu__item--disabled': !hasPublishFormTemplateChanges
        })} onSelect={() => hasPublishFormTemplateChanges && openSaveInput()} disabled={!hasPublishFormTemplateChanges}>
            <div className="menu__link">
          ...
          {canUpdateSelectedTemplate && selectedTemplate && <MenuItem className="publish-template-menu__item" onSelect={handleUpdateSelectedTemplate}>
              <div className="menu__link">
          ...
          {selectedTemplate && <MenuItem className="publish-template-menu__item" onSelect={() => handleToggleTemplatePin(selectedTemplate)}>
              <div className="menu__link">
      ...
      {showSaveInput && <div className="publish-template-save">
          <div className="publish-template-save__input-row">
            <FormField type="search" name={templateInputName} placeholder={__('Template name...')} value={templateName} autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck={false} aria-autocomplete="none" data-lpignore="true" data-1p-ignore="true" data-bwignore="true" onChange={e => setTemplateName(e.target.value)} onKeyDown={e => {
          if (e.key === 'Enter') {
            e.preventDefault();
            e.stopPropagation();
            handleSaveTemplate();
          ...
          <div className="publish-template-save__action-row">
            <button type="button" className="publish-template-save__confirm" onClick={e => {
          e.preventDefault();
          e.stopPropagation();
          handleSaveTemplate();
            ...
            <button type="button" className="publish-template-save__cancel" onClick={e => {
          e.stopPropagation();
          closeSaveInput();
        }}>
              {__('Cancel')}
          ...
          {existingTemplateByNameForSave && <span className="publish-template-save__overwrite-hint">
