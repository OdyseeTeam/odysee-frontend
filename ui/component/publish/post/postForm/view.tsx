    ...
    if (!emptyPostError) {
      if (fileEdited || nameEdited) {
        ...
        if (fileName) {
          ...
          try {
            const file = new File([blob], fileNameWithExt, { type: 'text/markdown' });
            if (typeof file.name !== 'string' || file.size === 0) throw new Error('File constructor broken');
            outputFile = file;
          } catch (e) {
            (blob as any).name = fileNameWithExt;
            (blob as any).lastModifiedDate = new Date();
            (blob as any).lastModified = Date.now();
            outputFile = blob as any;
          }
