...
function PublishFile(props: Props) {
  ...
  const claimChannelId = myClaimForUri && myClaimForUri.signing_channel && myClaimForUri.signing_channel.claim_id || activeChannelClaim && activeChannelClaim.claim_id;
  ...
  */
  ...
  function linkReplays() {
    return <p className="help">
        ...
        <I18nMessage tokens={{
        livestream_replay_instead: <Button button="link" label={__('Livestream Replay instead')} navigate={`/$/${PAGES.LIVESTREAM}?s=Replay`} />
      }}>
  ...
  return <Card className={classnames({
    'card--disabled': disabled || balance === 0
          ...
          <React.Fragment>
            <>
              <FileSelector disabled={disabled} currentPath={currentPath} onFileChosen={handleFileChange} placeholder={SIMPLE_SITE ? __('Select video, audio or image file to upload') : __('Select a file to upload')} autoFocus />
              {getUploadMessage()}
              {hasLivestreamData && linkReplays()}

              {fileSource === SOURCE_SELECT && <div className="main--empty empty">
                  <Spinner small />
                </div>}
            </>
            <div className="form-spacer">
              <FormField type="text" name="content_title" label={__('Title')} placeholder={__('Descriptive titles work best')} disabled={disabled} value={title} onChange={handleTitleChange} className="fieldset-group" max="200" ref={titleInput} />
            </div>
