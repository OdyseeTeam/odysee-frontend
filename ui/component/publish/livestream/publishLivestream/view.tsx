...
function PublishLivestream(props: Props) {
  ...
  const totalPages = hasLivestreamData && livestreamData.length > PAGE_SIZE ? Math.ceil(livestreamData.length / PAGE_SIZE) : 1;
  ...
  function getUploadMessage() {
    ...
    if (fileBitrate > BITRATE.RECOMMENDED) {
      return <p className="help--warning">
          <Icon icon={ICONS.INFO} />
          {fileBitrate > BITRATE.MAX ? __('Your video has a bitrate over ~16 Mbps and cannot be processed at this time. We suggest transcoding to provide viewers the best experience.') : __('Your video has a bitrate over 8 Mbps. We suggest transcoding to provide viewers the best experience.')}{' '}
          <Button button="link" label={__('Upload Guide')} href="https://help.odysee.tv/category-uploading/" />
        </p>;
    }
    ...
    if (isVid && !duration) {
      return <p className="help--warning">
          <Icon icon={ICONS.INFO} />
          {__("Couldn't detect the video encoding. This video will not be playable in most browsers. We recommend to use H264/AAC encoding with MP4 container.")}{' '}
          <Button button="link" label={__('Upload Guide')} href="https://help.odysee.tv/category-uploading/" />
        </p>;
    }
  ...
  return <Card className={classnames({
    'card--disabled': disabled || balance === 0
  })} actions={<div className="publish-row--no-margin">
          <React.Fragment>
            {
        ...
        /* @if TARGET='web' */
            ...
            <>
              {inEditMode && <fieldset-group>
                  <fieldset-section>
                    <label style={{
              marginBottom: 'var(--spacing-s)'
            }}>
                      {inEditMode && <FormField name="reuse-replay" label={__('Update only')} key="reuse-replay" type="radio" checked={liveEditType === 'update_only'} onClick={() => updatePublishForm({
            ...
            marginTop: 0
                    ...
                    {inEditMode && <FormField name="show-replays" label={replayTitleLabel} key="show-replays" type="radio" checked={liveEditType === 'use_replay'} onClick={() => updatePublishForm({
              liveEditType: 'use_replay'
            })} />}
                  </label>
                  <div className={classnames('replay-picker__container', {
            disabled: inEditMode && liveEditType !== 'use_replay'
                    ...
                    <fieldset-section>
                      ...
                      const durationElem = useStr ? item.data.fileDuration : durationMinutes === 1 ? __('%duration% minute', {
                        duration: durationMinutes
                      }) : __('%duration% minutes', {
                        duration: durationMinutes
                      });
                      return <React.Fragment key={item.data.fileLocation}>
                                    <tr className="livestream-data__row-spacer" />
                                    <tr onClick={() => setSelectedFileIndex((currentPage - 1) * PAGE_SIZE + i)} className={classnames('livestream-data__row', {
                          'livestream-data__row--selected': selectedFileIndex === (currentPage - 1) * PAGE_SIZE + i
                        })}>
                                      <td>
                                        <FormField type="radio" checked={selectedFileIndex === (currentPage - 1) * PAGE_SIZE + i} label={null} onChange={() => {}} onClick={() => setSelectedFileIndex((currentPage - 1) * PAGE_SIZE + i)} className="livestream-data__row-radio" />
                                      </td>
                                      <td>
                                        <div className="livestream-data__thumb-container">
                                          {item.data.thumbnails.slice(0, 3).map(thumb => <img key={thumb} className="livestream___thumb" src={thumb} />)}
                                        </div>
                                      </td>
                                      <td>
                                        {durationElem}
                                      ...
                                      <td>
                                        <CopyableText primaryButton copyable={normalizeUrlForProtocol(item.data.fileLocation)} snackMessage={__('Url copied.')} />
                                      </td>
                                    </tr>
                                  </React.Fragment>;
                    })}
                    ...
                    {totalPages > 1 && <fieldset-group class="fieldset-group--smushed fieldgroup--paginate">
                        <fieldset-section>
                          <ReactPaginate pageCount={totalPages} pageRangeDisplayed={2} previousLabel="‹" nextLabel="›" activeClassName="pagination__item--selected" pageClassName="pagination__item" previousClassName="pagination__item pagination__item--previous" nextClassName="pagination__item pagination__item--next" breakClassName="pagination__item pagination__item--break" marginPagesDisplayed={2} onPageChange={e => handlePaginateReplays(e.selected + 1)} forcePage={currentPage - 1} initialPage={currentPage - 1} containerClassName="pagination" />
                        </fieldset-section>
                      </fieldset-group>}
                  </div>
                </>}
              {showReplaySelector && !hasLivestreamData && !isCheckingLivestreams && <>
                  <label className="disabled" style={{
            marginTop: 0
          }}>
                    {inEditMode && <FormField name="show-replays" label={replayTitleLabel} key="show-replays" type="radio" checked={liveEditType === 'use_replay'} onClick={() => updatePublishForm({
              liveEditType: 'use_replay'
                  ...
                  <div className="empty disabled" style={{
            ...
            marginTop: 0
                    ...
                    {inEditMode && <FormField name="replay-source" label={replayTitleLabel} value="choose" key="show-replays-spin" type="radio" checked={liveEditType === 'use_replay'} onClick={() => updatePublishForm({
              liveEditType: 'use_replay'
            })} />}
                  </label>
              ...
              {inEditMode && <div className="file-upload">
                  <label style={{
            marginTop: 0
          }}>
                    <FormField name="replay-source" label={__('Upload Replay')} type="radio" checked={liveEditType === 'upload_replay'} onClick={() => updatePublishForm({
              liveEditType: 'upload_replay'
            })} />
                  </label>
                  <FileSelector disabled={liveEditType !== 'upload_replay'} currentPath={typeof filePath === 'string' ? filePath : filePath?.name} onFileChosen={handleFileChange} accept={SIMPLE_SITE ? 'video/mp4,video/x-m4v,video/*' : undefined} placeholder={__('Select video replay file to upload')} />
                  {getUploadMessage()}
                </div>}
            </>
            {
