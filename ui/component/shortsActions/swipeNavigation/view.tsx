...
const SwipeNavigationPortal = React.memo<Props>(({
  ...
  return createPortal(<div onClick={e => {
    ...
    if (el) {
      ...
      if (link && link.closest('.shorts-viewer__content-info, .shorts-page__view-toggle--overlay, .button--play')) {
    ...
    handlePlayPause();
  }} ref={overlayRef} className={classnames('swipe-navigation-overlay', className, {
    'swipe-navigation-overlay--enabled': isEnabled,
    'shorts__viewer--panel-open': sidePanelOpen
  })}>
        <MobileActions uri={uri} likeCount={likeCount} dislikeCount={dislikeCount} myReaction={myReaction} doReactionLike={doReactionLike} doReactionDislike={doReactionDislike} onCommentsClick={onCommentsClick} onInfoButtonClick={onInfoButtonClick} autoPlayNextShort={autoPlayNextShort} doToggleShortsAutoplay={doToggleShortsAutoplay} isUnlisted={isUnlisted} webShareable={webShareable} collectionId={collectionId} doOpenModal={doOpenModal} onShareClick={handleShareClick} />
