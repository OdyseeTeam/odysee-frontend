...
const ShortsActions = React.memo<Props>(({
  ...
  const content = <div className={classnames('shorts-page__navigation', {
    'shorts-page__navigation--mobile-desktop': isMobile
  })}>
        <>
          <Button className="shorts-page__actions-button shorts-page__actions-button--info" onClick={onInfoClick} icon={ICONS.INFO} iconSize={20} title={__('Show Details')} disabled={!hasPlaylist} />
          <Button className="shorts-page__actions-button shorts-page__actions-button--previous" onClick={() => {
        setCountersZeroed(true);
        onPrevious();
      }} icon={ICONS.UP} iconSize={24} title={__('Previous Short')} disabled={isAtStart || !hasPlaylist} />
          <Button className="shorts-page__actions-button shorts-page__actions-button--next" onClick={() => {
        setCountersZeroed(true);
        onNext();
      }} icon={ICONS.DOWN} iconSize={24} title={__('Next Short')} disabled={isAtEnd || !hasPlaylist} />
          <div className={classnames('shorts-page__ratings', {
        'shorts-page__ratings--no-slime': disableSlimes
      })} style={{ ...(disableReactions ? {
          visibility: 'hidden',
          pointerEvents: 'none'
        } : {}),
        '--ratings-gradient': (() => {
          ...
          return `linear-gradient(to bottom, var(--color-fire) ${fireStop}%, var(--color-slime) ${slimeStop}%)`;
            ...
            doReactionLike(uri);
          }} icon={myReaction === REACTION_TYPES.LIKE ? ICONS.FIRE_ACTIVE : ICONS.FIRE} iconSize={16} title={__('I Like This')} disabled={!hasPlaylist} requiresAuth authSrc="filereaction_like" className={classnames('shorts-page__actions-button button--file-action button-like', {
            'button--fire': myReaction === REACTION_TYPES.LIKE
              ...
              {countersZeroed ? <span className="counter-inline">0</span> : <Counter key={'fire-' + (claimId || '')} value={Number.isInteger(likeCount) ? likeCount : 0} precision={0} startFrom={0} />}
            </div>
            <div className="slime-and-count" style={disableSlimes ? {
          visibility: 'hidden'
        } : undefined}>
              <Button requiresAuth authSrc={'filereaction_dislike'} title={__('I dislike this')} className={classnames('shorts-page__actions-button button--file-action button-dislike', {
            'button--slime': myReaction === REACTION_TYPES.DISLIKE
          })} disabled={!hasPlaylist} label={<>
                    {myReaction === REACTION_TYPES.DISLIKE && <>
                        <div className="button__slime-stain" />
                        <div className="button__slime-drop1" />
                        <div className="button__slime-drop2" />
                      </>}
                  </>} iconSize={16} icon={myReaction === REACTION_TYPES.DISLIKE ? ICONS.SLIME_ACTIVE : ICONS.SLIME} onClick={() => {
            ...
            doReactionDislike(uri);
              ...
              {countersZeroed ? <span className="counter-inline">0</span> : <Counter key={'slime-' + (claimId || '')} value={Number.isInteger(dislikeCount) ? dislikeCount : 0} precision={0} startFrom={0} />}
            </div>
          </div>
          {channelUrl ? <div ref={followRef} className="shorts-actions__item" onMouseEnter={() => setAvatarHover(true)} onMouseLeave={() => setAvatarHover(false)} onClick={() => {
        const sub = {
        ...
        if (isSubscribed) {
          ...
          doChannelSubscribe(sub);
              ...
              <div className="shorts-floating-action shorts-floating-action--avatar">
                <ChannelThumbnail key={channelUrl} uri={channelUrl} hideStakedIndicator className="shorts-floating-action__avatar" />
                <div className={classnames('shorts-floating-action__subscribe', {
            ...
            </div> : <div className="shorts-actions__item shorts-actions__item--placeholder" />}

          <div className="shorts-actions__item">
            <Button className="shorts-page__actions-button shorts-page__actions-button--comments" onClick={onCommentsClick} icon={ICONS.COMMENTS_LIST} iconSize={16} title={__('Comments')} disabled={!hasPlaylist} />
            <p>{__('Comments')}</p>
          </div>
          ...
          <div className="shorts-actions__group">
            ...
            {!isUnlisted && <div className="shorts-actions__item">
                <Button className="shorts-page__actions-button" onClick={() => doOpenModal(MODALS.REPOST, {
            uri
          })} icon={ICONS.REPOST} iconSize={16} title={__('Repost this content')} disabled={!hasPlaylist} requiresChannel />
                <p>{__('Repost')}</p>
              </div>}
          </div>
          <div className="shorts-actions__group shorts-actions__group--bottom">
            <div className="shorts-actions__item">
              <Button className="shorts-page__actions-button shorts-page__actions-button--share" onClick={handleShareClick} icon={ICONS.SHARE} iconSize={16} title={isUnlisted ? __('Get a sharable link for your unlisted content') : __('Share')} disabled={!hasPlaylist} />
              <p>{__('Share')}</p>
            </div>
            <div className="shorts-actions__item">
              <Button className={classnames('shorts-page__actions-button button-bubble', {
            'button-bubble--active': autoPlayNextShort
          })} title={__('Autoplay Next')} onClick={doToggleShortsAutoplay} icon={ICONS.AUTOPLAY_NEXT} iconSize={16} disabled={isLoading || !hasPlaylist} />
              <p>{__('Auto Next')}</p>
            </div>
          </div>
        </>
      </div>;
  const portalTarget = typeof document !== 'undefined' ? document.querySelector('.shorts__viewer') || document.querySelector('.content__cover--shorts') : null;
  const effectOverlays = portalTarget && <>
        {fireEffect && createPortal(<div className="shorts-viewer-flames">
              {Array.from({
        length: 50
      }, (_, i) => <div key={i} className="shorts-viewer-flames__particle" style={{
