...
export default function SubscribeButton(props: Props) {
  ...
  const subscriptionLabel = isSubscribed ? __('Following --[button label indicating a channel has been followed]--') : __('Follow');
  ...
  if (!preferEmbed && isSubscribed && !permanentUrl && rawChannelName) {
    return <div className="button-group button-group-subscribed">
        <Button ref={buttonRef} iconColor="red" largestLabel={isMobile && shrinkOnMobile ? '' : subscriptionLabel} icon={ICONS.UNSUBSCRIBE} button={'alt'} requiresAuth={IS_WEB} label={label} title={titlePrefix} href={isEmbed && `${PAGES.AUTH_SIGNIN}?redirect=${encodeURIComponent(formatLbryUrlForWeb(uri))}`} onClick={!isEmbed ? e => {
        e.stopPropagation();
  ...
  return !preferEmbed && permanentUrl && claimName ? <div className="button-group">
      <Button ref={buttonRef} iconColor="red" className={`button-following${isSubscribed ? ' button-following--active' : ''}`} icon={isSubscribed && isHovering && !isBellHovering ? ICONS.UNSUBSCRIBE : isSubscribed ? ICONS.SUBSCRIBED : ICONS.SUBSCRIBE} button={'alt'} requiresAuth={IS_WEB} label={label} title={titlePrefix} href={isEmbed && `/$/${PAGES.AUTH_SIGNIN}?redirect=${encodeURIComponent(formatLbryUrlForWeb(uri))}`} onClick={!isEmbed ? e => {
      e.stopPropagation();
      ...
      if (isSubscribed) {
        ...
        {isSubscribed && uiNotificationsEnabled && <span ref={bellRef} className="button-following__bell" role="button" tabIndex={0} aria-label={notificationsDisabled ? __('Turn on notifications') : __('Turn off notifications')} onClick={e => {
        e.stopPropagation();
