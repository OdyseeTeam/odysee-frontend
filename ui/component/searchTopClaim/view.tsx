...
export default function SearchTopClaim(props: Props) {
  ...
  return <div className="search__header">
      {winningUri && <div className="claim-preview__actions--header">
          <a className="media__uri" target="_blank" rel="noreferrer" href="https://help.odysee.tv/category-blockchain/category-staking/increase/" title={__('Learn more about Credits on %DOMAIN%', {
        DOMAIN
      })}>
            <LbcSymbol prefix={__('Most supported')} />
          </a>
        </div>}
      {winningUri && winningClaim && <div className="card">
          <ClaimPreview hideRepostLabel showNullPlaceholder uri={winningUri} properties={claim => <span className="claim-preview__custom-properties">
                <ClaimRepostAuthor short uri={winningUri} />
                <ClaimEffectiveAmount uri={winningUri} />
              </span>} />
        </div>}
      {!winningUri && (isSearching || isResolvingWinningUri) && <div className="card">
          <ClaimPreview placeholder={'loading'} />
        </div>}
      {!winningUri && !isSearching && !isResolvingWinningUri && uriFromQuery && <div className="card card--section help--inline">
          <I18nMessage tokens={{
      ...
      {!hideLink && winningUri && <div className="section__actions--between section__actions--no-margin">
          <span />
          <Button button="link" className="search__top-link" label={<I18nMessage tokens={{
        name: <strong>{query}</strong>
      }}>View competing uploads for %name%</I18nMessage>} navigate={`/$/${PAGES.TOP}?name=${query}`} iconRight={ICONS.ARROW_RIGHT} />
        </div>}
    </div>;
}