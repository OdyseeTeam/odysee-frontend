...
export default function CopyableText(props: Props) {
  ...
  return <FormField type={maskInput ? 'password' : 'text'} className="form-field--copyable" hideValue={hideValue} readOnly name={name} label={label} value={copyable || ''} ref={input} onFocus={onFocus} inputButton={<>
        <Button button={primaryButton ? 'primary' : 'secondary'} icon={ICONS.COPY} onClick={handleCopyText} aria-label={__('Copy %copyable%', {
      copyable: copyable
    })} />
        {linkTo && <Button button={primaryButton ? 'primary' : 'secondary'} icon={ICONS.EXTERNAL} navigate={`${linkTo}${copyable}`} description={'Link'} />}
      </>} helper={enableInputMask && <Button button="link" onClick={() => setMaskInput(!maskInput)} label={maskInput ? __('Show') : __('Hide')} />} />;
}