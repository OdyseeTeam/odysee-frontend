...
type Props = {
  ...
  doAccountTipStatus: (any) => void,
};

export default function WalletFiatBalance(props: Props) {
  const { accountTotals, doAccountTipStatus } = props;
  const { total_received_unpaid, total_paid_out } = accountTotals;

  React.useEffect(() => {
    if (total_received_unpaid === undefined || total_paid_out === undefined) {
      doAccountTipStatus({ getTotals: true });
    }
  }, [doAccountTipStatus, total_paid_out, total_received_unpaid]);
