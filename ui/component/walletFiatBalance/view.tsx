...
export default function WalletFiatBalance(props: Props) {
  ...
  const {
    total_tipped: totalTippedAmount
  } = accountStatus || {};
  React.useEffect(() => {
    doTipAccountStatus();
  }, [doTipAccountStatus]);
  return <Card title={<>{__('Cash Balance')}</>} subtitle={__('You can view your balance and transaction history on Stripe from the Bank Accounts section.')} background actions={<>
