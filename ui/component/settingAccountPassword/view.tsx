...
export default function SettingAccountPassword(props: Props) {
  ...
  return <SettingsRow title={title} subtitle={subtitle} multirow>
      <Form onSubmit={handleSubmit} className="section">
        {hasPassword && <FormField type="password" name="setting_set_old_password" label={__('Old Password')} value={oldPassword} onChange={e => setOldPassword(e.target.value)} />}
        <FormField type="password" name="setting_set_new_password" label={__('New Password')} value={newPassword} onChange={e => setNewPassword(e.target.value)} />

        <div className="section__actions">
          <Button button="primary" type="submit" label={__('Set Password')} disabled={!newPassword} />
          {hasPassword ? <Button button="link" label={__('Forgot Password?')} navigate={`/$/${PAGES.AUTH_PASSWORD_RESET}`} /> : <Button button="link" label={__('Cancel')} onClick={() => goBack()} />}
        </div>
      </Form>
      {passwordSetError && <div className="section">
