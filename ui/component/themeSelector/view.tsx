...
export default function ThemeSelector(props: Props) {
  ...
  return <>
      <fieldset-section>
        <FormField name="theme_select" type="select" onChange={onThemeChange} value={currentTheme} disabled={automaticDarkModeEnabled}>
      ...
      <fieldset-section class="theme-checkbox">
        <FormField type="checkbox" name="automatic_dark_mode" onChange={() => onAutomaticDarkModeChange(!automaticDarkModeEnabled)} checked={automaticDarkModeEnabled} label={__('Automatic dark mode')} />

        {automaticDarkModeEnabled && <fieldset-group class="fieldset-group--smushed">
            <FormField type="select" name="automatic_dark_mode_range_start" onChange={value => onChangeTime(value, {
          fromTo: 'from',
          time: 'hour'
        })} value={darkModeTimes.from.hour} label={__('From --[initial time]--')}>
              {startHours.map(time => <option key={time} value={time}>
                  {formatHour(time, clock24h)}
                </option>)}
            </FormField>

            <FormField type="select" name="automatic_dark_mode_range_end" label={__('To --[final time]--')} onChange={value => onChangeTime(value, {
          fromTo: 'to',
          time: 'hour'
        })} value={darkModeTimes.to.hour}>
              {endHours.map(time => <option key={time} value={time}>
                  {formatHour(time, clock24h)}
                </option>)}
            </FormField>
          </fieldset-group>}
