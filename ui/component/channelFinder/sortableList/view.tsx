...
type Props = {
  list: Array<any>;
  onGetElemAtIndex: (item: any, index: number) => React.ReactElement<React.ComponentProps<any>, any>;
...
export default function SortableList(props: Props) {
  ...
  const DraggableItem = ({
    ...
    return <Lazy.Draggable draggableId={item} index={index}>
        ...
        return <div className={classnames('sortable__item', {
  ...
  const DroppableBin = ({
    ...
    return <Lazy.Droppable droppableId="bin-1">
        {(provided
      /* ,snapshot */
      ) => <div ref={provided.innerRef} {...provided.droppableProps} className={classnames('sortable__bin', className)}>
            {list.map((item, index) => <DraggableItem key={item} item={item} index={index} />)}
