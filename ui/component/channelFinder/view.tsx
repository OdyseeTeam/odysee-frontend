...
type Props = {
  ...
  onSelectedUrisChanged: (change: "remove" | "add" | "reorder", params: {
    uri?: string;
    to?: number;
    from?: number;
  }) => void;
...
export default function ChannelFinder(props: Props) {
  ...
  // **************************************************************************
  // **************************************************************************
  const CenteredSpinner = (props: {}) => <div className="main--empty">
      <Spinner />
    </div>;

  const LighthouseResults = (props: {}) => <>
      {!lighthouseResponse.loading && searchTerm?.length >= LIGHTHOUSE_MIN_CHARACTERS && lighthouseResponse.results && lighthouseResponse.results.filter(uri => uri !== resolvedUri).map(uri => <Entry key={uri} uri={uri} claim={claimsByUri[uri]} resolvingUris={resolvingUris} onClick={e => handleSuggestionClicked(uri)} iconRight={selectedUris.includes(uri) ? ICONS.COMPLETE : undefined} hideInvalid />)}
    </>;

  const UriSearchResult = (props: {}) => <>
      {!isResolvingUri && <>
          {claimsByUri[uriSearchTerm] ? <Entry key={claimsByUri[uriSearchTerm].permanent_url} uri={claimsByUri[uriSearchTerm].permanent_url} claim={claimsByUri[uriSearchTerm]} resolvingUris={resolvingUris} onClick={e => handleSuggestionClicked(claimsByUri[uriSearchTerm].permanent_url)} iconRight={selectedUris.includes(claimsByUri[uriSearchTerm].permanent_url) ? ICONS.COMPLETE : undefined} hideInvalid /> : isUrl ? <div className="main--empty">{__('No results')}</div> : null}
        </>}
    </>;

  const MiscSuggestions = (props: {}) => {
    const show = showSubscriptions && subscriptionUris.length > 0 && !lighthouseResponse.loading;
    return show ? <div className="channel-finder__misc-suggestions">
        <label>{__('Following')}</label>
        {subscriptionUris.map(uri => <Entry key={uri} uri={uri} claim={claimsByUri[uri]} resolvingUris={resolvingUris} onClick={e => handleSuggestionClicked(uri)} iconRight={selectedUris.includes(uri) ? ICONS.COMPLETE : undefined} hideInvalid />)}
      </div> : null;
  };
  ...
  function handleKeyPress(e) {// We have to use 'e.key' instead of 'e.keyCode' in this event.
  ...
  React.useEffect(() => {
    ...
    if (searchTermDebounced) {
      ...
      const term = isUrl ? searchTermDebounced : `lbry://${!searchTermDebounced.startsWith('@') ? '@' : ''}${searchTermDebounced}`;
  ...
  return <div className="channel-finder">
      <div className="channel-finder__top">
        <div className="channel-finder__input">
          <FormField type="text" name="search_term" className="form-field--address" label={label} placeholder={`\u{1F50E} ${placeholder || __('Enter channel name or URL')}`} value={searchTerm} error={isUrl && uriSearchTermError} onKeyPress={e => handleKeyPress(e)} onChange={e => setSearchTerm(e.target.value)} />
        </div>
      </div>
      <div className="channel-finder__middle">
        <div className="channel-finder__suggestion-list">
          {lighthouseResponse.loading || isResolvingUri ? <CenteredSpinner /> : <>
        ...
        <div className="channel-finder__selected-list">
          ...
          <SortableList list={selectedUris} onGetElemAtIndex={(uri, index) => <Entry key={uri} uri={uri} claim={claimsByUri[uri]} resolvingUris={resolvingUris} onClick={e => handleSelectedItemClicked(uri)} iconRight={ICONS.DELETE} iconRightOnHoverOnly iconRightErrorColor />} onDragEnd={handleSelectedItemDragged} />
        </div>
      </div>
      <div className="channel-finder__bottom">
        <FormField type="checkbox" name="suggest_followed_channels" label={__('Suggest followed channels')} checked={showSubscriptions} onChange={() => setShowSubscriptions(prev => !prev)} />
      </div>
    </div>;
}