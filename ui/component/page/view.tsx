...
const Footer = lazyImport(() => import('web/component/footer'
...
function Page(props: Props) {
  ...
  const theaterMode = renderMode === 'video' || renderMode === 'audio' || renderMode === 'unsupported' ? videoTheaterMode : false;
  ...
  return <>
      ...
      {!noHeader && <Header authHeader={authPage} authRedirect={authRedirect} backout={backout} sidebarOpen={sidebarOpen} isAbsoluteSideNavHidden={isAbsoluteSideNavHidden} setSidebarOpen={openSidebar} hideSidebarToggle={noSideNavigation} />}
      ...
      'main--popout-chat': isPopoutWindow
        ...
        {!authPage && (settingsPage ? <SettingsSideNavigation /> : !noSideNavigation && <SideNavigation sidebarOpen={sidebarOpen} setSidebarOpen={openSidebar} isMediumScreen={isSmallScreen} isOnFilePage={isOnFilePage} />)}
        ...
        'sidebar--pusher--filepage': !fullWidthPage
          ...
          <main id={'main-content'} className={classnames(MAIN_CLASS, className, {
          ...
          {!noFooter && <React.Suspense fallback={null}>
              <Footer />
            </React.Suspense>}
        </div>
      </div>
