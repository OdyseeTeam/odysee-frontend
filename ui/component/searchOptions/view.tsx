...
const SearchOptions = (props: Props) => {
  ...
  const setMinDurationMinutesDebounced = React.useCallback(debounce(m => updateSearchOptions(SEARCH_OPTIONS.MIN_DURATION, m), 750), []);
  const setMaxDurationMinutesDebounced = React.useCallback(debounce(m => updateSearchOptions(SEARCH_OPTIONS.MAX_DURATION, m), 750), []);
  ...
  const OBJ_TO_OPTION_ELEM = obj => {
    return Object.entries(obj).map(x => {
      return <option key={x[0]} value={x[0]}>
          {__(String(x[1]))}
        </option>;
    });
  };
  ...
  const typeElem = <>
      <div className="filter-values">
        <div>
          ...
          return <Button key={option} button="alt" // $FlowFixMe https://github.com/facebook/flow/issues/2221
          label={__(t[1])} className={classnames(`button-toggle`, {
            'button-toggle--active': options[SEARCH_OPTIONS.CLAIM_TYPE] === option
          })} onClick={() => setSearchOption(SEARCH_OPTIONS.CLAIM_TYPE, option)} />;
        })}
        </div>
        <Button button="close" className={classnames('close-button', {
        'close-button--visible': options[SEARCH_OPTIONS.CLAIM_TYPE] !== SEARCH_OPTIONS.INCLUDE_FILES_AND_CHANNELS
      })} icon={ICONS.REMOVE} onClick={() => updateSearchOptions(SEARCH_OPTIONS.CLAIM_TYPE, SEARCH_OPTIONS.INCLUDE_FILES_AND_CHANNELS)} />
      </div>
      {options[SEARCH_OPTIONS.CLAIM_TYPE] === SEARCH_OPTIONS.INCLUDE_FILES && <div className="media-types">
          {Object.entries(TYPES_ADVANCED).map(t => {
        const option = t[0];
        return <FormField key={option} name={option} type="checkbox" blockWrap={false} disabled={options[SEARCH_OPTIONS.CLAIM_TYPE] !== SEARCH_OPTIONS.INCLUDE_FILES} // $FlowFixMe https://github.com/facebook/flow/issues/2221
        label={__(t[1])} checked={!isFilteringByChannel && options[option]} onChange={() => updateSearchOptions(option, !options[option])} />;
      })}
        </div>}
    </>;
  const otherOptionsElem = <>
      <div className="filter-values">
        <FormField type="checkbox" name="exact-match" checked={options[SEARCH_OPTIONS.EXACT]} onChange={() => updateSearchOptions(SEARCH_OPTIONS.EXACT, !options[SEARCH_OPTIONS.EXACT])} label={__('Exact match')} />
        <Icon className="icon--help" icon={ICONS.HELP} tooltip size={16} customTooltipText={__('Find results that include all the given words in the exact order.\nThis can also be done by surrounding the search query with quotation marks (e.g. "hello world").')} />
      </div>
    </>;
  const uploadDateElem = <div className="filter-values">
      <FormField type="select" name="upload-date" value={options[SEARCH_OPTIONS.TIME_FILTER]} onChange={e => updateSearchOptions(SEARCH_OPTIONS.TIME_FILTER, e.target.value)} blockWrap={false}>
        {OBJ_TO_OPTION_ELEM(TIME_FILTER)}
      </FormField>
      <Button button="close" className={classnames('close-button', {
      'close-button--visible': options[SEARCH_OPTIONS.TIME_FILTER]
    })} icon={ICONS.REMOVE} onClick={() => updateSearchOptions(SEARCH_OPTIONS.TIME_FILTER, '')} />
    </div>;
  const sortByElem = <div className="filter-values">
      <FormField type="select" name="sort-by" blockWrap={false} value={options[SEARCH_OPTIONS.SORT]} onChange={e => updateSearchOptions(SEARCH_OPTIONS.SORT, e.target.value)}>
        {OBJ_TO_OPTION_ELEM(SORT_BY)}
      </FormField>
    </div>;
  const durationElem = <div className="filter-values">
      <div className="claim-search__duration-inputs-container">
        <FormField label={__('Min Minutes')} type="number" name="min_duration__minutes" blockWrap={false} value={minDurationMinutes} onChange={e => {
        setMinDurationMinutes(e.target.value);
        setMinDurationMinutesDebounced(e.target.value);
      }} />
        <FormField label={__('Max Minutes')} type="number" name="max_duration__minutes" blockWrap={false} value={maxDurationMinutes} onChange={e => {
  ...
  const uploadDateLabel = options[SEARCH_OPTIONS.CLAIM_TYPE] === SEARCH_OPTIONS.INCLUDE_CHANNELS ? __('Creation Date') : __('Upload Date');
  return <div>
      <div>
        <Button button="alt" label={__('Filter')} icon={ICONS.FILTER} iconRight={expanded ? ICONS.UP : ICONS.DOWN} onClick={toggleSearchExpanded} />
        {searchInLanguage && <LangFilterIndicator />}
      </div>
      <Form className={classnames('search__options', {
