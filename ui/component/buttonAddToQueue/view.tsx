...
function ButtonAddToQueue(props: Props) {
  ...
  async function handleQueue(e) {
    ...
    if (!hasClaimInQueue) {
      ...
      if (playingUrl) {
        ...
        if (!hasPlayingUriInQueue && autoplayMedia) doSetPlayingUri({ ...playingUri,
          ...paramsToAdd
        });
  ...
  if (menuItem) {
    return <MenuItem className="comment__menu-option" onSelect={handleQueue}>
  ...
  return <div className="claim-preview__hover-actions third-item">
      <Button title={__('Queue Mode')} label={label} className="button--file-action" icon={hasClaimInQueue ? ICONS.PLAYLIST_FILLED : ICONS.PLAYLIST_ADD} onClick={e => handleQueue(e)} tabIndex={focusable ? 0 : -1} />
    </div>;
}
