...
export default function HeaderProfileMenuButton(props: HeaderMenuButtonProps) {
  ...
  return <>
      {open && !isShortsPage && <Global styles={{
      body: {
      ...
      <div className="header__buttons">
        {!isMobile && <Menu>
            <Tooltip title={currentTheme === 'light' ? __('Dark') : __('Light')}>
              <Button className="header__navigationItem--icon" onClick={() => handleThemeToggle(automaticDarkModeEnabled, currentTheme, authenticated)}>
        ...
        {(isMobile || authenticated) && <Button id="basic-button" aria-controls={open ? 'basic-menu' : undefined} aria-haspopup="true" aria-expanded={open ? 'true' : undefined} onClick={handleClick} className={classnames('header__navigationItem', {
        'header__navigationItem--icon': !activeChannelUrl,
        'header__navigationItem--profilePic': activeChannelUrl
      })}>
            {activeChannelUrl ? <ChannelThumbnail uri={activeChannelUrl} hideTooltip small noLazyLoad showMemberBadge allowGifs /> : <Icon size={18} icon={ICONS.ACCOUNT} aria-hidden />}
          </Button>}

        {authenticated ? <ClickAwayListener onClickAway={handleClickAway}>
            <MuiMenu {...menuProps}>
              ...
              <HeaderMenuLink useMui {...uploadProps} page={PAGES.YOUTUBE_SYNC} icon={ICONS.YOUTUBE} name={__('Sync YouTube Channel')} />
              ...
              <HeaderMenuLink useMui page={PAGES.MEMBERSHIPS_LANDING} icon={ICONS.MEMBERSHIP} name={__('Memberships')} />
              ...
              <MuiMenuItem onClick={signOut}>
                <div className="menu__link" style={{
          ...
          </ClickAwayListener> : <ClickAwayListener onClickAway={handleClickAway}>
            <MuiMenu {...menuProps}>
