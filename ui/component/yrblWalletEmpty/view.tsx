...
export default function YrblWalletEmpty(props: Props) {
  const {
    includeWalletLink = false,
    type = 'sad'
  } = props;
  return <div className="main--empty">
      <Yrbl type={type} title={__('Your wallet is empty')} subtitle={<div>
            <p>
              <I18nMessage tokens={{
          lbc: <LbcSymbol />
        }}>
          ...
          lbc: <LbcSymbol />
            ...
            <Button button="primary" icon={ICONS.REWARDS} label={__('Receive Credits')} navigate={`/$/${PAGES.REWARDS}`} />
            {includeWalletLink && <Button icon={ICONS.RECEIVE} button="secondary" label={__('Your Address')} navigate={`/$/${PAGES.RECEIVE}`} />}
          </div>} />
    </div>;
}