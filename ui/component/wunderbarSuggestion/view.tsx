...
export default function WunderbarSuggestion(props: Props) {
  ...
  if (isResolvingUri) {
    return <ComboboxOption value={uri}>
  ...
  return <ComboboxOption value={uri}>
      <div className={classnames('wunderbar__suggestion', {
      'wunderbar__suggestion--channel': isChannel
    })}>
        {isChannel && <ChannelThumbnail small uri={uri} />}
        {!isChannel && <FileThumbnail uri={uri}>
            {
          /* @if TARGET='app' */
        }
            {!isCollection && <div className="claim-preview__file-property-overlay">
                <FileProperties uri={uri} small iconOnly />
              </div>}
            {
          /* @endif */
        }
            {isCollection && <div className="claim-preview__claim-property-overlay">
