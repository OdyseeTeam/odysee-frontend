...
const ClaimLink = (props: Props) => {
  ...
  if (allowPreview) {
    return <div className="claim-link">
        <ClaimLinkPreview uri={uri} title={claim?.value?.title} channel={claim?.signing_channel?.value?.title || claim?.signing_channel?.name} parentCommentId={parentCommentId} />
      </div>;
  }

  return <Button button="link" title={__("This channel isn't staking enough Credits for link previews.")} label={children} className="button--external-link" navigate={uri} />;
};
