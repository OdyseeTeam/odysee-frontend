...
function getChannelNameLink(channelUrl: string, channelName: string | null | undefined) {
  return <UriIndicator link showAtSign uri={channelUrl} channelInfo={{
    uri: channelUrl,
    name: channelName
  }} />;
}

export function generateNotificationTitle(rule: string, notificationParams: any, channelName: string | null | undefined) {
  switch (rule) {
    case RULE.COMMENT:
      {
        ...
        if (notificationParams?.dynamic?.amount > 0) {
          const amountStr = `${parseFloat(notificationParams.dynamic.amount)} ${notificationParams.dynamic.currency || 'LBC'}`;
          return channelUrl ? <I18nMessage tokens={{
          ...
          </I18nMessage> : notificationParams.device.title;
        } else {
          return channelUrl ? <I18nMessage tokens={{
            commenter: getChannelNameLink(channelUrl, channelName),
            title: <span className="notification__claim-title">{notificationParams.dynamic?.claim_title}</span>
    ...
    case RULE.NEW_CONTENT:
      {
        ...
        return channelUrl ? <I18nMessage tokens={{
          creator: getChannelNameLink(channelUrl, channelName)
        }}>
    ...
    case RULE.NEW_LIVESTREAM:
      {
        const channelUrl = notificationParams?.dynamic?.channel_url;
        return channelUrl ? <I18nMessage tokens={{
          streamer: getChannelNameLink(channelUrl, channelName)
        }}>
    ...
    case RULE.NEW_MEMBER:
      {
        ...
        }}>%creator_name%, you have a new member!</I18nMessage>;
      }
