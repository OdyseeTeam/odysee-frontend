...
export default function CreatorAnalytics(props: Props) {
  ...
  return <React.Fragment>
      {!stats && <div className="main--empty">
          {fetchingStats ? <Spinner delayed /> : <div>
              {error && <Yrbl type="sad" title={error === GENERIC_ERROR ? __('No stats found') : __('Error fetching stats')} subtitle={error === GENERIC_ERROR ? __('There are no stats for this channel yet, it will take a few views. Make sure you are signed in with the correct email and have data sharing turned on.') : __("You are not able to see this channel's stats. Make sure you are signed in with the correct email and have data sharing turned on.")} />}

              {!error && !channelHasClaims ? <Yrbl type="sad" title={__("You haven't uploaded anything")} subtitle={__('Upload something to start tracking your stats!')} actions={<div className="section__actions">
                      <Button button="primary" label={__('Upload Something')} onClick={() => {
            history.push(`/$/${PAGES.UPLOAD}`);
          }} />
                    </div>} /> : <Yrbl title={channelHasClaims ? __('No recent uploads') : __("You haven't uploaded anything with this channel yet!")} actions={<div className="section__actions">
                      <Button button="primary" label={__('Upload Something')} onClick={() => history.push(`/$/${PAGES.UPLOAD}`)} />
                    </div>} />}
            </div>}
        </div>}

      {stats && <div className="section">
          <div className="columns">
            <Card iconColor title={<span>{getChannelSubCountStr(stats.ChannelSubs)}</span>} icon={ICONS.SUBSCRIBE} subtitle={<div className="card__data-subtitle">
                  <span>
                    {0 > -1 && '+'}{' '}
            ...
            <Card icon={ICONS.EYE} title={<span>{__('%all_content_views% views', {
            all_content_views: stats.AllContentViews
          })}</span>} subtitle={<div className="card__data-subtitle">
                  <span>
          ...
          {stats.VideoURITopNew ? <Card className="section" title={__('Most viewed recent content')} body={<React.Fragment>
                  ...
                  <div className="section__subtitle card__data-subtitle">
                    <span>
                      {stats.VideoViewsTopNew === 1 ? __('1 view') : __('%view_count% views - %view_count_change% this week', {
              ...
              view_count_change: stats.VideoViewChangeTopNew
                ...
                </React.Fragment>} /> : <Card className="section" title={__('Your recent content')} subtitle={!stats.VideoURITopNew && __("No recent uploads found for this channel. Upload something new and track how it's performing here.")} actions={<div className="section__actions">
                  <Button button="primary" icon={ICONS.PUBLISH} label={__('Upload')} onClick={() => history.push(`/$/${PAGES.UPLOAD}`)} />
                </div>} />}

          {stats.VideoURITopCommentNew && stats.VideoCommentTopCommentNew > 0 && <Card className="section" title={__('Most commented recent content')} body={<React.Fragment>
                  <div className="card--inline">
                    <ClaimPreview uri={stats.VideoURITopCommentNew} />
                  </div>
                  <div className="section__subtitle card__data-subtitle">
                    <span>
                      {stats.VideoCommentTopCommentNew === 1 ? __('1 comment') : __('%comment_count% comments - %comment_count_change% this week', {
              comment_count: stats.VideoCommentTopCommentNew,
              comment_count_change: stats.VideoCommentChangeTopCommentNew
            })}
                    </span>
                    {stats.VideoCommentChangeTopCommentNew > 0 && <Icon icon={ICONS.TRENDING} iconColor="green" size={18} />}
                  </div>
                </React.Fragment>} />}

          {stats.VideoURITopAllTime && <Card className="section" title={__('Most viewed content all time')} body={<React.Fragment>
                  <div className="card--inline">
