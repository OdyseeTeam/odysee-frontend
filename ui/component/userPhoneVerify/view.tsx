...
type Props = {
  verifyUserPhone: (arg0: string) => void;
  resetPhone: () => void;
...
class UserPhoneVerify extends React.PureComponent<Props, State> {
  ...
  handleCodeChanged(event: React.SyntheticEvent<any>) {
    this.setState({
  ...
  render() {
    ...
    return <Card title={__('Enter the verification code')} subtitle={<>
            ...
            <Form onSubmit={this.handleSubmit.bind(this)}>
              <FormField type="text" name="code" placeholder="1234" value={this.state.code} onChange={event => {
          this.handleCodeChanged(event);
        }} label={__('Verification Code')} error={phoneErrorMessage} inputButton={<Submit label={__('Verify')} />} />
            </Form>
            <p className="help">
              <I18nMessage tokens={{
