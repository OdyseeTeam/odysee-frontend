...
const UpcomingClaims = (props: Props & StateProps & DispatchProps) => {
  ...
  const Header = () => {
    ...
    return <div className="claim-grid__header" onClick={() => {
      showHideSetting && hideScheduled(!hideUpcoming);
        ...
        <div className="button__content">
          ...
          {showHideSetting && <div className="upcoming-grid__visibility" onClick={() => hideScheduled(!hideUpcoming)}>
  ...
  return <div className={classnames('md:mb-xl', {
    'upcoming-grid': showHideSetting && tileLayout,
    'upcoming-list': !showHideSetting || !tileLayout,
    'upcoming-grid--extended': showAllUpcoming,
    'upcoming-grid--closed': hideUpcoming && showHideSetting || showHideSetting && list.total === 0 && !loading || !showHideSetting && list.total === 0 && !loading
  })}>
      <Header />
      {loading && <section className="claim-grid">
          {new Array(upcomingMax).fill(1).map((x, i) => <ClaimPreviewTile key={i} placeholder="loading" pulse />)}
        </section>}

      {!loading && list.total > 0 && <ClaimList uris={list.uris} tileLayout={tileLayout} showNoSourceClaims />}
      {list.total > upcomingMax && !showAllUpcoming && !isChannelPage && !hideUpcoming && <div className="upcoming-list__view-more">
          <Button label={__('Show more upcoming content')} button="link" iconRight={ICONS.ARROW_RIGHT} className="claim-grid__title--secondary" onClick={() => setShowAllUpcoming(true)} />
        </div>}

      {showAllUpcoming && !hideUpcoming && <div className="upcoming-list__view-more">
          <Button label={__('Show less upcoming content')} button="link" iconRight={ICONS.ARROW_RIGHT} className="claim-grid__title--secondary" onClick={() => {
        if (isMobileScreen) window.scrollTo({
          top: 0,
          left: 0,
          behavior: 'smooth'
        });
