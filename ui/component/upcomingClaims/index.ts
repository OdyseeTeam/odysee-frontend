...
const selectOptions = createSelector(state => state.comments.moderationBlockList, state => selectMutedAndBlockedChannelIds(state), (state, name, channelIds) => channelIds, (state, name, channelIds, isLivestream) => isLivestream, (state, name, channelIds, isLivestream, limitPerChannel) => limitPerChannel, (blocked, mutedAndBlockedIds, channelIds, isLivestream, limitPerChannel) => {
  return {
    page: 1,
    page_size: 50,
    no_totals: true,
    ...
    ...(isLivestream && limitPerChannel ? {
      limit_claims_per_channel: limitPerChannel
    } : {})
  };
}, {
  memoizeOptions: {
...
export default connect<_, Props, _, _, _, _>(select, perform, null, {
  areStatePropsEqual: (next: any, prev: any) => {
    if (prev.livestreamUris !== undefined && next.livestreamUris === undefined || prev.scheduledUris !== undefined && next.scheduledUris === undefined) {
