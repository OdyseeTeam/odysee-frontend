...
export default function FormFieldDurationCombo(props: Props) {
  ...
  return <fieldset-group class="ff-duration-combo fieldset-group--smushed">
      <FormField label={label || __('Duration')} name="duration_value" type="number" className="form-field--price-amount" min={min} value={duration.value || value} onWheel={e => e.preventDefault()} onChange={handleValueChange} onBlur={handleBlur} placeholder={placeholder || 5} disabled={disabled} step={step || 'any'} />
      <FormField label={'\u{2009}'} name="duration_unit" id="duration_unit" type="select" className="input--currency-select" disabled={disabled} onChange={handleUnitChange} value={duration.unit}>
        {(units || Object.keys(DEFAULT_UNITS)).map(c => <option key={c} value={c}>
