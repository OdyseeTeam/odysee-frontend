...
export default function LivestreamHyperchats(props: Props) {
  ...
  const stickerSuperChats = hyperChatsByAmount && hyperChatsByAmount.filter(({
    comment
  }) => !!parseSticker(comment));
  ...
  return !superChatTopTen ? null : <Slider isMobile={isMobile} hyperchatsHidden={hyperchatsHidden}>
      <div ref={elRef} className={classnames('livestream-hyperchats__wrapper', {
      'livestream-hyperchats__wrapper--mobile': isMobile
    })}>
        <div className="livestream-hyperchats">
          {pinnedComment && onPinClick && <div className={classnames('livestream-hyperchat livestream-hyperchat--pin', {
          active: pinActive
        })} onClick={onPinClick}>
          ...
          return <div key={comment_id} className={classnames('livestream-hyperchat', {
            ...
            active: selectedHyperchat && selectedHyperchat.comment_id === comment_id
                ...
                <div className={classnames('livestreamHyperchat__info', {
              'livestreamHyperchat__info--notSticker': stickerSuperChats && !isSticker
            })}>
                  <div className="livestreamHyperchat__info--user">
                    <CreditAmount hideTitle size={10} className="livestreamHyperchat__amount--large" amount={support_amount} isFiat={is_fiat} />
                  </div>
                </div>
              </div>;
        })}

          {showMore && <div className="chat__show-hyperchats">
              <Button title={__('Show More...')} button="inverse" className="close-button" onClick={() => toggleHyperChat()} iconRight={ICONS.ARROW_RIGHT} />
            </div>}
        </div>
      </div>
    </Slider>;
...
const Slider = (sliderProps: SliderProps) => {
  ...
  return <Slide direction="left" in={!hyperchatsHidden} mountOnEnter unmountOnExit>
      {children}
    </Slide>;
};