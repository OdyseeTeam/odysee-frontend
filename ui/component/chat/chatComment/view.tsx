...
export default function ChatComment(props: Props) {
  ...
  React.useEffect(() => {
    if (!exchangeRate) Lbryio.getExchangeRates().then(({
      LBC_USD
  ...
  React.useEffect(() => {
    if (hasUserMention) setUserMention(false); // eslint-disable-next-line react-hooks/exhaustive-deps -- TODO: no idea if valid
  }, [activeChannelClaim]);
  return <li className={classnames('livestream__comment', {
    ...
    'livestream__comment--minimal': isCompact
      ...
      {supportAmount > 0 && <div className="livestream-comment__hyperchat-banner">
      ...
      <div className="livestream-comment__body">
        ...
        {!isCompact || isPinned ? <>
            ...
            <div className="livestream-comment__info">
              <div className="livestream-comment__meta-information">
                ...
                'comment__author--creator': isStreamer
                  ...
                  <CommentMenuList uri={uri} commentId={commentId} authorUri={authorUri} commentIsMine={commentIsMine} isPinned={isPinned} isTopLevel disableEdit disableRemove={comment.removed} isLiveComment handleDismissPin={handleDismissPin} setQuickReply={handleCommentClick} />
                </Menu>
                ...
                {!isStreamer && !isModerator && !isGlobalMod && !odyseeMembership && isSprout && <CommentBadge label={__('Sprout')} icon={ICONS.BADGE_SPROUT} size={16} />}
                ...
                {isPinned && !hidePinLabel && <span className="comment__pin">
              ...
              {isSticker ? <div className="sticker__comment">
                  <OptimizedImage src={stickerUrlFromMessage} waitLoad loading="lazy" />
                </div> : <div className="livestream-comment__text">
                  {removed ? <Empty text={__('[Removed]')} /> : <ChatCommentContext.Provider value={{
              isLiveComment: true
            }}>
                      <MarkdownPreview content={message} promptLinks stakedLevel={stakedLevel} disableTimestamps setUserMention={setUserMention} hasMembership={Boolean(odyseeMembership)} isComment />
            ...
            {(isStreamer || isModerator || isGlobalMod || odyseeMembership) && <ChannelThumbnail uri={authorUri} xxxsmall />}
            {isGlobalMod && <CommentBadge label={__('Admin')} icon={ICONS.BADGE_ADMIN} size={16} />}
            {isModerator && <CommentBadge label={__('Moderator')} icon={ICONS.BADGE_MOD} size={16} />}
            {isStreamer && <CommentBadge label={__('Streamer')} icon={ICONS.BADGE_STREAMER} size={16} />}
            {!isStreamer && !isModerator && !isGlobalMod && isSprout && <CommentBadge label={__('Sprout')} icon={ICONS.BADGE_SPROUT} size={16} />}
            {odyseeMembership && <MembershipBadge membershipName={odyseeMembership} linkPage />}
            {creatorMembership && <MembershipBadge membershipName={creatorMembership} linkPage uri={uri} />}
            <Menu>
              <MenuButton className={classnames('button--uri-indicator comment__author', {
            ...
            {isSticker ? <div className="sticker__comment">
                ...
                {removed ? <Empty text={__('[Removed]')} /> : <MarkdownPreview content={message} promptLinks stakedLevel={stakedLevel} disableTimestamps setUserMention={setUserMention} hasMembership={Boolean(odyseeMembership)} isComment isMinimal={isCompact} />}
              </div>}
          </div>}
      </div>
      ...
      {handleDismissPin && !isMobile && <Button title={__('Dismiss')} button="inverse" className="close-button" onClick={handleDismissPin} icon={ICONS.REMOVE} />}
    </li>;
}