...
import { selectClaimForUri, selectThumbnailForUri, selectPurchaseTagForUri, selectPurchaseMadeForClaimId, selectRentalTagForUri, selectProtectedContentTagForUri, makeSelectTagInClaimOrChannelForUri } from "redux/selectors/claims";
...
import { doChangeVolume, doChangeMute, doAnalyticsBuffer, doAnalyticsViewForUri, doSetVideoSourceLoaded } from "redux/actions/app";
...
const select = (state, props) => {
  ...
  const position = startTime || (urlParams.get('t') !== null ? urlParams.get('t') : selectContentPositionForUri(state, uri));
