...
function FileViewerEmbeddedEnded(props: Props) {
  ...
  const prompts = isAuthenticated ? { ...DEFAULT_PROMPTS,
  ...
  return <div className="file-viewer__overlay">
      ...
      <div className="file-viewer__overlay-actions">
        <>
          {doReplay && <Button title={__('Replay')} button="link" label={preferEmbed ? __('Replay') : undefined} iconRight={ICONS.REPLAY} onClick={doReplay} />}
          {!preferEmbed && <>
              <a target="_blank" rel="noopener noreferrer" href={odyseeLink}>
                <Button label={__('Discuss')} iconRight={ICONS.EXTERNAL} button="primary" />
              </a>
              {!isAuthenticated && <a target="_blank" rel="noopener noreferrer" href={`${APP_URL}/$/signup?src=embed_signup`}>
                  <Button label={__('Join %SITE_NAME%', {
              SITE_NAME
            })} button="secondary" />
                </a>}
            </>}
