...
function SettingsMenuContent({
  ...
  const [localPipEnabled, setLocalPipEnabled] = useState(pipEnabled);
  React.useEffect(() => setLocalPipEnabled(pipEnabled), [pipEnabled]);
  const handleTogglePip = useCallback(() => {
    const next = !localPipEnabled;
    setLocalPipEnabled(next);
    settingsDispatch(doSetClientSetting(SETTINGS.PIP_ENABLED, next));
  }, [localPipEnabled, settingsDispatch]);
  ...
  return (
    <div key="main" className="media-settings-menu">
      ...
      {window.cordova && !isExternalEmbedPlayback && (
        <button type="button" className="media-settings-menu__item" onClick={handleTogglePip}>
          <svg
            className="media-settings-menu__icon"
            width={16}
            height={16}
          ...
          >
            ...
            <rect x="12" y="11" width="8" height="8" rx="1" fill="currentColor" stroke="none" />
          </svg>
          <span className="media-settings-menu__label">{__('Picture-in-Picture')}</span>
          <span className={`media-settings-toggle ${localPipEnabled ? 'media-settings-toggle--on' : ''}`}>
...
export default function OdyseeSkin(props) {
  ...
  React.useEffect(() => {
    const syncFsIcons = () => {
      ...
      const cordovaFs = Boolean(window.cordova && window.odysee?.fullscreen);
      const isFs = cordovaFs || (!!fsTarget && fsEl === fsTarget) || (!!shortsContainer && fsEl === shortsContainer);
