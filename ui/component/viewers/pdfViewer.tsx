...
class PdfViewer extends React.PureComponent<Props, State> {
  ...
  loadPdf = async () => {
    ...
    try {
      ...
      this.setState({ pdfDoc: pdf, numPages: pdf.numPages, loading: false }, () => this.renderPage(1));
    } catch (error) {
  ...
  nextPage = () => {
    ...
    if (pageNum < numPages) this.renderPage(pageNum + 1);
  };

  prevPage = () => {
    const { pageNum } = this.state;
    if (pageNum > 1) this.renderPage(pageNum - 1);
  };

  zoomIn = () => this.renderPage(this.state.pageNum, this.state.scale + 0.25);
  zoomOut = () => this.renderPage(this.state.pageNum, Math.max(0.25, this.state.scale - 0.25));

  togglePageSelector = () => this.setState({ showPageSelector: !this.state.showPageSelector });
