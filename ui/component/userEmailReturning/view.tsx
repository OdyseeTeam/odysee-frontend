...
function UserEmailReturning(props: Props) {
  ...
  return <div className={classnames('main__sign-in', {
    'main__sign-up--graphic': SIMPLE_SITE && !showEmailVerification
  })}>
      {showEmailVerification ? <UserEmailVerify /> : <Card title={__('Log in to %SITE_NAME%', {
      SITE_NAME
    })} actions={<div>
              <Form onSubmit={handleSubmit} className="section">
                <FormField autoFocus={!emailExistsFromUrl} placeholder={__('yourstruly@example.com')} type="email" id="username" autoComplete="on" name="sign_in_email" label={__('Email')} value={email} onChange={e => setEmail(e.target.value)} />
          ...
          /* @if TARGET='app' */
                ...
                <FormField type="checkbox" name="sync_checkbox" label={<React.Fragment>
                    ...
                    </React.Fragment>} checked={syncEnabled} onChange={() => setSyncEnabled(!syncEnabled)} />
                {
          /* @endif */
        }

                <div className="section__actions">
                  <Button autoFocus={emailExistsFromUrl} button="primary" type="submit" label={__('Log In')} disabled={!email || !valid || isPending} />
                  <Button button="link" onClick={handleChangeToSignIn} label={__('Sign Up')} />
                </div>
              </Form>
            </div>} nag={<>
              {!emailDoesNotExist && emailExistsFromUrl && <Nag type="helpful" relative message={__('That email is already in use. Did you mean to log in?')} />}
              {emailDoesNotExist && <Nag type="helpful" relative message={__("We can't find that email. Did you mean to sign up?")} actionText={__('Sign Up')} />}
              {!emailExistsFromUrl && !emailDoesNotExist && errorMessage && <Nag type="error" relative message={errorMessage} />}
