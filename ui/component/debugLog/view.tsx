...
function DebugLog(props: Props) {
  ...
  function getStackTraceElem(info: string | Error) {
    if (info instanceof Error && info.stack) {
      ...
      const lines: Array<string> = allLines.filter(x => {
        return !x.startsWith('Error: ') && !x.includes('ui/asserts.js') && !x.includes('at doAssert') && !x.includes('at assert') && !x.includes('at eval') && !x.includes('/node_modules/') && !x.includes('bindActionCreators.js');
      });
  ...
  return <div className={classnames('debug-log', {
    'debug-log--open': show
  })}>
      {debugLog.map((x, index) => <div className="debug-log__entry" key={index}>
          {getMessageElem(x)}
          {getDataElem(x)}
          {getStackTraceElem(x)}
        </div>)}
