...
export default function FileDescription(props: Props) {
  ...
  return <>
      ...
      <div className="card__bottom-actions">
        {!expandOverride && <>
            {!isLivestreamClaim && !isShort && <Button button="link" label={expanded ? __('Less') : __('More')} onClick={() => setExpanded(!expanded)} />}
            {isLivestreamClaim && <Button />}
          </>}

        <div className="section__actions--no-margin">
          {claimIsMine && hasSupport && <Button button="link" className="expandable__button" icon={ICONS.UNLOCK} aria-label={__('Unlock tips')} onClick={() => doOpenModal(MODALS.LIQUIDATE_SUPPORTS, {
          uri
        })} />}
