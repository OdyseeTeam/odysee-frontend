...
const LastReleaseChanges = (props: Props) => {
  ...
  const releaseVersionTitle = <p>
      {!hideReleaseVersion && __('A new version %release_tag% of LBRY is ready for you.', {
      release_tag: releaseTag
    })}
    </p>;
  const seeReleaseNotes = <p className="help">
      <I18nMessage tokens={{
      release_notes: <Button button="link" label={__('release notes')} href="https://github.com/lbryio/lbry-desktop/releases" />
    }}>
  ...
  useEffect(() => {
    ...
    fetch(lastReleaseUrl, options).then(response => response.json()).then(response => {
  ...
  return <div className="release__notes">
      {releaseVersionTitle}
      <p>
