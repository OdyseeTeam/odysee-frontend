...
function Paginate(props: Props) {
  ...
  return (// Hide the paginate controls if we are loading or there is only one page
    ...
    <Form style={totalPages <= 1 ? {
      display: 'none'
    } : null} onSubmit={handlePaginateKeyUp}>
      <fieldset-group class="fieldset-group--smushed fieldgroup--paginate">
        <fieldset-section>
          <ReactPaginate pageCount={totalPages} pageRangeDisplayed={2} previousLabel="‹" nextLabel="›" activeClassName="pagination__item--selected" pageClassName="pagination__item" previousClassName="pagination__item pagination__item--previous" nextClassName="pagination__item pagination__item--next" breakClassName="pagination__item pagination__item--break" marginPagesDisplayed={2} onPageChange={e => handleChangePage(e.selected + 1)} forcePage={currentPage - 1} initialPage={currentPage - 1} containerClassName="pagination" />
        </fieldset-section>
        {!isMobile && <FormField value={textValue} onChange={e => setTextValue(e.target.value)} className="paginate-channel" label={__('Go to page:')} type="text" name="paginate-file" />}
      </fieldset-group>
    </Form>
  );
}
