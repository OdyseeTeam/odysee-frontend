...
const WalletBalance = (props: Props) => {
  ...
  const handleSignIn = () => {
    ...
    if (showModal) doOpenModal(MODALS.CRYPTO_DISCLAIMERS);else window.wanderInstance.open();
  };

  return <div className={'columns'}>
      <div className="column">
        <Card title={<Symbol token="lbc" amount={Number(totalBalance).toFixed(8)} precision={6} isTitle counter />} subtitle={totalLocked > 0 ? <I18nMessage tokens={{
        lbc: <LbcSymbol />
      }}>
                Your total %lbc% balance. All of this is yours, but some %lbc% is in use on channels and content right
                now.
              </I18nMessage> : <span>{__('Your total balance.')}</span>} background actions={<>
            ...
            lbc_amount: <CreditAmount amount={totalLocked} precision={4} />
                ...
                <Button button="link" label={detailsExpanded ? __('View less') : __('View more')} iconRight={detailsExpanded ? ICONS.UP : ICONS.DOWN} onClick={() => setDetailsExpanded(!detailsExpanded)} />
              </h2>
              {detailsExpanded && <div className="section__subtitle">
                  <dl>
                    <dt>
                    ...
                    <dd>
                      <span className="dd__text">
                        {Boolean(tipsBalance) && <Button button="link" className="dd__button" disabled={operationPending} icon={ICONS.UNLOCK} onClick={() => doOpenModal(MODALS.MASS_TIP_UNLOCK)} />}
          ...
          /* @if TARGET='app' */
              ...
              {hasSynced ? <p className="section help">
                  {__('A backup of your wallet is synced with lbry.tv.')}
                  <HelpLink href="https://lbry.com/faq/account-sync" />
                </p> : <p className="help--warning">
          ...
          /* @endif */
              ...
              {(otherCount > WALLET_CONSOLIDATE_UTXOS || consolidateIsPending || consolidatingUtxos) && <p className="help">
                  <I18nMessage tokens={{
            now: <Button button="link" onClick={() => doUtxoConsolidate()} disabled={operationPending} label={consolidateIsPending || consolidatingUtxos ? __('Consolidating...') : __('Consolidate Now')} />,
      ...
      <div className="column">
        <Card title={!hasArConnection ? <Symbol token="wallet" amount="0" precision={2} isTitle counter /> : <>
                ...
                <div className="wallet-check-row">
                  ...
                  <div>
                    {!hasConnection && !isSigningIn ? <img src="https://thumbs.odycdn.com/bd2adbec2979b00b1fcb6794e118d5db.webp" alt="Failed" /> : isSigningIn ? <img src="https://thumbs.odycdn.com/fcf0fa003f3537b8e5d6acd1d5a96055.webp" alt="Loading..." /> : <img src="https://thumbs.odycdn.com/8ee966185b537b147fb7be4412b6bc68.webp" alt="Success" />}
                  </div>
                </div>
                ...
                <div className="wallet-check-row">
                  ...
                  <div>
                    {hasArConnection ? <img src="https://thumbs.odycdn.com/8ee966185b537b147fb7be4412b6bc68.webp" /> : <img src="https://thumbs.odycdn.com/bd2adbec2979b00b1fcb6794e118d5db.webp" />}
                  </div>
                </div>
              </> : <I18nMessage tokens={{
        ...
        price: <img style={{
          height: '16px'
              ...
              </I18nMessage>} background actions={<>
              {!hasConnection && !isSigningIn ? <div>
                  <I18nMessage tokens={{
            textD: <p>
                          To use AR on Odysee, you need to create and/or sign into Wander – a cryptocurrency wallet
                          compatible with AR.{' '}
                          <a href="https://help.odysee.tv/category-monetization/setup" target="_blank" rel="noopener noreferrer" className="link">
                            {__('Learn more')}
                          </a>
                        </p>,
            textM: <p>
            ...
            login: <a className="link" onClick={handleSignIn}>
                          Sign in
                        </a>,
            extension: <a className="link" rel="noreferrer" href="https://www.wander.app/download?tab=download-browser" target="_blank">
                          install browser extension
                        </a>,
            app: <a className="link" rel="noreferrer" href="https://www.wander.app/download?tab=download-mobile" target="_blank">
                          Wander Wallet app
                        </a>
          }}>
            ...
            status: <a className="link" onClick={() => {
              window.wanderInstance.open();
                ...
                </div> : !hasArConnection && addressInUse ? <div>
                  <p>
                    {__('The currently active Wander wallet is already connected to another Odysee account. Please switch to a different wallet.')}
                  </p>
            ...
            window.wanderInstance.open();
                ...
                </div> : !hasArConnection ? <div>
                  <I18nMessage tokens={{
            text: <p>To use AR on Odysee, the Wander wallet must be connected.</p>,
            link: <a className="link" onClick={() => doArConnect()}>
                          Connect now
                        </a>,
            login: <a className="link" onClick={() => window.wanderInstance.open()}>
                          change login
                        </a>
          }}>
              ...
              <div className="section__actions">
                <Button button="secondary" label={__(`Deposit${!isMobile ? ' Funds' : ''}`)} icon={ICONS.BUY} navigate={`/$/${PAGES.ARACCOUNT}?tab=buy`} disabled={!hasArSignin || !hasArConnection} />
                <Button button="secondary" label={__(`${!isMobile ? 'Send Payment' : 'Send'}`)} icon={ICONS.USD} navigate={`/$/${PAGES.ARACCOUNT}`} disabled={!hasArSignin || !hasArConnection} />
                <Button button="secondary" label={__(`${!isMobile ? 'Arweave ' : ''}Account`)} icon={ICONS.SETTINGS} navigate={`/$/${PAGES.ARACCOUNT}`} disabled={!hasArSignin || !hasArConnection} />
              </div>
            </>} />
      </div>
