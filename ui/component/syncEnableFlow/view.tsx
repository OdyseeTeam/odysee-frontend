...
function SyncEnableFlow(props: Props) {
  ...
  const handleSyncToggle = async () => {
    ...
    if (finalPrefs) {
      await Lbry.preference_set({
        key: mode === ENABLE_MODE ? SHARED_KEY : LOCAL_KEY,
        value: finalPrefs
      });
    }
  ...
  return <Card title={mode === ENABLE_MODE ? 'Enable Sync' : 'Disable Sync'} subtitle={<div>
          {(error || getSyncError) && <I18nMessage tokens={{
      click_here: <Button button="link" href="https://lbry.com/faq/accounts-and-sync#limitations" label={__('Click here')} />
    }}>
          ...
          {step === CONFIRM && <div className={'card__actions'}>
              <Button button="primary" name={'syncbutton'} label={mode === ENABLE_MODE ? __('Enable Sync') : __('Disable Sync')} onClick={() => handleSyncToggle()} />
              <Button button="link" name={'cancel'} label={__('Cancel')} onClick={() => closeModal()} />
            </div>}
          {(step === FETCH_FOR_ENABLE || step === FETCH_FOR_DISABLE) && <div className={'card__actions'}>
              <Button button="primary" name={'syncbutton'} label={mode === ENABLE_MODE ? __('Enable Sync') : __('Disable Sync')} onClick={() => handleSyncToggle()} disabled />
              <Button button="link" name={'cancel'} label={__('Cancel')} onClick={() => closeModal()} />
            </div>}
          {(error || getSyncError) && <div className={'card__actions'}>
