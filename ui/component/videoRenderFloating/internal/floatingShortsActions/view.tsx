  ...
  return <>
      <div className="content__shorts-floating-nav">
        <div className="shorts-floating-action">
          <Button onClick={onPrevious} icon={ICONS.UP} iconSize={16} title={__('Previous Short')} disabled={!onPrevious} />
        </div>
      ...
      <div className="content__shorts-floating-actions">
        <div className="shorts-floating-action" style={disableReactions ? {
        visibility: 'hidden',
        pointerEvents: 'none'
      } : undefined}>
          <Button onClick={() => {
          ...
          {Number.isInteger(likeCount) && <span className="shorts-floating-action__count">{formatNumberWithCommas(likeCount, 0)}</span>}
        </div>

        <div className="shorts-floating-action" style={disableReactions || disableSlimes ? {
        visibility: 'hidden',
        pointerEvents: 'none'
      } : undefined}>
          <Button onClick={() => {
          ...
          {Number.isInteger(dislikeCount) && <span className="shorts-floating-action__count">{formatNumberWithCommas(dislikeCount, 0)}</span>}
        </div>

        {channelUrl && <div className="shorts-floating-action shorts-floating-action--avatar" onMouseEnter={() => setAvatarHover(true)} onMouseLeave={() => setAvatarHover(false)} onClick={() => {
        const sub = {
        ...
        })} onClick={doToggleShortsAutoplay} icon={ICONS.AUTOPLAY_NEXT} iconSize={16} />
        </div>
      </div>
    </>;
};
