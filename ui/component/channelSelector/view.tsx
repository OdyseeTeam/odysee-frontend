...
function ChannelSelector(props: Props) {
  ...
  return <>
      <div className={classnames('channel-selector', {
      ...
      disabled: disabled
        ...
        <Menu>
          {isHeaderMenu && channelIds && channelIds.length > 1 ? <>
              ...
              {showAllOption && allOptionProps?.isSelected ? <AllSelector isSelected /> : pendingChannelFetch ? <LoadingSelector isSelected /> : incognito && !hideAnon || !activeChannelUrl ? <IncognitoSelector isSelected /> : <ChannelListItem channelId={activeChannelId} isSelected />}
            </MenuButton>}

          <MenuList className="menu__list channel-selector">
            {showAllOption && <MenuItem onSelect={allOptionProps?.onSelectAll}>
                <AllSelector />
              </MenuItem>}

            {channelIds && channelIds.map(channelId => <MenuItem key={channelId} onSelect={() => handleSelectOption(channelId)}>
                  <ChannelListItem channelId={channelId} />
                </MenuItem>)}

            {!hideAnon && <MenuItem onSelect={() => handleSelectOption(null)}>
                <IncognitoSelector />
              </MenuItem>}
            {!hideCreateNew && <MenuItem onSelect={() => push(`/$/${PAGES.CHANNEL_NEW}?redirect=${pathname}`)}>
      ...
      {isHeaderMenu && activeChannelUrl && <NavLink to={formatLbryUrlForWeb(activeChannelUrl)}>
