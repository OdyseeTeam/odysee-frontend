...
export default function HeaderMenuButtons(props: HeaderMenuButtonProps) {
  ...
  return authenticated ? <div className="header__buttons">
      <Tooltip title={__('Upload')}>
        <Button className="header__navigationItem--icon" onClick={() => doBeginPublish(PUBLISH_TYPES.FILE)}>
          <Icon size={18} icon={ICONS.PUBLISH} aria-hidden />
        </Button>
      </Tooltip>
      {livestreamEnabled && <Tooltip title={__('Go live')}>
          <Button className="header__navigationItem--icon" {...uploadProps} onClick={() => doBeginPublish(PUBLISH_TYPES.LIVESTREAM)}>
