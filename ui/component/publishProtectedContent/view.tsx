...
function PublishProtectedContent(props: Props) {
  ...
  if (!myMembershipTiers || myMembershipTiers && myMembershipTiers.length === 0) {
    return <>
        <Card background isBodyList title={__('Restrict Content')} body={<div className="settings-row publish-row--locked">
              <I18nMessage tokens={{
          activate_your_memberships: <Button navigate={`/$/${PAGES.CREATOR_MEMBERSHIPS}`} label={__('activate your memberships')} button="link" />
        }}>
  ...
  if (validTierIds && validTierIds.length === 0) {
    return <Card background isBodyList title={__('Restrict Content')} body={<div className="publish-row publish-row-tiers">
  ...
  if (validTierIds && validTierIds.length > 0) {
    ...
    return <>
        <Card background isBodyList title={__('Restrict Content')} body={<div className="publish-row publish-row-tiers">
              <FormField type="checkbox" disabled={paywall !== PAYWALL.FREE} checked={memberRestrictionOn} label={__('Restrict content to only allow subscribers to certain memberships to view it')} name={'toggleRestrictedContent'} className="restrict-content__checkbox" onChange={toggleMemberRestrictionOn} />

              {memberRestrictionOn && <div className="tier-list">
                  {myMembershipTiers.map((tier: CreatorMembership) => {
            const show = validTierIds && validTierIds.includes(tier.membership_id);
            return show ? <FormField disabled={paywall !== PAYWALL.FREE} key={tier.membership_id} type="checkbox" checked={memberRestrictionTierIds.includes(tier.membership_id)} label={tier.name} name={tier.membership_id} onChange={() => toggleMemberRestrictionTierId(tier.membership_id)} /> : <div key={tier.membership_id} className="dummy-tier" />;
          })}
                </div>}

              {paywall !== PAYWALL.FREE && <div className="error__text">
