...
export default function FileActions(props: Props) {
  ...
  const webShareable = costInfo && costInfo.cost === 0 && RENDER_MODES.WEB_SHAREABLE_MODES.includes(renderMode) || RENDER_MODES.TEXT_MODES.includes(renderMode) || isCollectionClaim || isChannel;
  const urlParams = new URLSearchParams(search);
  const collectionId = urlParams.get(COLLECTIONS_CONSTS.COLLECTION_ID);
  const showDownload = !isLivestreamClaim && !(disableDownloadButton && !claimIsMine) && !isMature && (!isFiatRequired || isFiatPaidAsPurchase) && (!isProtectedContent || isTierUnlocked) && (scheduledState === 'non-scheduled' || scheduledState === 'started');
  ...
  return <div className="media__actions">
      ...
      {(isMobile && (showRepost || claimIsMine) || showDownload || !claimIsMine) && <Menu>
          <MenuButton className="button--file-action--menu" onClick={e => {
        e.stopPropagation();
        e.preventDefault();
      }}>
          ...
          <MenuList className="menu__list">
            {isMobile && <>
                {showRepost && <MenuItem className="comment__menu-option" onSelect={handleRepostClick}>
                    <div className="menu__link">
                      <Icon aria-hidden icon={ICONS.REPOST} />
                      {claimMeta?.reposted > 1 ? __(`%repost_total% Reposts`, {
                ...
                {claimIsMine && <>
                    <MenuItem className="comment__menu-option" onSelect={() => {
              doPrepareEdit(claim, editUri);
                    ...
                    <MenuItem className="comment__menu-option" onSelect={() => doOpenModal(MODALS.CONFIRM_FILE_REMOVE, {
              uri
            })}>
                      <div className="menu__link">
            ...
            {showDownload && <MenuItem className="comment__menu-option" onSelect={handleWebDownload}>
                <div className="menu__link">
            ...
            {!claimIsMine && <MenuItem className="comment__menu-option" onSelect={() => push(`/$/${PAGES.REPORT_CONTENT}?claimId=${claimId}`)}>
                <div className="menu__link">
