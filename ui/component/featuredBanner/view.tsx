...
export default function FeaturedBanner(props: Props) {
  ...
  const [localBannerHidden, setLocalBannerHidden] = React.useState(() => sessionStorage.getItem('bannerHidden') === 'true');
  ...
  return <div className="featured-banner-wrapper" ref={wrapper} onMouseEnter={() => setPause(true)} onMouseLeave={() => setPause(false)}>
      <div className="featured-banner-rotator" style={{
      marginLeft: marginLeft
    }}>
        {featured && featured.items.map((item, i) => {
        return <NavLink className="featured-banner-image" onClick={e => handleAnchor(e, item.url)} to={getUriTo(item.url)} target={!item.url.includes('odysee.com') ? '_blank' : undefined} title={item.label} key={i} style={{
          minWidth: width
        }}>
                <img src={'https://thumbnails.odycdn.com/optimize/s:' + imageWidth + ':0/quality:95/plain/' + item.image} style={{
            width: width
          }} />
              </NavLink>;
      })}
      ...
      <div className="banner-controls">
        ...
        <div className="banner-active-indicator">
          {featured && featured.items.map((item, i) => {
          return <div key={i} className={i + 1 === index ? 'banner-active-indicator-active' : ''} onClick={() => setIndex(i + 1)} />;
        })}
        </div>
        {authenticated && <div className="banner-context-menu" ref={menuRef}>
