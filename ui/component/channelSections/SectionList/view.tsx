...
export default function SectionList(props: Props) {
  ...
  return <div className={classnames('channel_sections', {
    'channel_sections--disabled': fetchingCreatorSettings
      ...
      {editMode && <div className="channel_sections__actions">
          ...
          {sectionCount > 1 && <Button title={__('Sort')} button="secondary" icon={ICONS.ARRANGE} onClick={handleSort} />}
          <Button label={__('Add featured channels')} button="primary" icon={ICONS.ADD} onClick={handleAddFeaturedChannels} />
        </div>}
      <div className="channel_sections__list">
        {sectionCount === 0 && <div className="empty main--empty">
            {fetchingCreatorSettings && <Spinner />}
            {!fetchingCreatorSettings && __('No featured channels.')}
          </div>}
        {!fetchingCreatorSettings && featuredChannels && featuredChannels.map(fc => <Section key={fc.id} id={fc.id} title={fc.value.title} uris={fc.value.uris} channelId={claimId} />)}
      </div>
    </div>;
}