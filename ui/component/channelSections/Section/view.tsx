...
export default function Section(props: Props) {
  ...
  const ContextMenu = (props: {}) => <Menu>
      ...
      <MenuList className="menu__list">
        {isChannelMine && <>
            {!location.search.includes('sectionId') && <ContextMenuItem label={'View'} icon={ICONS.EYE} onSelect={() => push(`/$/${PAGES.FEATURED_CHANNELS}?${CSQ.CLAIM_ID}=${channelId}&${CSQ.SECTION_ID}=${id}`)} />}
  ...
  function handleSectionDelete() {
    doOpenModal(MODALS.CONFIRM, {
      title: title ? __('Delete "%list_name%"?', {
  ...
  // **************************************************************************
  // **************************************************************************
  return <div className="channel-section-card">
      ...
      <div className="channel-section-card__content">
        <div className="channel-section-card__item-row">
          <div className={classnames('channel-section-card__item-list')}>
            {uris.map(uri => <div key={uri} className="channel-section-card__item" onClick={() => push(formatLbryUrlForWeb(uri) + '?view=home')}>
