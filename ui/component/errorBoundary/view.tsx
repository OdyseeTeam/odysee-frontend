...
class ErrorBoundary extends React.Component<Props, State> {
  ...
  render() {
    ...
    const {
      sentryEventId,
      desktopErrorReported
    } = this.state;
    const errorWasReported = IS_WEB ? sentryEventId !== null : desktopErrorReported;

    if (hasError) {
      return <div className="main main--full-width main--empty">
          <Yrbl type="sad" title={__('Aw shucks!')} subtitle={<I18nMessage tokens={{
          refreshing_the_app_link: <Button button="link" className="load-screen__button" label={__('refreshing the app')} onClick={this.refresh} />
        }}>
                There was an error. Try %refreshing_the_app_link% to fix it. If that doesn't work, try pressing
                Ctrl+R/Cmd+R.
              </I18nMessage>} />
          {!errorWasReported && <div className="error__wrapper">
          ...
          {errorWasReported && <div className="error__wrapper">
            ...
            /* @if TARGET='web' */
              ...
              <span className="error__text">{__('Error ID: %sentryEventId%', {
              sentryEventId
            })}</span>
              {
            /* @endif */
