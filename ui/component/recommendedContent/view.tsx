...
export default React.memo<Props>(function RecommendedContent(props: Props) {
  ...
  React.useEffect(() => {
    ...
    if (claimId && recommendedContentUris && recommendedContentUris.length && nextRecommendedUri && viewMode === VIEW_ALL_RELATED) {
  ...
  return <Card isBodyList smallTitle={!isMobile && !isSmall} className="file-page__recommended" title={__('Related')} titleActions={signingChannel && <div className="recommended-content__bubble">
            {searchInLanguage && <LangFilterIndicator />}

            <Button className={classnames('button-bubble', {
      'button-bubble--active': viewMode === VIEW_ALL_RELATED
    })} label={__('Related')} onClick={() => setViewMode(VIEW_ALL_RELATED)} />
      ...
      claim_name: channelName
          ...
          {isSearching && <>
              {new Array(20).fill(1).map((x, i) => <ClaimPreview key={i} placeholder="loading" type="small" />)}
            </>}
          {viewMode === VIEW_ALL_RELATED && <ClaimList type="small" loading={isSearching} uris={recommendedContentUris} empty={__('No related content found')} onClick={handleRecommendationClicked} />}
          {viewMode === VIEW_MORE_FROM && signingChannel && <ClaimListDiscover hideAdvancedFilter tileLayout={false} showHeader={false} type="small" claimType={['stream']} orderBy="new" pageSize={20} infiniteScroll={false} hideFilters hasSource channelIds={[signingChannel.claim_id]} loading={isSearching} empty={__('No related content found')} />}
        </div>} />;
}, areEqual);
...
function areEqual(prevProps: Props, nextProps: Props) {
  ...
  if (a.uri !== b.uri || a.nextRecommendedUri !== b.nextRecommendedUri || a.isSearching !== b.isSearching || a.recommendedContentUris && !b.recommendedContentUris || !a.recommendedContentUris && b.recommendedContentUris || a.claim && !b.claim || !a.claim && b.claim) {
    return false;
  }
