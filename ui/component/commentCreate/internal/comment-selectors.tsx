...
export default function CommentSelectors(props: Props) {
  ...
  return <Tabs index={openTab} className={isOpen ? 'tabs tabs--comment tabs--open' : 'tabs'} onChange={() => {}}>
      ...
      <TabPanels>
        ...
        <TabPanel>
          {mount && <StickersPanel handleSelect={sticker => handleSelectSticker(sticker)} claimIsMine={claimIsMine} {...tabProps} />}
        </TabPanel>
      </TabPanels>
    </Tabs>;
}
...
function scrollToCategory(category, reference, isMobile) {
  ...
  reference && categoryAnchor && // $FlowIgnore
...
function handleHover(name) {
  ...
  if (preview) {
    ...
    if (name) preview.style.display = 'inline';else preview.style.display = 'none';
  }
}
...
const EmojisPanel = (emojisProps: EmojisProps) => {
  ...
  return <div className="selector-menu" ref={emojiSelectorRef}>
      ...
      <div className="emoji-categories">
        ...
        {CATEGORY_INFOS.map(x => <img key={x.key} onClick={() => scrollToCategory(x.key, emojiSelectorRef, isMobile)} onMouseEnter={() => handleHover(x.name)} onMouseLeave={() => handleHover('')} loading="lazy" src={`https://static.odycdn.com/emoticons/${x.mainImg}`} />)}
      </div>

      {
      /* <EmoteCategory title={__('Recently used')} {...defaultRowProps} /> */
    }
      {CATEGORY_INFOS.map(x => <EmoteCategory key={x.key} title={x.name} images={x.images} {...defaultRowProps} handleHover={handleHover} />)}
    </div>;
};
...
const StickersPanel = (stickersProps: StickersProps) => {
  ...
  const isMobile = useIsMobile();
  return <div className="selector-menu" ref={stickerSelectorRef}>
      <Button button="close" icon={ICONS.REMOVE} onClick={closeSelector} />
      <div id="emoji-code-preview" />
      <div className="emoji-categories">
        <img onClick={() => scrollToCategory('free', stickerSelectorRef, isMobile)} onMouseEnter={() => handleHover(__('Free'))} onMouseLeave={() => handleHover('')} loading="lazy" src="https://static.odycdn.com/stickers/HYPE/PNG/hype_with_border.png" />
        {!claimIsMine && <img onClick={() => scrollToCategory('tips', stickerSelectorRef, isMobile)} onMouseEnter={() => handleHover(__('Tips'))} onMouseLeave={() => handleHover('')} loading="lazy" src="https://static.odycdn.com/stickers/TIPS/png/with%20borderlarge$tip.png" />}
      </div>
      <StickerCategory title={__('Free')} images={FREE_GLOBAL_STICKERS} {...defaultRowProps} handleHover={handleHover} />
      {!claimIsMine && <StickerCategory title={__('Tips')} images={PAID_GLOBAL_STICKERS} {...defaultRowProps} handleHover={handleHover} />}
    </div>;
};
...
const EmoteCategory = (rowProps: RowProps) => {
  ...
  return <>
      ...
      <div className="emote-selector__items">
        {images && images.map(emote => {
        const {
          name,
          url
        } = emote;
        return <Button key={name} title={name} button="alt" className="button--file-action" onClick={() => handleSelect(name)} onMouseEnter={() => handleHover(name)} onMouseLeave={() => handleHover('')}>
...
const StickerCategory = (rowProps: RowProps) => {
  ...
  return <>
      ...
      <div className="sticker-selector__items">
        {images && images.map(sticker => {
        const {
          price,
          url,
          name
        } = sticker;
        return <Button key={name} title={name} button="alt" className="button--file-action" onClick={() => handleSelect(sticker)} onMouseEnter={() => handleHover(sticker)} onMouseLeave={() => handleHover('')}>
