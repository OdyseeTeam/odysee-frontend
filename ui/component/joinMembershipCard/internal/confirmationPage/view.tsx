...
const ConfirmationPage = (props: Props) => {
  ...
  return <div className="confirm__wrapper">
      <h1>{__('Almost done')}</h1>
      <ConfirmationSection label={isRenewal ? __(`Renew %channelName%'s Membership As`, {
      channelName
    }) : __(`Join %channelName%'s Membership As`, {
      channelName
      ...
      <ConfirmationSection className={'membership-tier__description'} label={__('Description')} value={selectedCreatorMembership.description} />
      ...
      {selectedCreatorMembership.perks && selectedCreatorMembership.perks.length > 0 && <ConfirmationSection label={__('Features and Perks')} value={<ul className="ul--no-style membership-tier__perks">
              {
        /* $FlowFixMe -- already handled above */
      }
              {selectedCreatorMembership.perks.map((tierPerk, i) => <li key={i}>{__(tierPerk.name)}</li>)}
            </ul>} />}
      ...
      {preferredCurrency && preferredCurrency === STRIPE.CURRENCIES.EUR ? <>
        ...
        </> : purchasePending ? <BusyIndicator message={__('Processing payment...')} /> : <>
          <div className="section__actions">
            {incognito && <p className="help">
                <div className="error__text">
                  {__("You are about to join as Anonymous, you won't be able to view or comment on chat at this time")}
                </div>
              </p>}
            {(!arBalance || dollarsPerAr && Number(dollarsPerAr) * arBalance < total) && <p className="help">
                <div className="error__text">{__('Insufficient Balance')}</div>
              </p>}

            <SubmitButton isRenewal={isRenewal} disabled={!arBalance || !!dollarsPerAr && Number(dollarsPerAr) * arBalance < total} modalState={{
          passedTierIndex: selectedMembershipIndex
        }} />
          ...
          <p className="help">
            <I18nMessage tokens={{
          membership_terms_and_conditions: <Button button="link" href="https://help.odysee.tv/category-memberships/" label={__('Membership Terms and Conditions')} />
        }}>
