...
const MembershipDetails = (props: Props) => {
  ...
  let accessText = __(membersOnly ? !isLivestream ? 'This membership does not give you access to the members-only comment section.' : 'This membership does not give you access to the members-only chat mode.' : 'This Tier does not grant you access to the currently selected content.');
  ...
  if (userHasACreatorMembership) {
    ...
    accessText = __(membersOnly ? !isLivestream ? 'This membership gives you access to the members-only comment section.' : 'This membership gives you access to the members-only chat mode.' : 'This membership gives you access to the current content.');
  }

  return <>
      {unlockableTierIds && <div className={'access-status' + ' ' + (membershipIsUnlockable ? 'green' : 'red')}>
          <p>{accessText}</p>
        </div>}

      {activeArStatus !== 'connected' && !isChannelTab ? <WalletStatus /> : <>
          ...
          <section className="membership-tier__infos">
            ...
            <span style={{
          display: 'flex'
        }}>${(Number(membership?.prices[0].amount) / 100).toFixed(2)}</span>
            ...
            <div className="membership-tier__perks">
              ...
              <div className="membership-tier__perks-content">
                {membership.perks && membership.perks.length > 0 ? <>
                    ...
                    <ul>
                      {membership.perks.map((tierPerk, i) => <li key={i}>{__(tierPerk.name)}</li>)}
                    </ul>
