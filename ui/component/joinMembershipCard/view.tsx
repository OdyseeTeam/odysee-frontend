...
const JoinMembershipCard = (props: Props) => {
  ...
  const cheapestPlanIndex = React.useMemo(() => {
    if (cheapestMembership) {
      return creatorMemberships && creatorMemberships.findIndex(membership => membership.membership_id === cheapestMembership.membership_id);
    }
  }, [cheapestMembership, creatorMemberships]);
  const [isOnConfirmationPage, setConfirmationPage] = React.useState(skipToConfirmation);
  const [selectedMembershipIndex, setMembershipIndex] = React.useState(passedTierIndex || cheapestPlanIndex || membershipIndex);
  const selectedCreatorMembership: CreatorMembership | null | undefined = creatorMemberships && creatorMemberships[selectedMembershipIndex];
  ...
  function handleJoinMembership() {
    ...
    doMembershipBuy(membershipBuyParams).then(() => {
      ...
      doToast({
        message: __("You are now a '%membership_tier_name%' member for %creator_channel_name%, enjoy the perks and special features!", {
  ...
  if (isUrlParamModal && (membershipMine === undefined || creatorMemberships === undefined || purchasedChannelMembership)) {
  ...
  return <Form onSubmit={handleJoinMembership}>
      <Card className={classnames('card--join-membership', {
      ...
      'membership-tier6': selectedMembershipIndex === 5
            ...
            {isOnConfirmationPage && creatorMemberships?.length ? <ConfirmationPage {...pageProps} onCancel={isChannelTab ? doHideModal : () => setConfirmationPage(false)} /> : <PreviewPage {...pageProps} handleSelect={() => setConfirmationPage(true)} setMembershipIndex={setMembershipIndex} unlockableTierIds={unlockableTierIds} membersOnly={membersOnly} isLivestream={isLivestream} />}
          </>} />
    </Form>;
};
