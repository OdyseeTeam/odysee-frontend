...
const RewardLink = (props: Props) => {
  ...
  return !reward ? null : <Button button={button ? 'primary' : 'link'} disabled={disabled || isPending} label={<LbcMessage>{displayLabel}</LbcMessage>} aria-label={displayLabel} onClick={() => {
    claimReward(reward);
  }} />;
};
