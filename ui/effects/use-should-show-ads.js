...
export default function useShouldShowAds(hasPremiumPlus: boolean, doSetAdBlockerFound: (boolean) => void, userCountry) {
    ...
    if (ad_blocker_detected === undefined) {
      ...
      fetch(GOOGLE_AD_URL)
        .then((response) => {
          ...
          doSetAdBlockerFound(detected); // Also stash in redux for re-use.
        })
        .catch(() => {
