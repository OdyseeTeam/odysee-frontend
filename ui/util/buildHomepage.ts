...
export const getHomepageRowForCat = (key: string, cat: HomepageCat) => {
  ...
  return {
    ...
    options: {
      ...
      releaseTime: `>${Math.floor(moment().subtract(cat.daysOfContent || 30, 'days').startOf('hour').unix())}`
    }
  };
};
export function GetLinksData(all: any, // HomepageData type?
isSmallScreen: boolean, isMediumScreen: boolean, isLargeScreen: boolean, isHomepage?: boolean, authenticated?: boolean, showPersonalizedChannels?: boolean, showPersonalizedTags?: boolean, subscribedChannelIds?: Array<ClaimId>, followedTags?: Array<Tag>, showIndividualTags?: boolean, showNsfw?: boolean) {
  ...
  if (isHomepage && showPersonalizedChannels && subscribedChannelIds) {
    const RECENT_FROM_FOLLOWING = {
      ...
      options: {
        ...
        releaseTime: subscribedChannelIds.length > 20 ? `>${Math.floor(moment().subtract(9, 'months').startOf('week').unix())}` : `>${Math.floor(moment().subtract(1, 'year').startOf('week').unix())}`,
...
export type HomepageTitles = "Recent From Following" | "Featured" | "Discover" | "Pop Culture" | "Artists" | "Education" | "Lifestyle" | "Gaming" | "Spooky" | "Tech" | "Comedy" | "Music" | "Sports" | "Finance 2.0";