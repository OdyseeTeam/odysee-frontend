...
export const transformNewLivestreamData = (data: LivestreamAllResponse): LivestreamInfoByCreatorIds => data.reduce((acc, curr) => {
  acc[curr.ChannelClaimID] = {
    type: 'application/x-mpegurl',
    isLive: curr.Live,
    viewCount: curr.ViewerCount,
    creatorId: curr.ChannelClaimID,
    activeClaim: { ...transformLivestreamClaimData(curr.ActiveClaim),
...
export const killStream = async (channelId: string, channelName: string) => {
  try {
    ...
    const apiData = await fetch(`${LIVESTREAM_KILL}channel_claim_id=${channelId}&channel_name=${encodedChannelName}&signature_ts=${streamData.t}&signature=${streamData.s}`);
...
export function filterActiveLivestreamUris(channelIds: Array<string> | null | undefined, excludedChannelIds: Array<string> | null | undefined, activeLivestreamByCreatorId: LivestreamByCreatorId, viewersById: LivestreamViewersById) {
