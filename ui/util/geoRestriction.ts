 ...
 */
export function getGeoRestrictionForClaim(claim: StreamClaim | null | undefined, locale: LocaleInfo, geoBlockLists: GBL | null | undefined) {
  if (locale && geoBlockLists && claim) {
    const claimId: string | null | undefined = claim.claim_id;
    const channelId: string | null | undefined = getChannelIdFromClaim(claim);
    let geoConfig: GeoConfig | null | undefined;
    ...
    if (geoConfig) {
      ...
      return specials.find((x: GeoRestriction) => x.id === 'EU-ONLY' && locale.is_eu_member) || countries.find((x: GeoRestriction) => x.id === locale.country) || continents.find((x: GeoRestriction) => x.id === locale.continent) || null;
    }
