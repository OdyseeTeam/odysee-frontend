 ...
 */
export function isChannelClaim(claim: Claim | null | undefined, uri?: string) {
...
export function getChannelIdFromClaim(claim: Claim | null | undefined) {
  if (claim) {
...
export const getNameFromClaim = (claim: Claim | null | undefined) => claim && claim.name;
export function getChannelNameFromClaim(claim: Claim | null | undefined) {
...
export function getChannelPermanentUrlFromClaim(claim: Claim | null | undefined) {
  const channelFromClaim = getChannelFromClaim(claim);
  return channelFromClaim && channelFromClaim.permanent_url;
}
export function getClaimMetadata(claim: Claim | null | undefined) {
  const metadata = claim && claim.value;
  return metadata || (claim === undefined ? undefined : null);
}
export function getClaimTags(claim: Claim | null | undefined) {
  const metadata = getClaimMetadata(claim);
  return metadata && metadata.tags;
}
export function claimContainsTag(claim: Claim | null | undefined, tag: string) {
  const metadata = getClaimMetadata(claim);
...
export function isClaimProtected(claim: Claim | null | undefined) {
  const tags = getClaimTags(claim);
  return tags && tags.includes(MEMBERS_ONLY_CONTENT_TAG);
}
export function isClaimUnlisted(claim: Claim | null | undefined) {
  const tags = getClaimTags(claim);
  return tags ? tags.includes(VISIBILITY_TAGS.UNLISTED) : false;
}
export function isClaimPrivate(claim: Claim | null | undefined) {
  const tags = getClaimTags(claim);
  return tags ? tags.includes(VISIBILITY_TAGS.PRIVATE) : false;
}
export function getClaimScheduledState(claim: Claim | null | undefined): ClaimScheduledState {
  const tags = getClaimTags(claim);
...
export function getClaimTitle(claim: Claim | null | undefined) {
  const metadata = getClaimMetadata(claim);
...
export const isStreamPlaceholderClaim = (claim: StreamClaim | null | undefined) => {
...
export const isClaimYouTubeMirror = (claim: Claim | null | undefined): boolean => {
  ...
  return /\.\.\.\s*\n\s*(?:https?:\/\/)?(?:www\.)?(?:youtube\.com\/(?:watch\?v=|embed\/|v\/|shorts\/)|youtu\.be\/)[\w-]+/i.test(description);
};
