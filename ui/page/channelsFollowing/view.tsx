...
function ChannelsFollowingPage(props: Props) {
  ...
  return !hasSubscribedChannels ? <ChannelsFollowingDiscoverPage /> : <Page noFooter fullWidthPage={tileLayout} className="main__channelsFollowing">
      {!fetchingActiveLivestreams && <>
          <UpcomingClaims name="channels_following" channelIds={channelIds} tileLayout={tileLayout} liveUris={activeLivestreamUris} />

          <ClaimListDiscover streamType={SIMPLE_SITE ? CS.CONTENT_ALL : undefined} tileLayout={tileLayout} headerLabel={<h1 className="page__title">
                <Icon icon={ICONS.SUBSCRIBE} />
                <label>{__('Following')}</label>
              </h1>} hideMembersOnly={hideMembersOnly} defaultOrderBy={CS.ORDER_BY_NEW} channelIds={channelIds} meta={<>
                <Button icon={ICONS.SEARCH} button="alt" label={__('Discover Channels')} navigate={`/$/${PAGES.CHANNELS_FOLLOWING_DISCOVER}`} />
                <Button icon={ICONS.SETTINGS} button="alt" label={__('Manage')} navigate={`/$/${PAGES.CHANNELS_FOLLOWING_MANAGE}`} />
              </>} subSection={<LivestreamSection tileLayout={tileLayout} channelIds={channelIds} />} hasSource csOptionsHook={tagSearchCsOptionsHook} />
        </>}
    </Page>;
}
