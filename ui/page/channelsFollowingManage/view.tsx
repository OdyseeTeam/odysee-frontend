...
export default function ChannelsFollowingManage(props: Props) {
  ...
  const {
    ...
    isLoadingPage,
    bumpPage
  } = useClaimListInfiniteScroll(subscribedChannelUris, doResolveUris, FOLLOW_PAGE_SIZE);
  ...
  return <Page noFooter>
      ...
      {page < 0 ? <div className="main--empty">
          <Spinner delayed />
        </div> : uris && uris.length === 0 ? <Empty padded text={__('No followed channels.')} /> : <>
          ...
          {!filteredUris && lastActiveSubs && lastActiveSubs.length === SIDEBAR_SUBS_DISPLAYED && <>
            ...
            return <div key={sub.uri} className="navigation-link__wrapper navigation__subscription">
                      <Button navigate={sub.uri} className="navigation-link navigation-link--with-thumbnail" activeClass="navigation-link--active">
          ...
          {!filteredUris && uris.length > 0 && <>
              ...
              <ClaimList uris={uris.slice(0, (page + 1) * FOLLOW_PAGE_SIZE)} onScrollBottom={bumpPage} page={page + 1} pageSize={FOLLOW_PAGE_SIZE} loading={isLoadingPage} useLoadingSpinner showHiddenByUser showUnresolvedClaims />
            </>}
        </>}
    </Page>;
}