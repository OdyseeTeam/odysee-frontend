...
function isTruthyQueryValue(value: string | null | undefined): boolean {
...
export default function YoutubeSync(props: Props) {
  ...
  const autoOpenSyncParam = hasAutoOpenSyncPrimaryParam ? urlParams.get(AUTO_OPEN_SYNC_PARAM) : urlParams.get(AUTO_OPEN_SYNC_PARAM_ALT);
  ...
  const youtubeAuthErrorMessage = hasYoutubeAuthError && !errorMessage ? __('There was a problem connecting this YouTube channel. Please try again.') : errorMessage;
  ...
  const Wrapper = (props: {
    ...
    return inSignUpFlow ? <>{props.children}</> : <Page noSideNavigation authPage>
        {props.children}
      </Page>;
  };
  ...
  return <Wrapper>
      <div className="main__channel-creation">
        {showYoutubeTransferStatus ? <React.Suspense fallback={null}>
            <YoutubeTransferStatus alwaysShow addNewChannel={handleNewChannel} autoOpenSync={shouldAutoOpenSync} />
          </React.Suspense> : <Card className="card--youtube-sync" title={__('Sync your YouTube channel to %site_name%', {
        site_name: IS_WEB ? SITE_NAME : 'Odysee'
      })} subtitle={__(`Don't want to manually upload? Get your YouTube videos in front of the %site_name% audience.`, {
        site_name: IS_WEB ? SITE_NAME : 'Odysee'
      })} actions={<Form onSubmit={handleCreateChannel}>
                <fieldset-group class="fieldset-group--smushed fieldset-group--disabled-prefix">
                  <fieldset-section>
                    <label htmlFor="auth_first_channel">
                      {nameError ? <span className="error__text">{nameError}</span> : __('Your desired %site_name% channel name', {
                site_name: IS_WEB ? SITE_NAME : 'Odysee'
              })}
                    </label>
                  ...
                  <FormField autoFocus placeholder={__('channel')} type="text" name="yt_sync_channel" className="form-field--short" value={channel} onChange={handleChannelChange} />
                </fieldset-group>
                <FormField name="language_select" type="select" label={__('Channel language')} onChange={event => setLanguage(event.target.value)} value={language}>
                  {sortLanguageMap(SUPPORTED_LANGUAGES).map(([langKey, langName]) => <option key={langKey} value={langKey}>
                      {langName}
                    </option>)}
                </FormField>
                <FormField type="checkbox" name="yt_sync_terms" checked={acknowledgedTerms} onChange={() => setAcknowledgedTerms(!acknowledgedTerms)} label={<I18nMessage tokens={{
          terms: <Button button="link" label={__('these terms')} href="https://odysee.com/$/youtubetos" />,
          faq: <Button button="link" label={__('how the program works')} href="https://help.odysee.tv/category-syncprogram/whatisyoutubesync/" />,
          site_name: SITE_NAME
        }}>
                ...
                <div className="section__actions">
                  <Button button="primary" type="submit" disabled={nameError || !channel || !acknowledgedTerms} label={__('Claim Now')} />

                  {inSignUpFlow && !hasYoutubeAuthError && <Button button="link" label={__('Skip')} onClick={() => doToggleInterestedInYoutubeSync()} />}

                  {hasYoutubeAuthError && <Button button="link" label={__('Skip')} navigate={`/$/${PAGES.YOUTUBE_SYNC}?reset_scroll=youtube`} />}
                </div>
                <div className="help--card-actions">
                  <I18nMessage tokens={{
            learn_more: <Button button="link" label={__('Learn more')} href="https://help.odysee.tv/category-syncprogram/" />,
            community_guidelines: <Button button="link" label={__('Community Guidelines')} href="https://help.odysee.tv/communityguidelines/" />
          }}>
              ...
              </Form>} nag={youtubeAuthErrorMessage && <Nag message={youtubeAuthErrorMessage} type="error" relative />} />}
      </div>
    </Wrapper>;
}