...
function Overview(props: Props) {
  ...
  function handleCheckForm() {
    ...
    if (rawAmount !== trimmedAmount && amountInput) {
      ...
      amountInput.setSelectionRange(Number(amountCursor) - (Number(rawAmount?.length) - Number(trimmedAmount?.length)), Number(amountCursor) - (Number(rawAmount?.length) - Number(trimmedAmount?.length)));
    }

    if (rawAddress !== trimmedAddress && addressInput) {
      addressInput.value = trimmedAddress;
      addressInput.setSelectionRange(Number(addressCursor) - (Number(rawAddress?.length) - Number(trimmedAddress?.length)), Number(addressCursor) - (Number(rawAddress?.length) - Number(trimmedAddress?.length)));
    }

    const isValidArweaveAddress = address => /^[A-Za-z0-9_-]{43}$/.test(String(address));

    const check = trimmedAmount && Number(trimmedAmount) <= Number(balance.ar) && isValidArweaveAddress(trimmedAddress) && trimmedAddress !== wallet.address;
    setCanSend(check);
  }
  ...
  return <Card className={activeArStatus !== 'connected' ? `card--overview card--disabled` : `card--overview`} title={cardHeader()} background actions={<>
          <div className="payment-options-wrapper">
            <div className="payment-options-card">
              ...
              <div className="payment-options">
                <h2 className="section__title--small">
                  <I18nMessage tokens={{
              learnMore: <div className="learn-more">
                          <Icon icon={ICONS.INFO} />
                          <a href="https://help.odysee.tv/category-monetization" target="_blank" rel="noopener noreferrer">
                            {__('Learn more')}
                          </a>
                        </div>
            }}>
            ...
            {showQR && <div className="payment-options">
                ...
                <div className="payment-option">
                  <div className="sendAr-row">
                    ...
                    <div className="sendAr-row__receiver">
                      ...
                      <input ref={inputReceivingAddressRef} type="text" placeholder={`00000000000000000000000000000000000000000`} onChange={handleCheckForm} />
                    </div>
                    <div className="sendAr-row__send">
                      <Button button="primary" title={__('Send')} label={__('Send')} disabled={!canSend || arWalletStatus?.sending} onClick={handleSendClick} />
                    </div>
                  </div>
                </div>
              </div>
          ...
          {walletType !== 'NATIVE_WALLET' && <div>
              <h2 className="section__title--small">
                <I18nMessage tokens={{
          learnMore: <div className="learn-more">
                        <Icon icon={ICONS.INFO} />
                        <a href="https://help.odysee.tv/category-monetization/wander" target="_blank" rel="noopener noreferrer">
                          {__('Learn more')}
                        </a>
                      </div>
        }}>
              ...
              <div className="payment-options-content">
                ...
                /* <h3>{__('Warning')}</h3> */
                    ...
                    <div className="payment-option__wallet-button">
                      <Button button="primary" label={__('Open wallet')} icon={ICONS.WANDER} onClick={() => window.wanderInstance.open()} />
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="transaction-history">
            {!transactions ? <Spinner type="small" /> : transactions.map((transaction, index) => <div key={index} className="transaction-history__row">
