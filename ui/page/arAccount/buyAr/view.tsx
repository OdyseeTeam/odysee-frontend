...
function BuyAr(props: Props) {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      ...
      const payment = active.paymentOptions.filter(paymentOption => paymentOption.isActive !== false && paymentOption.name !== '');
  ...
  const onFiatAmountChange = async () => {
    const res = await fetch(`https://api-stg.transak.com/api/v1/pricing/public/quotes?partnerApiKey=${apiKey}&fiatCurrency=GBP&cryptoCurrency=ETH&isBuyOrSell=BUY&network=ethereum&paymentMethod=credit_debit_card&fiatAmount=100`, {
      method: 'GET',
      headers: {
        accept: 'application/json'
      }
  ...
  const handleSetActiveFiat = (fiat: any) => {
    setActiveFiat(fiat);
    const payment = fiat.paymentOptions.filter(paymentOption => paymentOption.isActive !== false && paymentOption.name !== '');
    setPaymentOptions(payment);
  };
  ...
  const FiatOption = ({
    ...
    return <div className="fiatOption">
        <div className="fiatOption-wrapper" dangerouslySetInnerHTML={{
        __html: fiat.icon
      }} />
  ...
  return fiats.length > 0 && <>
        <Card className={activeArStatus !== 'connected' ? `card--buyAr card--disabled` : `card--buyAr`} title={cardHeader()} background actions={<>
                  ...
                  <div className="buyAr-input">
                    ...
                    <Menu>
                      ...
                      <MenuList className="menu__list channel-selector">
                        {fiats && fiats.map(fiat => // eslint-disable-next-line react/prop-types
              ...
              <Button button="primary" label={__('Review Purchase on Transak')} onClick={() => window.open(`https://global.transak.com/?apiKey=${apiKey}&defaultCryptoCurrency=AR&defaultFiatAmount=${fiatAmount}&defaultFiatCurrency=${activeFiat?.symbol ?? ''}&walletAddress=${wallet.address}&defaultPaymentMethod=${paymentOption ?? ''}`, '_blank', 'noopener,noreferrer')} disabled={!fiatAmount} />
              <p className="help">
                <I18nMessage tokens={{
          learnMore: <Button button="link" href="https://help.odysee.tv/category-monetization/exchanges" label={__('Learn more')} />
        }}>
      ...
      /* Additional Purchase Options */
        ...
        <Card className="card--buyAr-alternatives" title={<div className="buyAr-alternatives-header">
              <span>{__('Alternative Purchase Options')}</span>
              {wallet?.address && <div className="buyAr-header-address">
                  <span className="buyAr-address-label">Your AR Address:</span>
                  <CopyableText copyable={wallet.address} />
                </div>}
            </div>} background actions={<>
        ...
        /* Credit Card Providers */
              ...
              <div className="buyAr-section">
                ...
                <div className="buyAr-providers-grid">
                  {creditCardProviders.map((provider, index) => <a key={index} href={provider.url} target="_blank" rel="noopener noreferrer" className="buyAr-provider-card">
        ...
        /* Crypto Swap Services */
              ...
              <div className="buyAr-section">
                ...
                <div className="buyAr-providers-grid">
                  {cryptoSwapServices.map((service, index) => <a key={index} href={service.url} target="_blank" rel="noopener noreferrer" className="buyAr-provider-card">
                      <div className="buyAr-provider-content">
                        <h4 className="buyAr-provider-name">
                          {service.name}
                          <Icon icon={ICONS.EXTERNAL} />
        ...
        /* Cryptocurrency Exchanges */
              ...
              <div className="buyAr-section">
                ...
                <p className="buyAr-section-subtitle">
                  {__('Trade AR on established cryptocurrency exchanges. Requires account registration. Withdraw to your AR Address after purchase.')}
                </p>
                <div className="buyAr-exchanges-grid">
                  {exchanges.map((exchange, index) => <a key={index} href={exchange.url} target="_blank" rel="noopener noreferrer" className="buyAr-exchange-link">
                      {exchange.name}
                      <Icon icon={ICONS.EXTERNAL} />
                    </a>)}
                </div>
              ...
              <div className="buyAr-footer-note">
                <p>
                  {__('Odysee does not endorse any specific provider or exchange. Please do your own research and ensure you understand the risks before making any purchase.')}
                </p>
              </div>
            </>} />
      </>;
