...
export default function ChannelsPage(props: Props) {
  ...
  if (!hasChannels && !hasYoutubeChannels) {
    return <Page className="channelsPage-wrapper">
        {fetchingChannels ? <div className="main--empty">
            <Spinner delayed />
          </div> : <Yrbl title={__('No channels')} subtitle={__("You haven't created a channel yet. All of your beautiful channels will be listed here!")} actions={<div className="section__actions">
  ...
  return <Page className="channelsPage-wrapper">
      <div className="card-stack">
        {hasYoutubeChannels && <React.Suspense fallback={null}>
            <YoutubeTransferStatus hideChannelLink />
          </React.Suspense>}

        <ClaimList header={<h1 className="page__title">
              <Icon icon={ICONS.CHANNEL} />
              <label>{__('Your channels')}</label>
            </h1>} headerAltControls={<>
              <Button button="secondary" label={__('Sync YouTube Channel')} icon={ICONS.YOUTUBE} navigate={`/$/${PAGES.YOUTUBE_SYNC}`} />
              <Button button="secondary" icon={ICONS.CHANNEL} label={__('New Channel')} navigate={`/$/${PAGES.CHANNEL_NEW}`} />
            </>} loading={fetchingChannels} uris={channelUrls} renderActions={claim => {
        const claimsInChannel = claim.meta.claims_in_channel;
        return claimsInChannel === 0 ? <span /> : <div className="section__actions">
                <Button button="alt" icon={ICONS.ANALYTICS} label={__('Analytics')} onClick={() => {
