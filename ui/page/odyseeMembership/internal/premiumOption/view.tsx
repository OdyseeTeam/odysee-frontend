...
const PremiumOption = (props: Props) => {
  ...
  if (membershipView) {
    ...
    return <Wrapper name={MembershipDetails.name}>
        ...
        {(!isCancelled ? Subscription.canceled_at === 0 : !membershipStillValid) && <Button button="alt" membership-id={Membership.membership_id} onClick={() => doOpenCancelationModalForMembership(membership)} className="cancel-membership-button" label={__('Cancel membership')} icon={ICONS.FINANCE} />}
      </Wrapper>;
  }
...
const Wrapper = (props: WrapperProps) => {
  ...
  return <div className="premium-option" key={name}>
