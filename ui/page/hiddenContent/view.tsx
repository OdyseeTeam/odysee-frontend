...
function HiddenContentPage(props: Props & StateProps & DispatchProps) {
  ...
  // --------------------------------------------------------------------------
  const Header = () => {
    return <div className="hidden-content__header">
        <div className="hidden-content__filter">
          <Button button="alt" key="unlisted" label={__('Unlisted')} onClick={() => setFilter(FILTER.UNLISTED)} className={classnames(`button-toggle`, {
          'button-toggle--active': filter === FILTER.UNLISTED
        })} />
          <Button button="alt" key="scheduled" label={__('Scheduled')} onClick={() => setFilter(FILTER.SCHEDULED)} className={classnames(`button-toggle`, {
  ...
  const NoAccess = () => {
    return <Yrbl type="sad" title={__('Sorry, no soup for you!')} actions={<div className="section__actions">
  ...
  return <Page>
      {canView ? <>
          <Header />
          <ClaimSearchView key={filter === FILTER.UNLISTED ? 'unlisted' : 'scheduled'} csOptions={filter === FILTER.UNLISTED ? csOptionsUnlisted : csOptionsScheduled} layout="list" pagination="infinite" />
        </> : <NoAccess />}
    </Page>;
}
