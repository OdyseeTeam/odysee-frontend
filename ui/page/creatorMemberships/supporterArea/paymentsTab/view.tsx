...
function PaymentsTab(props: IProps) {
  ...
  const transactionsToList = (channelIdsToList && channelIdsToList.length ? transactions.filter(t => channelIdsToList.includes(t.subscriber_channel_claim_id)) : transactions).sort((a, b) => new Date(b.initiated_at).getTime() - new Date(a.initiated_at).getTime());
  return <>
      <div className="membership-payments-table__wrapper">
        <table className="table">
          ...
          <tbody>
            {transactionsToList && transactionsToList.map(transaction => {
