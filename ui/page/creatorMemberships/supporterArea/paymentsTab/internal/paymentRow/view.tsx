...
function View(props: IProps) {
  ...
  return <tr key={transaction.transaction_id}>
      ...
      <td className="channelThumbnail">
        {recipientUri ? <UriIndicator focusable={false} uri={recipientUri} link>
            <ChannelThumbnail xsmall link uri={recipientUri} />
            <label>{recipientChannel?.name}</label>
          </UriIndicator> : <div>Anon</div>}
      </td>
      <td className="channelThumbnail">
        {senderUri ? <UriIndicator focusable={false} uri={senderUri} link>
      ...
      <td>
        <span dir="auto" className="button__label">
          <Button button="primary" icon={ICONS.MEMBERSHIP} navigate={creatorChannelPath + '?view=membership'} label={membership && membership.name} />
        </span>
      </td>
