...
function View(props: IProps) {
  ...
  const {
    name: recipientName,
    claim_id: recipientClaimId
  } = recipientChannel || {};
  const {
    name: senderName,
    claim_id: senderClaimId
  } = senderChannel || {};
  const recipientUri = recipientChannel ? buildURI({
    channelName: recipientName,
    channelClaimId: recipientClaimId
  }) : null;
  const senderUri = senderChannel ? buildURI({
  ...
  return <tr key={transaction.transaction_id}>
      ...
      <td className="channelThumbnail">
        {recipientUri ? longList ? <Button button={'link'} label={recipientName} navigate={recipientUri} /> : <UriIndicator focusable={false} uri={recipientUri} link>
              <ChannelThumbnail xsmall link uri={recipientUri} />
              <label>{recipientChannel?.name}</label>
            </UriIndicator> : <div>Anon</div>}
      </td>
      <td className="channelThumbnail">
        {senderUri ? longList ? <Button button={'link'} label={senderName} navigate={senderUri} /> : <UriIndicator focusable={false} uri={senderUri} link>
      ...
      <td className="payment-txid">
        {!transaction.transaction_id.startsWith('in_') && <CopyableText hideValue linkTo={`https://viewblock.io/arweave/tx/`} copyable={transaction.transaction_id} />}
      </td>
