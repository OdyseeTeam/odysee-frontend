...
function TiersTab(props: Props) {
  ...
  function removeChannelMembershipForId(membershipId) {
    setChannelMemberships(previousMemberships => {
      const newChannelMemberships = previousMemberships.filter(membership => membership.membership_id !== membershipId);
  ...
  //   );
  // }
  return <div className={classnames('tier-edit-functionality')}>
      {channelMemberships && channelMemberships.filter(m => m.enabled === true || m.saved === false).map((membershipTier, membershipIndex) => {
      ...
      return <div className="membership-tier__wrapper" key={membershipIndex}>
                {isEditing ? <EditingTier membership={membershipTier} hasSubscribers={hasSubscribers} removeEditing={() => removeEditingForMembershipId(membershipId)} addChannelMembership={newMembership => {
        ...
        }} /> : <MembershipTier membership={membershipTier} index={membershipIndex} hasSubscribers={hasSubscribers} addEditingId={() => addEditingForMembershipId(membershipId)} removeMembership={() => removeChannelMembershipForId(membershipId)} exchangeRate={exchangeRate} />}
              </div>;
    })}

      {(!channelMemberships || channelMemberships.length < 100) &&
    /* todo change back to 6? */
    <Button button="primary" onClick={e => {
    ...
    }} className="add-membership__button" label={__('Add Tier for %channel_name%', {
      channel_name: activeChannelClaim?.name || ''
    })} icon={ICONS.ADD} />}

      <HelpHub href="https://help.odysee.tv/category-memberships/category-creatorportal/creatingtiers/" image="h264" text={__('Need some ideas on what tiers to make? Ms. H.264 has lots of ideas in the %help_hub%.')} />
    </div>;
}
