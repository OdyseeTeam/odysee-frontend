...
class TransactionListTableItem extends React.PureComponent<Props, State> {
  constructor() {
    super();
    this.state = {
      abandonState: ABANDON_STATES.READY
    };
    (this as any).abandonClaim = this.abandonClaim.bind(this);
    (this as any).getLink = this.getLink.bind(this);
  }
  ...
  getLink(type: string, tip: boolean, valueType: string) {
    ...
    if (tip && type === TXO.SUPPORT) {
      return <Button disabled={abandonState === ABANDON_STATES.DONE} button="secondary" icon={ICONS.UNLOCK} onClick={this.abandonClaim} title={__('Unlock tip')} />;
    }

    const abandonTitle = type === TXO.SUPPORT ? 'Abandon Support' : 'Abandon Claim';
    return <Button disabled={abandonState === ABANDON_STATES.DONE} button="secondary" icon={ICONS.DELETE} onClick={this.abandonClaim} title={__(abandonTitle)} />;
  }

  abandonClaim() {
    this.props.revokeClaim(this.props.txo, abandonState => this.setState({
      abandonState
    }));
  }
  ...
  render() {
    ...
    const isLbryViewReward = txo.signing_channel && (txo.signing_channel.channel_id === '74562a01e5836c04040b80bf1f4d685fec02cc90' || txo.signing_channel.channel_id === '7d0b0f83a195fd1278e1944ddda4cda8d9c01a56');
    ...
    return <tr>
        <td className="table__date">
          {timestamp ? <div>
            ...
            </div> : <span className="empty">{__('Pending')}</span>}
        </td>
        <td className="table__item--actionable">
          <span>
            {isTip && __('Tip') || type === 'support' && !isTip && __('Support --[noun; transaction type]--') || valueType && (valueType === 'stream' && __('Upload') || __(toCapitalCase(valueType))) || type && __(toCapitalCase(type))}
          </span>{' '}
        ...
        <td>
          ...
          {isLbryViewReward && <div className="table__item-label">{__('%SITE_NAME% view Credit', {
            SITE_NAME
          })}</div>}
          {isTip && signingChannel && !isLbryViewReward && <div className="table__item-label">
        ...
        <td className="table__item--align-right">
          <CreditAmount showPlus={isMinus} amount={isMinus ? Number(0 - amount) : Number(amount)} precision={8} showLBC={false} />
        </td>
      </tr>;
  }
