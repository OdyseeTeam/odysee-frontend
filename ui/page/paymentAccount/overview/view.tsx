...
function Overview(props: Props) {
    ...
    (async () => {
      if (window.arweaveWallet) {
        try {
          ...
          if (receivedData && receivedData.data && receivedData.data.transactions && receivedData.data.transactions.edges) {
  ...
  const address = '';
  return <Card className={!arWalletStatus ? `card--overview card--disabled` : `card--overview`} title={cardHeader()} background actions={<>
        ...
        return <div className="transaction-history__row" key={index}>
