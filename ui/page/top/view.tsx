...
function TopPage(props: Props) {
  ...
  if (!name) {
    return <Page className="topPage-wrapper">
  ...
  return <Page className="topPage-wrapper">
      ...
      <ClaimListDiscover name={channelActive ? `@${queryName}` : queryName} defaultFreshness={CS.FRESH_ALL} defaultOrderBy={CS.ORDER_BY_TOP} streamType={SIMPLE_SITE ? CS.CONTENT_ALL : undefined} meta={<div className="search__top-links">
          ...
          </div>} includeSupportAction renderProperties={claim => <span className="claim-preview__custom-properties">
          ...
          </span>} header={<div className="claim-search__menu-group">
            <Button label={queryName} button="alt" onClick={() => setChannelActive(false)} className={classnames('button-toggle', {
        'button-toggle--active': !channelActive
      })} />
            <Button label={`@${queryName}`} button="alt" onClick={() => setChannelActive(true)} className={classnames('button-toggle', {
        'button-toggle--active': channelActive
      })} />
          </div>} />
    </Page>;
