...
type Props = {
  ...
  // -- redux --
  isMarkdownPost: boolean | null | undefined;
  isLivestreamClaim: boolean | null | undefined;
  chatDisabled: boolean | null | undefined;
};
...
const ClaimPage = (props: Props) => {
  ...
  const ClaimRenderWrapper = React.useMemo(() => ({
    ...
    children: any;
  }) => <Page className="file-page" filePage isMarkdown={!!isMarkdownPost} noSideNavigation={isEmbedPath && !!isMarkdownPost}>
            {children}
          </Page>, [isMarkdownPost, isEmbedPath]);
  ...
  if (isChannel) {
    ...
    if (editing) {
      const ChannelPageEditingWrapperLocal = ({
        children
      }: {
        children: any;
      }) => <Page className="channelPage-wrapper channelPage-edit-wrapper" noFooter fullWidthPage noSideNavigation={isEmbedPath}>
          {children}
        </Page>;
    ...
    return <ClaimPageComponent uri={uri} ClaimRenderWrapper={ChannelPageWrapper} Wrapper={Page} latestContentPath={latestContentPath} liveContentPath={liveContentPath} />;
  }
  ...
  return <ClaimPageComponent uri={uri} ClaimRenderWrapper={ClaimRenderWrapper} Wrapper={Page} latestContentPath={latestContentPath} liveContentPath={liveContentPath} />;
};
