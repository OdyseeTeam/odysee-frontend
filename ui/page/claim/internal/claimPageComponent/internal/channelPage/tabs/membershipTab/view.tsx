...
const MembershipTab = (props: Props) => {
  ...
  const activeMemberships = myMembershipSubscriptions && myMembershipSubscriptions.length > 0 ? myMembershipSubscriptions.filter(ms => ms.subscription.is_active === true) : [];
  ...
  return <>
      {activeMemberships.length > 0 && <div className={'membership-tab-item__wrapper'}>
          <div className={'card__header--between membership-tab-header__wrapper'}>
            ...
            <div className="button--view-memberships">
              <Button icon={ICONS.MEMBERSHIP} button="primary" type="submit" label={__('See all my memberships')} navigate={`/$/${PAGES.MEMBERSHIPS_SUPPORTER}`} />
            </div>
          </div>
          {activeMemberships.map((subscription, index) => <MembershipSub uri={uri} membershipSub={subscription} key={subscription.membership.name} />)}
        </div>}
