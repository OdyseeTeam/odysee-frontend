...
type Props = {
  ...
  doFetchThumbnailClaimsForCollectionIds: (params: {
...
function HomeTabSection(props: Props) {
  ...
  const shouldPerformSearch = !singleClaimUri && !fetchingClaimSearch && !timedOut && !claimSearchResults && !collectionUrls && section;
  ...
  React.useEffect(() => {
    const timer = setTimeout(() => {
      ...
      setIsSearching(true);
      lighthouse.search(`&s=${encodeURIComponent(searchQuery)}` + `&channel_id=${encodeURIComponent(channelClaimId)}` + `&nsfw=${'false'}` + `&resolve=${'true'}` + `&size=${SEARCH_PAGE_SIZE}`).then(({
        body: results
      }) => {
    ...
    return () => clearTimeout(timer); // eslint-disable-next-line react-hooks/exhaustive-deps -- @see TODO_NEED_VERIFICATION (definitely needs channelClaimId, no?)
  }, [searchQuery]);
  ...
  function getTitle() {
    switch (section.type) {
      ...
      case 'channels':
        return featuredChannels && featuredChannels.find(list => list.id === section.claim_id) && featuredChannels.find(list => list.id === section.claim_id).value.title;
  ...
  return <div className="home-section-content">
      {!editMode && index === 0 && <UpcomingClaims name="homeTab" channelIds={scheduledChanIds} tileLayout={false} showHideSetting={false} liveUris={liveUris} isChannelPage />}
      {editMode && <div className="home-section-header-wrapper">
          <div className="home-section-header-option">
            <label>{__('Type')}</label>
            <select name="type" value={section.type || 'select'} onChange={e => handleEditCollection({
          change: {
            field: e.target.name,
          ...
          {section.type === 'content' && <div className="home-section-header-option">
              <label>{__('File Type')}</label>
              <select name="file_type" value={section.file_type || CS.FILE_TYPES} onChange={e => handleEditCollection({
          change: {
            field: e.target.name,
            value: e.target.value
          }
          ...
          {section.type === 'playlist' && <div className="home-section-header-option">
              <label>{__('Playlist')}</label>
              <select name="claim_id" value={section.claim_id || 'select'} onChange={e => handleEditCollection({
          change: {
            field: e.target.name,
            value: e.target.value
          }
                ...
                {publishedList && publishedList.map((list, i) => {
            return <option key={i} value={list}>
                        {publishedCollections[list].name}
                      </option>;
          })}
              </select>
            </div>}
          {section.type === 'channels' && <div className="home-section-header-option">
          ...
          change: {
            ...
            value: e.target.value
                ...
                {featuredChannels && featuredChannels.map((list, i) => {
            return <option key={i} value={list.id}>
                        {list.value.title}
                      </option>;
          })}
              </select>
            </div>}
          {(section.type === 'content' || section.type === 'playlists') && <div className="home-section-header-option">
              <label>{__('Order By')}</label>
              <select name="order_by" value={section.order_by && section.order_by[0] || 'release_time'} onChange={e => handleEditCollection({
          change: {
            field: e.target.name,
            value: e.target.value
          }
          ...
          {section.type === 'featured' && <>
              <div className="home-section-header-option">
                ...
                <input id="featured" name="search" value={searchQuery} onChange={handleInputChange} placeholder={__('Search')} />
              </div>
              <div className="home-section-header-option">
                <label>{__('Results')}</label>
                <select name="claim_id" value={section.claim_id || 'select'} disabled={!searchResults} onChange={e => handleEditCollection({
            change: {
              field: e.target.name,
              value: e.target.value
            }
              ...
              return <option key={i} value={result.claimId}>
                          {result.title}
                        </option>;
      ...
      {isLoading && <div className="home-section-content">
          <div className="section">
            <h2 className="home-section-title">{__('Loading...')}</h2>
            <section className="claim-grid">
              {new Array(12).fill(0).map((x, i) => <ClaimPreviewTile key={i} placeholder="loading" pulse />)}
            </section>
          </div>
        </div>}
      {!isLoading && !editMode && (requiresSearch && claimSearchResults && claimSearchResults.length === 0 || section.type === 'playlist' && !section.claim_id) && <div className="section">
            <div className="empty empty--centered">{__('No Content Found')}</div>
          </div>}
      {section.type && (section.claim_id || collectionUrls || claimSearchResults && claimSearchResults.length > 0 || section.type === 'featured' && hasFeaturedClaim) && <div className="section">
            {section.type !== 'featured' ? <>
                <h2 className="home-section-title">{collectionName || getTitle() || __('Loading...')}</h2>
                {(!featuredChannel || featuredChannel && featuredChannel.value.uris.length > maxChannelsPerRow) && <label className="show-more" onClick={() => handleViewMore(section)}>
                    {__('View More')}
                    <Icon icon={ICONS.ARROW_RIGHT} />
                  </label>}
                {section.type !== 'channels' ? (collectionUrls || claimSearchResults || collectionClaimIds) && <ClaimListDiscover hideFilters hideAdvancedFilter hideLayoutButton tileLayout infiniteScroll={false} useSkeletonScreen={false} uris={collectionUrls || claimSearchResults} maxClaimRender={maxClaimsInSection} claimIds={collectionClaimIds} /> : featuredChannel && <ChannelSection key={'featured-channels'} uris={featuredChannel && featuredChannel.value.uris.slice(0, maxChannelsPerRow)} channelId={channelClaimId} />}
              </> : <FeaturedSection uri={singleClaimUri || claimSearchResults && claimSearchResults[0]} claimId={section.claim_id} />}
          </div>}
    </div>;
}
