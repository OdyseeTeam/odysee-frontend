...
function MembershipSubscribed(props: IProps) {
  ...
  const canRenew = membershipSub.subscription.earliest_renewal_at && now > new Date(membershipSub.subscription.earliest_renewal_at) && !pending;
  return <>
      <Card className="membership membership-tab" body={<>
            <div className={'membership__body membership-tier' + styleIndex}>
              <div className="membership__plan-header">
                ...
                {isActive && !isCanceled && <Menu>
                    <MenuButton className="menu__button">
                      <Icon size={18} icon={ICONS.SETTINGS} />
                    </MenuButton>
                    <MenuList className={'menu__list membership-tier' + styleIndex}>
                      <MenuItem className="comment__menu-option" onSelect={() => doOpenCancelationModalForMembership(membershipSub)}>
              ...
              <div className="membership__plan-content">
                ...
                {perks && <div className="membership-tier__perks">
                    <label>{__('Odysee Perks')}</label>

                    <ul>
                      {perks.map((tierPerk, i) => <li key={i} className="membership__perk-item">
                          {__(tierPerk.name)}
                        </li>)}
                    </ul>
                  </div>}
                ...
                <div className="membership__plan-actions">
                  {tipsEnabled && (isActive && !isCanceled ? canRenew ? <Button icon={ICONS.MEMBERSHIP} button="primary" label={__('Renew for $%membership_price% this month', {
              membership_price: (membershipSub.subscription.current_price.amount / 100).toFixed(membershipSub?.subscription.current_price.amount < 100 ? 2 : 0) // tiers
              ...
              doOpenModal(MODALS.JOIN_MEMBERSHIP, {
                ...
                isRenewal: true
                          ...
                          {pending ? __('Renewal being processed') : __('You can renew this membership on or after %renewal_date%', {
                ...
                end_date: formattedEndOfMembershipDate
                  ...
                  {!tipsEnabled && <div>Enjoy this legacy membership while the creator onboards the new tip system</div>}
                </div>
              </div>
            </div>
          </>} />
