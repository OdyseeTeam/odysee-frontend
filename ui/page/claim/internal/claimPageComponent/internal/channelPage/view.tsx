...
import "./style.scss";
const HiddenNsfwClaims = lazyImport(() => import('component/hiddenNsfwClaims'
/* webpackChunkName: "hiddenNsfwClaims" */
));
const TABS_FOR_CHANNELS_WITH_CONTENT = [CHANNEL_PAGE.VIEWS.HOME, CHANNEL_PAGE.VIEWS.CONTENT, CHANNEL_PAGE.VIEWS.PLAYLISTS, CHANNEL_PAGE.VIEWS.CHANNELS];
type Props = {
  uri: string;
  claim: ChannelClaim;
...
function ChannelPage(props: Props) {
  ...
  if (!showClaims && (isHomeTab || TABS_FOR_CHANNELS_WITH_CONTENT.includes(viewParam) || viewParam === CHANNEL_PAGE.VIEWS.ABOUT)) {
  ...
  const isMyYouTubeChannel = claim && youtubeChannels && youtubeChannels.some(({
    channel_claim_id,
    sync_status,
    transfer_state
  }) => {
    if (channel_claim_id === claim.claim_id && sync_status !== YOUTUBE_STATUSES.YOUTUBE_SYNC_ABANDONDED && transfer_state !== YOUTUBE_STATUSES.YOUTUBE_SYNC_COMPLETED_TRANSFER) {
      return true;
    }
  });
  ...
  if (channelIsMine) {
    collectionEmpty = hasUnpublishedCollections ? <section className="main--empty">
        {<p>
            <I18nMessage tokens={{
      ...
      </section> : <section className="main--empty">{__('You have no playlists! Create one from any playable content.')}</section>;
  } else {
  ...
  function handleViewMore(section) {
    function getOrderBy() {
      return section.order_by && section.order_by[0] === 'trending_group' ? CS.ORDER_BY_TRENDING : section.order_by[0] === 'effective_amount' ? CS.ORDER_BY_TOP : CS.ORDER_BY_NEW;
    }

    function getFileType() {
      return section.file_type && section.file_type.length === 1 && section.file_type[0] === 'video' ? CS.FILE_VIDEO : section.file_type && section.file_type.length === 1 && section.file_type[0] && section.file_type[0] === 'audio' ? CS.FILE_AUDIO : section.file_type && section.file_type.length === 1 && section.file_type[0] && section.file_type[0] === 'document' ? CS.FILE_DOCUMENT : section.file_type && section.file_type.length === 1 && section.file_type[0] && section.file_type[0] === 'image' ? CS.FILE_IMAGE : undefined;
    }
  ...
  return <ChannelPageContext.Provider value>
      <header className={classnames('channel-cover', {
      ...
      backgroundImage: 'url(' + String(coverUrl) + ')'
        ...
        <div className="channel-header-content">
          <div className="channel__quick-actions">
            {isMyYouTubeChannel && <Button button="alt" label={__('Claim Your Channel')} icon={ICONS.YOUTUBE} navigate={`/$/${PAGES.CHANNELS}`} />}
            {!(isBlocked || isMuted || isMature) && <JoinMembershipButton uri={uri} />}
            {!(isBlocked || isMuted || isMature || channelIsBlackListed) && <ClaimShareButton uri={uri} webShareable shrinkOnMobile />}
            {!(isBlocked || isMuted || isMature) && <ClaimSupportButton uri={uri} shrinkOnMobile />}
            {!(isBlocked || isMuted || isMature) && (!channelIsBlackListed || isSubscribed) && <SubscribeButton uri={permanentUrl} shrinkOnMobile />}
            ...
            <div className="channel__meta">
              <Tooltip title={formattedSubCount} followCursor placement="top">
                <span>
                  ...
                  {Number.isInteger(subCount) ? <HelpLink href="https://help.odysee.tv/category-interaction/following/" /> : '\u00A0'}
                </span>
              </Tooltip>
            </div>
            <div className="channel__edit">
              {channelIsMine && <>
                  {pending ? <span>{__('Your changes will be live in a few minutes')}</span> : <Button button="alt" title={__('Edit')} onClick={() => push(`?${CHANNEL_PAGE.QUERIES.VIEW}=${CHANNEL_PAGE.VIEWS.EDIT}`)} icon={ICONS.EDIT} iconSize={18} disabled={pending} />}
                </>}
            </div>
          </div>
        </div>
      ...
      {isMature ? <React.Suspense fallback={null}>
          <HiddenNsfwClaims uri={uri} mature />
        </React.Suspense> : (isBlocked || isMuted) && !viewBlockedChannel ? <div className="main--empty">
          <Yrbl title={isBlocked ? __('This channel is blocked') : __('This channel is hidden')} subtitle={isBlocked ? __('Are you sure you want to view this content? Viewing will not unblock @%channel%', {
        channel: channelName
        ...
        </div> : <Tabs onChange={onTabChange} index={tabIndex}>
        ...
        'tab__wrapper--fixed': scrollPast
          ...
          behavior: 'smooth'
              ...
              <ChannelThumbnail className={classnames('channel__thumbnail--channel-page', {
            'channel__thumbnail--channel-page-fixed': scrollPast
          })} uri={uri} allowGifs isChannel hideStakedIndicator />
            </div>
            <TabList>
              ...
              <Tab disabled={editing || !showClaims || !hasShorts} className={classnames({
            'tab--hidden': !hasShorts || hideShorts
          })} aria-selected={tabIndex === 2} onClick={() => onTabChange(2)}>
                {__('Shorts')}
              </Tab>
              ...
              <Tab className="tab--membership" aria-selected={tabIndex === 5} disabled={editing || isOdyseeChannel} onClick={() => onTabChange(5)}>
                {__('Membership')}
              </Tab>
          ...
          })} aria-selected={tabIndex === 7} onClick={() => onTabChange(7)}>
          ...
          <TabPanels>
            <TabPanel>
              {currentView === CHANNEL_PAGE.VIEWS.HOME && <HomeTab uri={uri} editMode={channelIsMine} handleViewMore={e => handleViewMore(e)} />}
            </TabPanel>
            <TabPanel>
              {currentView === CHANNEL_PAGE.VIEWS.CONTENT && <ContentTab uri={uri} channelIsBlackListed={channelIsBlackListed} viewHiddenChannels claimType={['stream', 'repost']} empty={<section className="main--empty">{__('No Content Found')}</section>} filters={filters} excludeShorts />}
            </TabPanel>
            <TabPanel>
              <div style={{
            display: currentView === CHANNEL_PAGE.VIEWS.SHORTS ? 'block' : 'none'
          }}>
                <ContentTab uri={uri} channelIsBlackListed={channelIsBlackListed} viewHiddenChannels claimType={['stream', 'repost']} empty={<section className="main--empty">{__('No Shorts Found')}</section>} filters={filters} loadedCallback={handleShortsLoaded} shortsOnly />
              </div>
            </TabPanel>
            <TabPanel>
              {currentView === CHANNEL_PAGE.VIEWS.PLAYLISTS && <ContentTab claimType={'collection'} uri={uri} channelIsBlackListed={channelIsBlackListed} viewHiddenChannels empty={collectionEmpty} />}
            </TabPanel>
            <TabPanel>
            ...
            <TabPanel>
              {!hideAboutTab && currentView === CHANNEL_PAGE.VIEWS.ABOUT && <AboutTab uri={uri} channelIsBlackListed={channelIsBlackListed} />}
            </TabPanel>
            <TabPanel>
              {channelIsMine && currentView === CHANNEL_PAGE.VIEWS.SETTINGS && <CreatorSettingsTab activeChannelClaim={claim} />}
            </TabPanel>
          </TabPanels>
        </Tabs>}
