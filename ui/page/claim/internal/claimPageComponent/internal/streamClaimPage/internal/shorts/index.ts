...
import { selectClaimIsNsfwForUri, selectClaimForUri, makeSelectTagInClaimOrChannelForUri, selectClaimSearchByQuery, selectTitleForUri } from "redux/selectors/claims";
import { selectContentPositionForUri, selectPlayingCollectionId, selectIsUriCurrentlyPlaying, selectIsAutoplayCountdownForUri } from "redux/selectors/content";
...
import { doSetShortsSidePanel, doToggleShortsSidePanel, doSetShortsPlaylist, doSetShortsViewMode, doSetShortsAutoplay, doClearShortsPlaylist } from "redux/actions/shorts";
...
const selectShortsRecommendedContent = createSelector([selectShortsPlaylist, selectShortsViewMode, (state, props) => {
  if (!props?.uri) return [];
  ...
  for (const queryKey in searchResults) {
    if (queryKey.includes(`s=${titleEncoded}`) && queryKey.includes(`max_aspect_ratio=${SETTINGS.SHORTS_ASPECT_RATIO_LTE}`)) {
  ...
  return claimSearchByQuery[searchKey] ? claimSearchByQuery[searchKey].map(uri => {
...
const select = (state, props) => {
  ...
  const nextShortUri = currentIndex >= 0 && currentIndex < shortsRecommendedUris.length - 1 ? shortsRecommendedUris[currentIndex + 1] : null;
  ...
  return {
    ...
    nextRecommendedShort: currentIndex >= 0 && currentIndex < shortsRecommendedUris.length - 1 ? shortsRecommendedUris[currentIndex + 1] : null,
    previousRecommendedShort: currentIndex > 0 ? shortsRecommendedUris[currentIndex - 1] : null,
    currentIndex,
    channelId,
    channelName: claim?.signing_channel?.name,
    channelDisplayName: channelUri ? selectTitleForUri(state, channelUri) || claim?.signing_channel?.name : claim?.signing_channel?.name,
