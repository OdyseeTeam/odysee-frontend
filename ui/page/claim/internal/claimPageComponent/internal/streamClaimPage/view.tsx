...
const MarkdownPostPage = lazyImport(() => import('./internal/markdownPost'
/* webpackChunkName: "markdownPost" */
));
const VideoPlayersPage = lazyImport(() => import('./internal/videoPlayers'
/* webpackChunkName: "videoPlayersPage" */
));
const LivestreamPage = lazyImport(() => import('./internal/livestream'
...
const StreamClaimPage = (props: Props) => {
  ...
  function renderClaimLayout() {
    if (RENDER_MODES.UNRENDERABLE_MODES.includes(renderMode)) {
      return <>
          {thumbnail && <div className={PRIMARY_IMAGE_WRAPPER_CLASS}>
    ...
    if (RENDER_MODES.TEXT_MODES.includes(renderMode)) {
      return <div className="file-page__pdf-wrapper">
          <StreamClaimRenderInline uri={uri} />
        </div>;
    }

    if (renderMode === RENDER_MODES.IMAGE) {
      return <div className={PRIMARY_IMAGE_WRAPPER_CLASS}>
          <StreamClaimRenderInline uri={uri} />
        </div>;
    }
  ...
  function dmcaInfo() {
    return <section className="card--section dmca-info">
        <p>
  ...
  return <>
      <div className={classnames('section card-stack', `file-page__${renderMode}`)}>
        ...
        {contentUnlocked && <div className="file-page__secondary-content">
            <section className="file-page__media-actions">
              <React.Suspense fallback={null}>
                {commentsDisabled ? <Empty {...emptyMsgProps} text={__('The creator of this content has disabled comments.')} /> : isMobile && !isLandscapeRotated ? <React.Fragment>
                  ...
                  </React.Fragment> : <CommentsList {...commentsListProps} notInDrawer />}
              </React.Suspense>
            </section>
          </div>}
      </div>
