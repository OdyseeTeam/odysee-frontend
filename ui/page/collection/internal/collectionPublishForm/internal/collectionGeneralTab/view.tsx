...
function CollectionGeneralTab(props: Props) {
  ...
  function handleUpdateThumbnail(update: Record<string, string>) {
    ...
    if (url?.length >= 0) {
      const newParams = url.length === 0 ? {
        thumbnail_url: undefined
      } : update;
      updateFormParams(!publishing ? {
  ...
  React.useEffect(() => {
    const thumbnailError = thumbError && thumbStatus !== THUMBNAIL_STATUSES.COMPLETE ? __('Invalid thumbnail') : thumbStatus === THUMBNAIL_STATUSES.IN_PROGRESS ? __('Please wait for thumbnail to finish uploading') : undefined;
  ...
  return <div className="card card--background collection-edit__wrapper">
      {publishing && <>
          <ChannelSelector autoSet channelToSet={collectionChannelId} onChannelSelect={id => updateFormParams({
        channel_id: id
      })} />
          ...
          <div className="collection__name-wrapper">
            ...
            <div className="collection__name">
              <FormUrlName channelName={collectionChannelName} name={name} autoFocus={!hasClaim} disabled={hasClaim} onChange={e => updateFormParams({
            name: e.target.value || ''
          })} />
      ...
      <div className="collection__title">
        <h2>{__('Title')}</h2>
        <FormField type="text" name="collection_title" placeholder={__('My Awesome Playlist')} value={title || ''} onChange={e => updateFormParams({
        title: e.target.value || ''
      })} />
      </div>

      <fieldset-section>
        <SelectThumbnail thumbnailParam={thumbnailUrl} thumbnailParamError={thumbError} thumbnailParamStatus={thumbStatus} updateThumbnailParams={handleUpdateThumbnail} optional />
      </fieldset-section>

      <h2>{__('Description')}</h2>
      <FormField type="markdown" name="collection_description" value={typeof description === 'string' && description || ''} onChange={value => updateFormParams({
      description: value || ''
      ...
      <Card background body={<div className="publish-row">
            <TagsSelect suggestMature={!SIMPLE_SITE} disableAutoFocus hideHeader label={__('Selected Tags')} empty={__('No tags added')} excludedControlTags={[TAGS.DISABLE_COMMENTS_TAG, TAGS.DISABLE_DOWNLOAD_BUTTON_TAG, TAGS.DISABLE_REACTIONS_COMMENTS_TAG, TAGS.DISABLE_SLIMES_COMMENTS_TAG]} limitSelect={TAGS_LIMIT} help={__("Add tags that are relevant to your content so those who're looking for it can find it more easily. If your content is best suited for mature audiences, ensure it is tagged 'mature'.")} placeholder={__('gaming, crypto')} onSelect={newTags => {
        const validatedTags = [];
