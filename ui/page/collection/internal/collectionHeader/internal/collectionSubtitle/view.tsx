...
const CollectionTitle = (props: Props) => {
  ...
  return <div>
      {sourceId && <span className="collection__subtitle">
          <Button iconRight={ICONS.EXTERNAL} label={__('View copied playlist source')} button="link" navigate={`/$/${PAGES.PLAYLIST}/${sourceId}`} />
        </span>}

      {collectionCount || collectionCount === 0 ? <span className="collection__subtitle">
          {collectionCount === 1 ? __('1 item') : __('%collectionCount% items', {
        collectionCount
      })}

          {hasClaim && <div className="collection-subtitle__info">
              ...
              <Button button="link" className="dim" onClick={() => handleExpand(EXPAND.CREDIT_DETAILS)}>
                <LbcSymbol postfix={expand === EXPAND.CREDIT_DETAILS ? __('Hide') : getFormattedCreditsAmount(claimAmount)} />
              </Button>

              <ClaimSupportsLiquidateButton uri={uri} />
            </div>}
        </span> : <Skeleton variant="text" animation="wave" className="header__navigationItem--balanceLoading" />}
