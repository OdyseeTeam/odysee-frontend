        ...
        <SectionElement>
          {!isBuiltin && <>
              ...
              {uri && <>
                  ...
                  {collectionHasEdits && publishError && <Tooltip title={__('Last publish failed. Open menu to retry.')} arrow={false} enterDelay={100}>
                      <div className="pending-change">
          ...
          <Menu>
            <MenuButton className="menu__button" onClick={e => {
            e.stopPropagation();
            e.preventDefault();
          }}>
            ...
            <MenuList className="menu__list">
              {isMyCollection && isNotADefaultList && !showEdit && <MenuItem className="comment__menu-option" onSelect={() => push(`/$/${PAGES.PLAYLIST}/${collectionId}?${COLLECTION_PAGE.QUERIES.VIEW}=${COLLECTION_PAGE.VIEWS.EDIT}`)}>
                  <div className="menu__link">
                    <Icon aria-hidden icon={ICONS.EDIT} />
                    {__('Edit')}
                  </div>
                </MenuItem>}
              {isMyCollection && !isBuiltin && claimId && <MenuItem className="comment__menu-option" onSelect={() => doSetCollectionAutoPublish(collectionId, !autoPublish)}>
                  <div className="menu__link">
                    <Icon aria-hidden icon={ICONS.PUBLISH} />
                    {autoPublish ? __('Disable Auto-publish') : __('Enable Auto-publish')}
                  </div>
                </MenuItem>}
              {isMyCollection && !isBuiltin && claimId && collectionHasEdits && publishError && <MenuItem className="comment__menu-option" onSelect={() => doRetryCollectionPublish(collectionId)}>
                  <div className="menu__link">
                    <Icon aria-hidden icon={ICONS.REFRESH} />
                    {__('Retry Publish Now')}
                  </div>
                </MenuItem>}
              {!isMyCollection && claimId && <MenuItem className="comment__menu-option" onSelect={() => doToggleCollectionSavedForId(claimId)}>
                  <div className="menu__link">
                    <Icon aria-hidden icon={ICONS.PLAYLIST_ADD} />
                    {__('Save')}
                  </div>
                </MenuItem>}
              {isMyCollection && <MenuItem className="comment__menu-option" onSelect={() => setShowEdit(true)}>
                  <div className="menu__link">
                    <Icon aria-hidden icon={ICONS.ARRANGE} />
                    {__('Arrange Items')}
                  </div>
                </MenuItem>}
              <MenuItem className="comment__menu-option" onSelect={() => doOpenModal(MODALS.COLLECTION_CREATE, {
              ...
              {isMyCollection && isNotADefaultList && <MenuItem className="comment__menu-option" onSelect={() => doOpenModal(MODALS.COLLECTION_DELETE, {
              uri,
              collectionId,
              ...
              {!isMyCollection && claimId && <MenuItem className="comment__menu-option" onSelect={() => push(`/$/${PAGES.REPORT_CONTENT}?claimId=${claimId}`)}>
                  <div className="menu__link">
