...
function CollectionActions(props: Props) {
  ...
  return <>
      <div className={classnames('media__actions justify-space-between collection-actions', {
      stretch: isMobile
    })}>
        <SectionElement>
          ...
          {!isBuiltin && !disableFileReactions && <>{uri && <>{ENABLE_FILE_REACTIONS && <FileReactions uri={uri} />}</>}</>}
        </SectionElement>
