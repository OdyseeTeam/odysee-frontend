...
class SettingsPage extends React.PureComponent<Props> {
  ...
  render() {
    ...
    if (isAuthenticated && !prefsReady) {
      return <Page noFooter settingsPage noSideNavigation backout={{
        title: __('Settings'),
        backLabel: __('Save')
    ...
    return <Page noFooter settingsPage noSideNavigation backout={{
      title: __('Settings'),
      backLabel: __('Save')
    }} className="card-stack" key={language}>
        {!isAuthenticated && IS_WEB && <>
            <SettingUnauthenticated />
            <div className="main--empty">
              <Yrbl type="happy" title={__('Sign up for full control')} subtitle={__('Unlock new buttons that change things.')} actions={<div className="section__actions">
                    <Button button="primary" icon={ICONS.SIGN_UP} label={__('Sign Up')} navigate={`/$/${PAGES.AUTH}`} />
                  </div>} />
            </div>
          </>}

        {!IS_WEB && noDaemonSettings ? <section className="card card--section">
            <div className="card__title card__title--deprecated">{__('Failed to load settings.')}</div>
          </section> : <div className={classnames('card-stack', {
