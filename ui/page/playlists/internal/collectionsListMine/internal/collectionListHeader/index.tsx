...
export default function CollectionsListMine(props: Props) {
  ...
  return <div className="section__header-action-stack">
        ...
        <div className="claim-search__wrapper--wrap">
          ...
          <div className="claim-search__menu-group">
            <div className="claim-search__menu-subgroup">
              ...
              {Object.values(COLS.LIST_TYPE).map(value => <Button label={__(String(value))} key={String(value)} button="alt" // $FlowFixMe
            onClick={() => handleFilterTypeChange(String(value))} className={classnames('button-toggle', {
              'button-toggle--active': filterType === value
            })} />)}
            </div>
            <div className="claim-search__menu-subgroup">
              <FormField className="claim-search__dropdown" type="select" name="sort_by" value={sortOption.key} onChange={e => handleChange({
              key: e.target.value,
              value: COLS.SORT_ORDER.ASC
            })}>
                {Object.entries(COLS.SORT_VALUES).map(([key, value]) => <option key={key} value={key}>
                    {
                  /* $FlowFixMe */
                }
                    {__(value.str)}
              ...
              <FormField className="claim-search__dropdown" type="select" name="order_by" value={sortOption.value} onChange={e => handleChange({
              key: sortOption.key,
              value: e.target.value
            })}>
                {Object.entries(COLS.SORT_ORDER).map(([key, value]) => // $FlowFixMe
