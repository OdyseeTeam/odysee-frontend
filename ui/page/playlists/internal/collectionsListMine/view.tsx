...
export default function CollectionsListMine(props: Props) {
  ...
  const defaultSortOption = sortByParam ? {
    key: sortByParam,
  ...
  const paginatedCollections = React.useMemo(() => filteredCollections.slice(totalPages >= page ? firstItemIndexForPage : 0, lastItemIndexForPage), [filteredCollections, firstItemIndexForPage, lastItemIndexForPage, page, totalPages]);
  ...
  React.useEffect(() => {
    setPersistedOption(sortOption); // eslint-disable-next-line react-hooks/exhaustive-deps -- (setPersistedOption is custom setState, can ignore)
  }, [sortOption]);
  return <>
      <SectionLabel label={__('Your Playlists')} />

      <CollectionsListContext.Provider value={{
      searchText,
      setSearchText,
      totalLength,
      filteredCollectionsLength
    }}>
        <CollectionListHeader filterType={filterType} isTruncated={totalLength > filteredCollectionsLength} setFilterType={setFilterType} // $FlowFixMe
      ...
      {filteredCollectionsLength > 0 ? <ul className={classnames('ul--no-style claim-list', {
      playlists: !isMobile
    })}>
          {
        /* !isMobile && <TableHeader /> */
          ...
          {paginatedCollections.map(key => <CollectionPreview collectionId={key} key={key} />)}

          {totalPages > 1 && <PageItemsLabel totalLength={filteredCollectionsLength} firstItemIndexForPage={firstItemIndexForPage} paginatedCollectionsLength={paginatedCollections.length} />}

          <Paginate totalPages={totalPages} shouldResetPageNumber={filterParamsChanged} />
        </ul> : <div className="empty main--empty">{__('No matching playlists')}</div>}
    </>;
}