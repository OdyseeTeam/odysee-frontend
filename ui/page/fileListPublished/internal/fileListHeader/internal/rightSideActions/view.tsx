...
const RightSideActions = () => {
  ...
  return <div className="claim-search__wrapper--wrap">
      ...
      {isFilteringEnabled && <div className="claim-search__menu-group">
          <div className="claim-search__menu-subgroup">
            <FormField className="claim-search__dropdown uploads__dropdown" type="select" name="sort_by" value={sortOption.key} onChange={e => handleChange({
          key: e.target.value,
          value: FILE_LIST.SORT_ORDER.ASC
        })}>
              {Object.entries(FILE_LIST.SORT_VALUES).map(([key, value]) => <option key={key} value={key}>
                  {
              /* $FlowFixMe */
            }
                  {__(value.str)}
            ...
            <FormField className="claim-search__dropdown uploads__dropdown" type="select" name="order_by" value={sortOption.value} onChange={e => handleChange({
          key: sortOption.key,
          value: e.target.value
        })}>
              {Object.entries(FILE_LIST.SORT_ORDER).map(([key, value]) => // $FlowFixMe
