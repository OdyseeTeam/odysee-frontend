...
function FileListPublished(props: Props) {
  ...
  const memoizedSortOption = React.useMemo(() => {
    ...
    } : sortOption; // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const method = [FILE_LIST.FILE_TYPE.UNLISTED.key, FILE_LIST.FILE_TYPE.SCHEDULED.key, FILE_LIST.FILE_TYPE.PAID.key, FILE_LIST.FILE_TYPE.PAID_LEGACY.key].includes(filterType) && !isFilteringEnabled ? FILE_LIST.METHOD.CLAIM_SEARCH : FILE_LIST.METHOD.CLAIM_LIST;
  ...
  const claimsToShow = React.useMemo(() => {
    ...
    return claims;
  }, [myClaims, myStreamClaims, myRepostClaims, myUnlistedClaims, myScheduledClaims, myPaidClaims, myPaidClaimsLegacy, filterType, isFilteringEnabled]);
  const filteredClaims = React.useMemo(() => {
    ...
    return result.sort((claimA, claimB) => {
      ...
      let firstComparisonItem = memoizedSortOption.value === FILE_LIST.SORT_ORDER.ASC ? nameComparisonClaimA : nameComparisonClaimB;
      let secondComparisonItem = memoizedSortOption.value === FILE_LIST.SORT_ORDER.ASC ? nameComparisonClaimB : nameComparisonClaimA;
      const comparisonObj = {};
  ...
  const AdvisoryMsg = () => {
    if (filterType === FILE_LIST.FILE_TYPE.UNLISTED.key) {
      return <div className="flp__advisory">
          <Icon icon={ICONS.INFO} />
          <p>
            {__('A special link is required to share unlisted contents. The link can be obtained from "Copy Link" in the context menu, or the "Share" functionality in the file page.')}
          </p>
        </div>;
    }
  ...
  function getClaimListResultsJsx() {
    ...
    const claimUrls = !isFilteringEnabled ? urls : filteredClaims.slice(startIndex, endIndex).map(claim => claim.permanent_url);
    const totalClaims = !isFilteringEnabled ? urlTotal : filteredClaims?.length;
    return <>
        {!!urls && <>
            <ClaimList noEmpty persistedStorageKey="claim-list-published" showHiddenByUser uris={fetching ? [] : claimUrls} loading={fetching} />
            {isLoadingLong && isFilteringEnabled && <div className="main--empty">
                <Spinner type="small" />
                <p>{__('Larger upload list may take time to load with filters enabled')}</p>
              </div>}

            {getFetchingPlaceholders()}
            <Paginate totalPages={totalClaims > 0 ? Math.ceil(totalClaims / Number(pageSize)) : 1} shouldResetPageNumber={filterParamsChanged} />
          </>}
      </>;
  }
  ...
  function getClaimSearchResultsJsx() {
    ...
    <ClaimSearchView key={filterType} csOptions={csOptions[filterType]} layout="list" pagination="infinite" /> : <Yrbl title={__('No uploads')} subtitle={__("You haven't uploaded anything yet. This is where you can find them when you do!")} />;
  }

  function getFetchingPlaceholders() {
    return <>
        {method === FILE_LIST.METHOD.CLAIM_LIST && fetching && new Array(Number(pageSize)).fill(1).map((x, i) => {
  ...
  useEffect(() => {
    ...
    if (method === FILE_LIST.METHOD.CLAIM_LIST) {
      fetchClaimListMine(params.page, params.page_size, true, // $FlowFixMe
      Object.values(FILE_LIST.FILE_TYPE) // $FlowFixMe
      .find(fileType => fileType.key === filterType).cmd.split(','), true, channelIdsClaimList);
    } else {
      doClearClaimSearch();
    }
  }, [uploadCount, params, filterType, fetchClaimListMine, method, isFilteringEnabled, channelIdsClaimList, doClearClaimSearch]);
  useEffect(() => {
  ...
  return <Page>
      <div className="card-stack">
        {!!myChannelIds?.length && myChannelIds?.length > 1 && <ChannelSelector hideAnon allOptionProps={{
        onSelectAll: () => setIsAllSelected(true),
      ...
      {!(myClaims && myClaims.length) && method === FILE_LIST.METHOD.CLAIM_LIST && <React.Fragment>
          {!fetching && hasClaims === false ? <section className="main--empty">
              <Yrbl title={filterType === FILE_LIST.FILE_TYPE.REPOSTS ? __('No Reposts') : __('No uploads')} subtitle={filterType === FILE_LIST.FILE_TYPE.REPOSTS ? __("You haven't reposted anything yet.") : __("You haven't uploaded anything yet. This is where you can find them when you do!")} actions={filterType !== FILE_LIST.FILE_TYPE.REPOSTS && <div className="section__actions">
                      <Button button="primary" label={__('Upload Something New')} onClick={() => doBeginPublish('file')} />
                    </div>} />
            </section> : <section className="main--empty">
              <Spinner delayed />
            </section>}
        </React.Fragment>}
    </Page>;
