...
class RewardsPage extends PureComponent<Props> {
  ...
  renderPageHeader() {
    ...
    if (user && !user.is_reward_approved && rewardsEnabled) {
      ...
      return <Card className="section" title={__('Receive Credits validation pending')} body={<React.Fragment>
              <p>
                {__('This account must undergo review before you can participate in the credits program. Not all users and regions may qualify.')}{' '}
                {__('This can take anywhere from a few hours to several days. Please be patient.')}
              </p>

              <p>
                {__('We apologize for this inconvenience, but have added this additional step to prevent abuse. Users on VPN or shared connections will continue to see this message and are not eligible for Credits.')}
              </p>
              <p>
                <I18nMessage tokens={{
  ...
  renderCustomRewardCode() {
    ...
    return <RewardTile key={REWARD_TYPES.TYPE_GENERATED_CODE} reward={{
      reward_type: REWARD_TYPES.TYPE_GENERATED_CODE,
      reward_title: __('Custom Code'),
      reward_description: __('Are you a supermodel or rockstar that received a custom Credits code? Claim it here.')
  ...
  renderUnclaimedRewards() {
    ...
    if (!IS_WEB && daemonSettings && !daemonSettings.share_usage_data) {
      ...
      return <p className="help">{__('This application is unable to receive credits due to an authentication failure.')}</p>;
    } else if (!rewards || rewards.length <= 0) {
      return <Card title={__('No credits available')} subtitle={claimed && claimed.length ? __("You have claimed all available credits! We're regularly adding more so be sure to check back later.") : __('There are no credits available at this time, please check back later.')} actions={<Button button="primary" navigate={`/$/${PAGES.DISCOVER}`} label={__('Go Home')} />} />;
    }

    const isNotEligible = !user || !user.primary_email || !user.has_verified_email || !user.is_reward_approved;
    return <div aria-hidden={isNotEligible} className={classnames('card__list', {
      'card--disabled': isNotEligible
    })}>
        {rewards.map(reward => <RewardTile disabled={isNotEligible} key={reward.claim_code} reward={reward} />)}
