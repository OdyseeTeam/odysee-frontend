...
export default function ModalFeaturedChannelsSort(props: Props) {
  ...
  return <Modal isOpen type="custom" className="modalFCSort" onAborted={doHideModal}>
      <Card title={__('Sort Featured Channels')} body={<SortableList list={sectionIds} onGetElemAtIndex={(id, index) => <div key={id} className="modalFCSort__item">
                {getEntry(id)}
              </div>} onIsHiddenAtIndex={(id, index) => !isFcType(id)} onDragEnd={handleDragEnd} />} actions={<div className="section__actions">
