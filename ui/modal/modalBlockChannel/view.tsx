...
type Props = {
  ...
  contentClaimIsMine: boolean | null | undefined;
  ...
  doHideModal: () => void;
  doCommentModBlock: (commenterUri: string, offendingCommentId: string | null | undefined, timeoutSec: number | null | undefined) => void;
  doCommentModBlockAsAdmin: (commenterUri: string, offendingCommentId: string | null | undefined, blockerId: string | null | undefined, timeoutSec: number | null | undefined) => void;
  doCommentModBlockAsModerator: (commenterUri: string, offendingCommentId: string | null | undefined, creatorUri: string, blockerId: string | null | undefined, timeoutSec: number | null | undefined) => void;
};
export default function ModalBlockChannel(props: Props) {
  ...
  React.useEffect(() => {
    if (isPersonalTheOnlyTab || tab === TAB.MODERATOR && !activeChannelIsModerator || tab === TAB.ADMIN && !activeChannelIsAdmin) {
      setTab(TAB.PERSONAL);
    }
  ...
  function getTabElem(value, label) {
    return <Button key={value} label={__(label)} button="alt" onClick={() => setTab(value)} className={classnames('button-toggle', {
    ...
    switch (tab) {
      ...
      case TAB.MODERATOR:
        return <p className="help">
            {contentChannelClaim ? __('Block this channel on behalf of %creator%.', {
  ...
  function getBlockTypeElem(value, label, disabled = false, disabledLabel = '') {
    return <FormField type="radio" name={value} key={value} label={disabled && disabledLabel ? __(disabledLabel) : __(label)} disabled={disabled} checked={blockType === value} onChange={() => setBlockType(value)} />;
  }

  function getTimeoutDurationElem() {
    return <FormFieldDuration name="time_out" value={timeoutInput} onChange={e => setTimeoutInput(e.target.value)} onResolve={valueInSeconds => setTimeoutSec(valueInSeconds)} />;
  }
  ...
  function getActiveChannelElem() {
    return activeChannelClaim ? <div className="block-modal--active-channel">
    ...
    switch (tab) {
      ...
      case TAB.MODERATOR:
        if (activeChannelClaim && contentChannelClaim) {
          doCommentModBlockAsModerator(commenterUri, offendingCommentId, contentChannelClaim.permanent_url, activeChannelClaim.claim_id, duration);
        }
  ...
  */
  return <Modal isOpen type="card" onAborted={doHideModal}>
      <Card title={__('Block Channel')} subtitle={getCommenterPreview(commenterUri)} actions={<>
            {!isPersonalTheOnlyTab && <div className="section__actions">
            ...
            <div className="section section--vertical-compact">
              ...
              <div className="block-modal--values">
                <fieldset>
                  ...
                  {getBlockTypeElem(BLOCK.TIMEOUT, 'Timeout --[time-based ban instead of permanent]--', !isTimeoutAvail, 'Timeout (only available on content that you own)')}
                </fieldset>
