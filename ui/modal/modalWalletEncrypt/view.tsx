...
type Props = {
  ...
  encryptWallet: (arg0: string | null | undefined) => void;
...
class ModalWalletEncrypt extends React.PureComponent<Props, State> {
  ...
  onChangeNewPassword(event: React.SyntheticEvent) {
    this.setState({
      newPassword: event.target.value
    });
  }

  onChangeRememberPassword(event: React.SyntheticEvent) {
    this.setState({
      rememberPassword: event.target.checked
    });
  }

  onChangeNewPasswordConfirm(event: React.SyntheticEvent) {
    this.setState({
      newPasswordConfirm: event.target.value
    });
  }

  onChangeUnderstandConfirm(event: React.SyntheticEvent) {
  ...
  render() {
    ...
    return <Modal isOpen title={__('Encrypt wallet')} contentLabel={__('Encrypt wallet')} type="custom" onConfirmed={() => this.submitEncryptForm()} onAborted={closeModal}>
        <Form onSubmit={() => this.submitEncryptForm()}>
          ...
          <fieldset-section>
            <FormField autoFocus error={passwordMismatch === true ? 'Passwords do not match' : false} label={__('Password')} placeholder={__('Shh...')} type="password" name="wallet-new-password" onChange={event => this.onChangeNewPassword(event)} />
          </fieldset-section>
          <fieldset-section>
            <FormField error={passwordMismatch === true ? 'Passwords do not match' : false} label={__('Confirm Password')} placeholder={__('Your eyes only')} type="password" name="wallet-new-password-confirm" onChange={event => this.onChangeNewPasswordConfirm(event)} />
          </fieldset-section>
          <fieldset-section>
            <FormField label={__('Remember Password')} type="checkbox" name="wallet-remember-password" onChange={event => this.onChangeRememberPassword(event)} checked={this.state.rememberPassword} />
          </fieldset-section>

          <div className="help--warning">
            {__('If your password is lost, it cannot be recovered. You will not be able to access your wallet without a password.')}
          </div>
          <FormField inputButton={<Submit label={failMessage ? __('Encrypting Wallet') : __('Encrypt wallet')} />} error={understandError === true ? __('You must enter "%acknowledgement_text%"', {
          acknowledgement_text: acknowledgementText
        }) : false} label={__('Enter "%acknowledgement_text%"', {
          acknowledgement_text: acknowledgementText
        })} placeholder={__('Type "%acknowledgement_text%"', {
          acknowledgement_text: acknowledgementText
        })} type="text" name="wallet-understand" onChange={event => this.onChangeUnderstandConfirm(event)} />
