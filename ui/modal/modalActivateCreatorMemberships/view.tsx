...
export default function ModalRemoveCard(props: Props) {
  ...
  const activateMembershipsButton = <Button button="primary" icon={ICONS.MEMBERSHIP} label={__('Activate Memberships')} // onClick={deleteMembership}
  />;
  const addBankAccountButton = <Button button="primary" icon={ICONS.FINANCE} label={__('Add A Bank Account')} navigate={`$/${PAGES.SETTINGS_STRIPE_ACCOUNT}`} />;
  ...
  return <Modal ariaHideApp={false} isOpen type="card" onAborted={closeModal}>
      <Card className="stripe__confirm-remove-membership" title={__('Activate Memberships')} subtitle={!bankAccountConfirmed ? needToAddABankAccountText : activateYourMembershipsText} actions={<div className="section__actions" style={{
      marginTop: '10px'
    }}>
