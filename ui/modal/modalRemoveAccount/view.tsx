...
export default function ModalRemoveAccount(props: Props) {
  ...
  const showButton = !buttonClicked && (!isPendingDeletion || !isWalletEmpty) && isLoadingAccountInfoSuccess && !isLoadingAccountInfo;
  React.useEffect(() => {
  ...
  return <Modal isOpen type="custom" width="wide">
      <Card title={__('Delete account')} subtitle={<>
            ...
            {showButton && <FormField type="checkbox" name="forfeit_credits_checkbox" label={__('Remove all content from the account and forfeit credits left on the account to Odysee')} checked={isForfeitChecked} disabled={isBusy} onChange={() => setIsForfeitChecked(!isForfeitChecked)} />}
            {showButton && isWalletMerged && <div className="help--warning">
                <p>
            ...
            {showButton && hasYouTubeChannels && <div className="help--warning">
            ...
            <div className="section__actions">
              {isBusy || isLoadingAccountInfo ? <BusyIndicator message={isBusy ? __('Removing content...') : __('Loading account info...')} /> : showButton && <Button button="primary" label={isBusy ? <Spinner type="small" /> : !isPendingDeletion ? __('Remove content and send deletion request') : __('Remove content')} disabled={isBusy || !isForfeitChecked} onClick={handleOnClick} />}
