...
function ModalRemoveCollection(props: Props) {
  ...
  function getBody() {
    if (simplify) {
      ...
      return hasClaim ? <>
          <p>{__('This will permanently delete the list.')}</p>
          <p>{__('Type "%list_name%" to confirm.', {
            list_name: collectionName
          })}</p>
          <FormField value={confirmName} type={'text'} onChange={e => setConfirmName(e.target.value)} style={{
          marginBottom: 'var(--spacing-s)'
        }} />
          <FormField name="keep-private" type="checkbox" label={__('Delete publish but keep private playlist')} checked={keepPrivate} onChange={() => setKeepPrivate(prev => !prev)} />
        </> : <I18nMessage tokens={{
  ...
  return <Modal isOpen contentLabel={__('Confirm Playlist Unpublish')} type="card" onAborted={doHideModal}>
      <Card title={__('Delete Playlist')} body={<>
            {getBody()}
            <p className="help error__text">{__('This action is permanent and cannot be undone')}</p>
          </>} actions={<div className="section__actions">
            <Button button="primary" label={__('Delete')} disabled={!simplify && hasClaim && collectionName !== confirmName} onClick={() => {
