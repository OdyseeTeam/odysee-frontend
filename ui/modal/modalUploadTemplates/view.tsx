...
function getTemplatePreviewFields(templateData: UploadTemplateData): Array<{
  ...
  if (data.description) fields.push({
    label: __('Description'),
    value: String(data.description)
  });
  const tagsLabel = getTagsLabel(data.tags);
  if (tagsLabel) fields.push({
    label: __('Tags'),
    value: tagsLabel
  });
  ...
  if (data.visibility) fields.push({
    label: __('Visibility'),
    value: String(data.visibility)
  });
  if (data.nsfw === true) fields.push({
    label: __('Mature'),
    value: __('Yes')
  });
  if (data.licenseType) fields.push({
    label: __('License'),
...
function getTemplateSearchText(template: TemplateEntry): string {
  const previewFieldValues = getTemplatePreviewFields(template.data || {}).map(field => field.value).join(' ');
...
export default function ModalUploadTemplates(props: Props) {
  ...
  const sourceTemplatesByChannelId = React.useMemo(() => {
    ...
    orderedChannelClaims.forEach(channelClaim => {
      ...
      nextTemplatesByChannelId[channelId] = getUploadTemplatesFromSettings(settingsByChannelId && settingsByChannelId[channelId]);
    });
    return nextTemplatesByChannelId;
  }, [orderedChannelClaims, settingsByChannelId]);
  const [templatesByChannelId, setTemplatesByChannelId] = React.useState<Record<string, Array<UploadTemplate>>>(sourceTemplatesByChannelId);
  ...
  const sortedTemplates = React.useMemo(() => [...allTemplateEntries].sort((a, b) => {
  ...
  const changedChannelIds = React.useMemo(() => orderedChannelClaims.map(channelClaim => channelClaim.claim_id).filter(channelId => {
  ...
  const filteredTemplates = React.useMemo(() => normalizedSearchQuery ? sortedTemplates.filter(template => getTemplateSearchText(template).includes(normalizedSearchQuery)) : sortedTemplates, [sortedTemplates, normalizedSearchQuery]);
  React.useEffect(() => {
  ...
  function updateTemplatesForChannel(channelId: string, updater: (arg0: Array<UploadTemplate>) => Array<UploadTemplate>) {
    markEdited();
  ...
  function togglePreview(templateKey: string) {
    setExpandedPreviewByTemplateKey(prev => ({ ...prev,
  ...
  function handleDelete(template: TemplateEntry) {
    updateTemplatesForChannel(template.channelId, channelTemplates => channelTemplates.filter(existingTemplate => existingTemplate.id !== template.id));
  ...
  function handleTogglePin(template: TemplateEntry) {
    updateTemplatesForChannel(template.channelId, channelTemplates => channelTemplates.map(existingTemplate => existingTemplate.id === template.id ? { ...existingTemplate,
  ...
  function handleDuplicate(template: TemplateEntry) {
    ...
    const duplicateTemplate: UploadTemplate = { ...template,
      id: uuid(),
      name: createdName,
      createdAt: Date.now(),
  ...
  function handleConfirmRename(template: TemplateEntry) {
    ...
    const duplicateTemplate = (templatesByChannelId[template.channelId] || []).find(existingTemplate => existingTemplate.id !== template.id && existingTemplate.name.toLowerCase() === normalizedName.toLowerCase());
  ...
  return <Modal isOpen type="custom" width="wide" onAborted={doHideModal}>
      <Card title={__('Manage Upload Templates')} subtitle={__('Preview what each template fills, prefill instantly, or rename and delete templates.')} body={<div className="upload-templates-manage">
            {shouldShowSearch && <div className="upload-templates-manage__search">
                <FormField type="text" name="upload_template_search" value={searchQuery} autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck={false} placeholder={__('Search templates...')} onChange={e => setSearchQuery(e.target.value)} />
                {normalizedSearchQuery && <div className="upload-templates-manage__search-count">
                    {__('%count% results', {
            ...
            {sortedTemplates.length === 0 ? <div className="main--empty">{__('No templates saved yet.')}</div> : filteredTemplates.length === 0 ? <div className="main--empty">{__('No templates match your search.')}</div> : <div className="upload-templates-manage__list">
          ...
          return <div key={templateKey} className="upload-templates-manage__item">
                      {isEditing ? <div className="upload-templates-manage__edit-row">
                          <FormField type="text" name={editInputName} value={editName} autoComplete="new-password" autoCorrect="off" autoCapitalize="off" spellCheck={false} aria-autocomplete="none" data-lpignore="true" onChange={e => setEditName(e.target.value)} onKeyDown={e => {
                if (e.key === 'Enter') {
                  e.preventDefault();
                  e.stopPropagation();
                ...
                if (e.key === 'Escape') {
                  ...
                  closeEditor();
                          ...
                          <button type="button" className="upload-templates-manage__action-btn" onClick={() => handleConfirmRename(template)} disabled={!editName.trim()} title={__('Save name')}>
                            <Icon icon={ICONS.COMPLETE} />
                          </button>
                          <button type="button" className="upload-templates-manage__action-btn" onClick={closeEditor} title={__('Cancel')}>
                            ...
                            <div className="upload-templates-manage__info">
                              ...
                              <div className="upload-templates-manage__text">
                                <div className="upload-templates-manage__name-row">
                                  ...
                                  {template.isPinned && <span className="upload-templates-manage__pinned">
                                      <Icon icon={ICONS.PIN} />
                                      {__('Pinned')}
                                    </span>}
                                </div>
                                <div className="upload-templates-manage__meta">
                                  ...
                                  <span>{__('Created %date%', {
                          date: getDateLabel(template.createdAt)
                        })}</span>
                                  <span>
                                    {template.lastUsedAt ? __('Last used %date%', {
                          date: getDateLabel(template.lastUsedAt)
                        }) : __('Never used')}
                                  </span>
                                </div>
                                {!isPreviewExpanded && <div className="upload-templates-manage__preview-summary">
                                    {previewFields.length > 0 ? __('%count% fields ready to prefill', {
                        count: previewFields.length
                      }) : __('No fields set in this template.')}
                                  </div>}
                                {isPreviewExpanded && visiblePreviewFields.length > 0 ? <div className="upload-templates-manage__preview">
                                    {visiblePreviewFields.map(previewField => <span key={`${templateKey}:${previewField.label}`} className="upload-templates-manage__preview-item" title={`${previewField.label}: ${previewField.value}`}>
                                    ...
                                    {hiddenPreviewFieldCount > 0 && <span className="upload-templates-manage__preview-more">
                                        {__('+%count% more', {
                          count: hiddenPreviewFieldCount
                        })}
                                      </span>}
                                  </div> : isPreviewExpanded && <div className="upload-templates-manage__preview-empty">
                                      {__('No fields set in this template.')}
                                    </div>}
                              </div>
                            </div>
                          ...
                          <div className="upload-templates-manage__actions">
                            <button type="button" className="upload-templates-manage__text-btn" onClick={() => togglePreview(templateKey)}>
                              <Icon icon={isPreviewExpanded ? ICONS.UP : ICONS.DOWN} />
                              {isPreviewExpanded ? __('Hide fields') : __('View fields (%count%)', {
                    count: previewFields.length
                  })}
                            </button>
                            <button type="button" className="upload-templates-manage__text-btn upload-templates-manage__text-btn--prefill" onClick={() => handlePrefillNow(template)}>
                              <Icon icon={ICONS.COPY} />
                              {__('Prefill now')}
                            </button>
                            <button type="button" className="upload-templates-manage__text-btn" onClick={() => handleTogglePin(template)}>
                              <Icon icon={ICONS.PIN} />
                              {template.isPinned ? __('Unpin') : __('Pin')}
                            </button>
                            <button type="button" className="upload-templates-manage__text-btn" onClick={() => handleDuplicate(template)}>
                              <Icon icon={ICONS.COPY_LINK} />
                              {__('Duplicate')}
                            </button>
                            <button type="button" className="upload-templates-manage__text-btn" onClick={() => handleStartRename(template)}>
                              <Icon icon={ICONS.EDIT} />
                              {__('Edit name')}
                            </button>
                            <button type="button" className="upload-templates-manage__text-btn upload-templates-manage__text-btn--delete" onClick={() => handleDelete(template)}>
