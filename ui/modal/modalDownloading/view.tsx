...
export default function ModalDownloading() {
  ...
  const downloadComplete = useAppSelector(selectDownloadComplete);
  const downloadItem = useAppSelector(selectUpgradeDownloadPath);

  function startUpgrade() {
    dispatch(doStartUpgrade());
  }
    ...
    <Modal title={__('Downloading update')} isOpen contentLabel={__('Downloading update')} type="custom">
      ...
      {downloadComplete ? (
        <React.Fragment>
          ...
          <p>{__('To launch installation manually, close LBRY (CTRL-Q) and run the command below in the terminal.')}</p>
