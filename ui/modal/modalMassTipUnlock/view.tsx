...
export default function ModalSupportsLiquidate(props: Props) {
  ...
  return <Modal isOpen contentLabel={__('Unlock all tips')} type="card" confirmButtonLabel="done" onAborted={doHideModal}>
      <Card icon={ICONS.UNLOCK} title={__('Unlock all tips')} subtitle={<>
            <p>
              <I18nMessage tokens={{
          lbc: <LbcSymbol />
        }}>
          ...
          learn_more: <Button button="link" label={__('Learn More')} href="https://help.odysee.tv/category-blockchain/category-staking/increase/" />
            ...
            <div className="section__actions">
              <Button button="primary" onClick={() => doTipClaimMass()} disabled={massClaimingTips} label={massClaimingTips ? __('Working...') : __('Unlock All')} />
            </div>
            {supportCount > WALLET_CONSOLIDATE_UTXOS && <span className="help">{__('You have a lot of tips. This could take some time.')}</span>}
          </>} />
    </Modal>;
}