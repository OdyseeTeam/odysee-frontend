...
export default function PreorderAndPurchaseContentCard(props: Props) {
  ...
  const rentDuration = tags?.rentalTag?.expirationTimeInSeconds ? secondsToDhms(tags.rentalTag.expirationTimeInSeconds) : '';
  ...
  return <Form onSubmit={handleSubmit}>
      <Card title={__(STRINGS[transactionType].title)} className="fiat-order" actions={<div className="fiat-order__actions">
        ...
        /* confirm purchase - needs to check balance and disable */
            ...
            {!canReceiveTips ? <div className="monetization-disabled">
                USD Monetization isn't available. It may not be set up yet or has been disabled by the creator.
              </div> : activeArStatus !== 'connected' ? <WalletStatus /> : null}
            {waitingForBackend ? <BusyIndicator message={__('Processing order...')} /> : <SubmitArea handleSubmit={handleSubmit} pendingSdkPayment={pendingSdkPayment} costInfo={costInfo} label={STRINGS[transactionType].button} fiatSymbol={fiatSymbol} tipAmount={tipAmount} tags={tags} rentLabel={STRINGS['rental'].button} rentTipAmount={rentTipAmount} rentDuration={rentDuration} disabled={activeArStatus !== 'connected' || !canReceiveTips} rentDisabled={cantAffordRent} purchaseDisabled={cantAffordPurchase} preorderDisabled={cantAffordPreorder} uri={uri} setWaitingForBackend={setWaitingForBackend} doPlayUri={doPlayUri} doHideModal={doHideModal} />}
            <p className="help">
              <I18nMessage tokens={{
...
const SubmitArea = (props: any) => <div className="handle-submit-area">
    <Button button="primary" onClick={() => props.handleSubmit()} label={__(props.label, {
    currency: props.fiatSymbol,
    amount: props.tipAmount.toString(),
    duration: props.rentDuration
  })} icon={props.tags.rentalTag ? ICONS.BUY : ICONS.TIME} disabled={props.disabled || props.purchaseDisabled} />

    {props.tags.purchaseTag && props.tags.rentalTag && <Button button="primary" onClick={() => props.handleSubmit('rent')} label={__(props.rentLabel, {
    currency: props.fiatSymbol,
    amount: props.rentTipAmount.toString(),
    duration: props.rentDuration
  })} icon={ICONS.TIME} disabled={props.disabled || props.rentDisabled} />}

    {props.pendingSdkPayment && <Button button="primary" requiresAuth onClick={() => {
    props.setWaitingForBackend(true);
    props.doPlayUri(props.uri, true, undefined, props.doHideModal);
  }} label={<I18nMessage tokens={{
    currency: <Icon icon={ICONS.LBC} />,
    amount: props.costInfo?.cost || '?'
  }}>
            Purchase for %currency%%amount%
          </I18nMessage>} icon={ICONS.BUY} />}
  </div>;