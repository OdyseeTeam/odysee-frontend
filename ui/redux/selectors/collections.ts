...
import { selectClaimForUri, selectClaimForClaimId, selectClaimIsPendingForId, selectClaimsById, selectClaimIdsByUri, selectMyCollectionClaimIds, selectResolvedCollectionsById, selectMyCollectionClaimsById, selectClaimIsMineForId } from "redux/selectors/claims";
import { normalizeURI, parseURI } from "util/lbryURI";
import { createCachedSelector } from "re-reselect";
import { selectUserCreationDate } from "redux/selectors/user";
import { selectIsCollectionPlayingForId, selectCollectionForIdIsPlayingShuffle, selectCollectionForIdIsPlayingLoop, selectCanPlaybackFileForUri } from "redux/selectors/content";
...
const selectRouterSearchString = (state: State) => state.router && state.router.location && state.router.location.search || '';
...
export const selectThumbnailClaimsFetchingCollectionIds = (state: State) => selectState(state).thumbnailClaimsFetchingCollectionIds;
...
export const selectCollectionAutoPublishForId = (state: State, id: string) => Boolean(selectCollectionAutoPublishMap(state)[id]);
export const selectCollectionAutoPublishScheduledAtMap = (state: State) => selectState(state).autoPublishScheduledAtById || {};
export const selectCollectionAutoPublishScheduledAtForId = (state: State, id: string) => selectCollectionAutoPublishScheduledAtMap(state)[id] || null;
export const selectCollectionIsPublishingForId = (state: State, id: string) => Boolean(selectCollectionPublishingMap(state)[id]);
export const selectCollectionPublishErrorForId = (state: State, id: string) => selectCollectionPublishErrorMap(state)[id];
export const selectAreThumbnailClaimsFetchingForCollectionIds = (state: State, ids: string) => selectThumbnailClaimsFetchingCollectionIds(state).includes(ids);
const selectCollectionIdsWithItemsResolvedSet = createSelector(selectCollectionIdsWithItemsResolved, resolvedIds => new Set(resolvedIds || []));
export const selectCollectionHasItemsResolvedForId = createCachedSelector(selectCollectionIdsWithItemsResolvedSet, (state: State, id: string) => id, (resolvedIdsSet, id) => resolvedIdsSet.has(id))((state: State, id: string) => String(id));
export const selectUnpublishedCollectionsList = createSelector(selectMyUnpublishedCollections, unpublishedCollections => Object.keys(unpublishedCollections));
export const selectCollectionSavedForId = (state: State, id: string) => new Set(selectSavedCollectionIds(state)).has(id);
export const selectSavedCollections = createSelector(selectResolvedCollectionsById, selectSavedCollectionIds, (resolvedCollectionsById, savedIds) => {
...
export const selectHasLocalSyncCollections = createSelector(selectMyUnpublishedCollections, selectMyEditedCollections, selectSavedCollections, (unpublished, edited, saved) => {
...
export const selectCollectionHasEditsForId = (state: State, id: string) => Boolean(selectEditedCollectionForId(state, id));
...
export const selectCollectionHasUnsavedEditsForId = (state: State, id: string) => Boolean(selectUnsavedChangesCollectionForId(state, id));
...
export const selectMyPublishedCollections = createSelector(selectMyCollectionClaimsById, selectMyEditedCollections, selectMyUpdatedCollections, (myCollections, edited, updated) => {
  ...
  Object.entries(edited).forEach(([id, item]) => {
    ...
    if (!updated[id]) {
      ...
      // $FlowFixMe
      myPublishedCollections[id] = { ...myPublishedCollections[id],
...
export const selectMyPublicLocalCollections = createSelector(selectMyPublishedCollections, selectMyEditedCollections, selectMyUpdatedCollections, (myCollectionsById, edited, updated) => {
...
export const selectIsMyCollectionPublishedForId = (state: State, id: string) => Boolean(selectMyPublicCollectionForId(state, id));
export const selectPublishedCollectionNotEditedForId = createSelector(selectIsMyCollectionPublishedForId, selectCollectionHasEditsForId, (isPublished, hasEdits) => isPublished && !hasEdits);
...
export const selectAreCollectionItemsFetchingForId = (state: State, id: string) => new Set(selectCollectionItemsFetchingIds(state)).has(id);
...
export const selectCollectionForId = createSelector((state, id) => id, selectCollectionsById, selectResolvedCollectionsById, selectRouterSearchString, (id, collectionsById, resolved, search) => {
  if (!id) return id;
...
export const selectClaimSavedForUrl = createSelector((state, url) => url, selectBuiltinCollections, selectMyPublicLocalCollections, selectMyUnpublishedCollections, selectMyEditedCollections, (url, bLists, myRLists, uLists, eLists) => {
  const collections = [bLists, uLists, eLists, myRLists];
  // $FlowFixMe
  const claimId = url.match(/[a-f0-9]{40}/)?.[0];
  return collections.some(list => // $FlowFixMe
  Object.values(list).some(({
    items
  }) => items?.some(item => item === url || item === claimId)));
});
...
export const makeSelectClaimMenuCollectionsForUrl = () => createSelector([selectLastUsedCollectionIds, selectCollectionsById, selectBuiltinCollections, selectMyUnpublishedCollections, selectMyEditedCollections, selectMyCollectionClaimIds, (state, url) => url], (lastUsedCollectionIds, collectionsById, builtinCollections, unpublishedCollections, editedCollections, myPublishedIds, url) => {
  // $FlowFixMe
  const claimId = url.match(/[a-f0-9]{40}/)?.[0];
  ...
  const lastUsedCollections = lastUsedCollectionIds.filter(id => myCollectionIds.has(id)) // Only include my collections
  .map(id => {
    const collection = collectionsById[id];
  ...
  const collectionsContainingClaim = (Object.entries(collectionsById) as Array<[string, any]>).filter(([id, collection]) => myCollectionIds.has(id) && // Only include my collections
  // $FlowFixMe
  collection.items?.some(item => item === url || item === claimId) && !lastUsedCollections.some(lastUsedCollection => lastUsedCollection.id === collection.id)).map(([id, collection]) => ({ ...collection,
    hasClaim: true
  }));
  const claimMenuCollections = lastUsedCollections.concat(collectionsContainingClaim);
  return claimMenuCollections;
});
export const selectCollectionForIdHasClaimUrl = (state: State, id: string, uri: string) => Boolean(selectCollectionForIdClaimForUriItem(state, id, uri));
...
export const selectUpdatedAtForCollectionId = createSelector(selectCollectionForId, selectUserCreationDate, selectUpdatedCollectionForId, (collection, userCreatedAt, updated) => {
  const isBuiltin = COLLECTIONS_CONSTS.BUILTIN_PLAYLISTS.includes(collection?.id);
  const collectionUpdatedAt = toMilliseconds(updated?.updatedAt || collection?.updatedAt || collection?.createdAt || 0);
...
export const selectCreatedAtForCollectionId = createSelector(selectCollectionForId, (state: State, id: string) => id, selectUserCreationDate, (collection, id, userCreatedAt) => {
...
export const selectCountForCollectionId = (state: State, id: string) => getItemCountForCollection(selectCollectionForId(state, id));
...
export const selectIsCollectionPrivateForId = (state: State, id: string) => Boolean(selectUnpublishedCollectionForId(state, id) || COLLECTIONS_CONSTS.BUILTIN_PLAYLISTS.includes(id));
...
export const selectClaimIdsForCollectionId = createSelector(selectHasPrivateCollectionForId, selectItemsForCollectionId, selectClaimIdsByUri, (isPrivate, items, byUri) => {
...
export const selectHasUnavailableClaimIdsForCollectionId = createSelector(selectClaimIdsForCollectionId, claimIds => claimIds && claimIds.includes(null));
export const selectUrlsForCollectionId = createCachedSelector((state, collectionId, itemCount) => itemCount, selectItemsForCollectionId, selectClaimsById, (itemCount, items, claimsById) => {
...
export const selectFirstPlayableUrlForCollectionId = createCachedSelector(selectUrlsForCollectionId, state => state, (uris, state) => uris ? uris.find(uri => selectCanPlaybackFileForUri(state, uri)) : undefined)((state, collectionId) => String(collectionId));
export const selectUrlsForCollectionIdNonDeleted = createCachedSelector(selectUrlsForCollectionId, (state, collectionId) => collectionId, state => state, (uris, collectionId, state) => {
  if (!uris) return uris;
...
export const selectCountForCollectionIdNonDeleted = createCachedSelector(selectUrlsForCollectionIdNonDeleted, uris => uris ? uris.length : uris)((state, collectionId) => String(collectionId));
export const selectCollectionForIdClaimForUriItem = createSelector((state: State, id: string, uri: string) => uri, (state: State, id: string, uri: string) => selectClaimForUri(state, uri), selectUrlsForCollectionId, selectItemsForCollectionId, (uri, claim, collectionUrls, collectionItems) => {
...
export const selectIndexForUrlInCollectionForId = createSelector(selectCollectionForIdClaimForUriItem, selectUrlsForCollectionId, (uriItem, collectionUrls) => {
  const index = collectionUrls && collectionUrls.findIndex(uri => uri === uriItem);
  if (index > -1) return index;
  return null;
});
export const selectIndexForUriInPlayingCollectionForId = createSelector(selectCollectionForIdClaimForUriItem, selectUrlsForCollectionId, selectCollectionForIdIsPlayingShuffle, (uriItem, collectionUrls, playingCollectionShuffleUrls) => {
...
export const selectPreviousUriForUriInPlayingCollectionForId = createCachedSelector(selectUrlsForCollectionId, selectIndexForUriInPlayingCollectionForId, selectCollectionForIdIsPlayingShuffle, selectCollectionForIdIsPlayingLoop, (collectionUrls, currentIndex, playingCollectionShuffleUrls, isLooped) => {
...
export const selectNextUriForUriInPlayingCollectionForId = createCachedSelector(selectUrlsForCollectionId, selectIndexForUriInPlayingCollectionForId, selectCollectionForIdIsPlayingShuffle, selectCollectionForIdIsPlayingLoop, (collectionUrls, currentIndex, playingCollectionShuffleUrls, isLooped) => {
  if (currentIndex === null) return null;
  const uris = playingCollectionShuffleUrls || collectionUrls;
