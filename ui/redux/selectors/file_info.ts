...
import { selectClaimOutpointForUri, selectIsFetchingClaimListMine, selectMyClaims, makeSelectContentTypeForUri, makeSelectClaimForUri, selectIsStreamPlaceholderForUri, selectClaimForUri } from "redux/selectors/claims";
...
export const selectIsFetchingFileListDownloadedOrPublished = createSelector(selectIsFetchingFileList, selectIsFetchingClaimListMine, (isFetchingFileList, isFetchingClaimListMine) => isFetchingFileList || isFetchingClaimListMine);
...
export const makeSelectDownloadingForUri = uri => createSelector(selectDownloadingByOutpoint, makeSelectFileInfoForUri(uri), (byOutpoint, fileInfo) => {
...
export const makeSelectLoadingForUri = uri => createSelector(selectUrisLoading, makeSelectClaimForUri(uri), (fetchingByOutpoint, claim) => {
  if (!claim) {
    return false;
  }
...
export const selectFileInfosDownloaded = createSelector(selectFileInfosByOutpoint, selectMyClaims, (byOutpoint, myClaims) => Object.values(byOutpoint).filter(fileInfo => {
  const myClaimIds = myClaims.map(claim => claim.claim_id);
  return fileInfo && myClaimIds.indexOf(fileInfo.claim_id) === -1 && (fileInfo.completed || fileInfo.written_bytes);
}));
export const selectDownloadingFileInfos = createSelector(selectDownloadingByOutpoint, selectFileInfosByOutpoint, (downloadingByOutpoint, fileInfosByOutpoint) => {
...
export const selectDownloadedUris = createSelector(selectFileInfosDownloaded, // We should use permament_url but it doesn't exist in file_list
info => info.slice().map(claim => `lbry://${claim.claim_name}#${claim.claim_id}`));
export const makeSelectMediaTypeForUri = uri => createSelector(makeSelectFileInfoForUri(uri), makeSelectContentTypeForUri(uri), (fileInfo, contentType) => {
...
export const makeSelectUriIsStreamable = uri => createSelector(makeSelectMediaTypeForUri(uri), mediaType => {
  const isStreamable = ['audio', 'video', 'image'].indexOf(mediaType) !== -1;
  return isStreamable;
});
export const makeSelectDownloadPathForUri = uri => createSelector(makeSelectFileInfoForUri(uri), fileInfo => {
  return fileInfo && fileInfo.download_path;
});
export const makeSelectFilePartlyDownloaded = uri => createSelector(makeSelectFileInfoForUri(uri), fileInfo => {
  if (!fileInfo) {
    return false;
  }
...
export const makeSelectFileNameForUri = uri => createSelector(makeSelectFileInfoForUri(uri), fileInfo => {
...
export const makeSelectSearchDownloadUrlsForPage = (query, page = 1) => createSelector(selectFileInfosDownloaded, fileInfos => {
...
export const makeSelectSearchDownloadUrlsCount = query => createSelector(selectFileInfosDownloaded, fileInfos => {
