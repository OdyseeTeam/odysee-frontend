...
import { selectClaimsById, selectMyClaimsWithoutChannels, selectResolvingUris, selectClaimsByUri, selectCollectionClaimPublishUpdateMetadataForId } from "redux/selectors/claims";
import { WEB_PUBLISH_SIZE_LIMIT_GB } from "config";
import { CHANNEL_ANONYMOUS } from "constants/claim";
import { SCHEDULED_LIVESTREAM_TAG } from "constants/tags";
import { selectCollectionForId, selectClaimIdsForCollectionId, selectIsCollectionPrivateForId, selectCollectionHasEditsForId, selectCollectionHasUnsavedEditsForId, selectCollectionTitleForId } from "redux/selectors/collections";
...
export const selectPublishFormValues = createSelector(selectState, state => state.settings, selectIsStillEditing, (publishState, settingsState, isStillEditing) => {
...
export const selectMyClaimForUri = createCachedSelector(selectPublishFormValues, selectIsStillEditing, selectClaimsById, selectMyClaimsWithoutChannels, (state, caseSensitive) => caseSensitive, ({
  editingURI,
  uri
}, isStillEditing, claimsById, myClaims, caseSensitive = true) => {
  let {
    channelName: contentName,
    streamName: claimName
  } = parseURI(uri);
  const {
    streamClaimId: editClaimId
  } = parseURI(editingURI);
  ...
  if (isStillEditing) {
    ...
    if (caseSensitive) {
      return myClaims.find(claim => !contentName ? claim.name === claimName : claim.name === contentName || claim.name === claimName);
    } else {
...
export const selectIsResolvingPublishUris = createSelector(selectState, selectResolvingUris, ({
  uri,
  name
}, resolvingUris) => {
  if (uri) {
...
export const selectTakeOverAmount = createSelector(selectState, selectMyClaimForUri, selectClaimsByUri, ({
  name
}, myClaimForUri, claimsByUri) => {
  if (!name) {
    return null;
  }
 ...
 * @return ?Array<number>
 */
export const selectValidTierIdsForCurrentForm = createSelector((state: State) => state.publish.type, (state: State) => state.publish.liveCreateType, (state: State) => state.publish.liveEditType, (state: State) => state.publish.channelId, selectIncognito, selectMembershipsListByCreatorId, (type, liveCreateType, liveEditType, channelId, incognito, tiersByCreatorId) => {
...
// ****************************************************************************
export const selectMemberRestrictionStatus = createSelector((state: State) => state.publish.memberRestrictionOn, (state: State) => state.publish.memberRestrictionTierIds, (state: State) => state.publish.visibility, (state: State) => state.publish.channelId, selectIncognito, selectMembershipsListByCreatorId, selectValidTierIdsForCurrentForm, (memberRestrictionOn, memberRestrictionTierIds, visibility, channelId, incognito, tiersByCreatorId, validTierIds) => {
...
export const selectUploadCount = createSelector(selectCurrentUploads, currentUploads => currentUploads && Object.keys(currentUploads).length);
// ****************************************************************************
// ****************************************************************************
export const selectIsScheduled = (state: State) => selectState(state).tags.some(t => t.name === SCHEDULED_LIVESTREAM_TAG);
export const selectCollectionClaimUploadParamsForId = (state: State, collectionId: string) => {
  ...
  if (isPrivate) {
    const collectionPublishCreateParams: CollectionPublishCreateParams = { ...privateCollectionParams,
      bid: 0.0001,
