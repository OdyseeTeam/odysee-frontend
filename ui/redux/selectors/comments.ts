...
import { selectClaimsById, selectMyClaimIdsRaw, selectMyChannelClaimIds, selectClaimIdForUri, selectClaimIdsByUri } from "redux/selectors/claims";
...
export const selectPinnedCommentsForUri = createCachedSelector(selectClaimIdForUri, (state: State) => state.comments.commentById, (state: State) => state.comments.pinnedCommentsById, (claimId, byId, pinnedCommentsById) => {
...
export const selectModerationBlockList = createSelector(state => selectState(state).moderationBlockList, moderationBlockList => {
  return moderationBlockList ? moderationBlockList.reverse() : [];
});
export const selectAdminBlockList = createSelector(selectState, state => state.adminBlockList ? state.adminBlockList.reverse() : []);
export const selectModeratorBlockList = createSelector(selectState, state => state.moderatorBlockList ? state.moderatorBlockList.reverse() : []);
...
export const selectModeratorBlockListDelegatorsMap = (state: State) => selectState(state).moderatorBlockListDelegatorsMap;
...
export const selectHasAdminChannel = createSelector(selectState, state => {
  ...
  return false; /// Lint doesn't like this:
...
export const selectTopLevelCommentsByClaimId = createSelector(state => selectState(state).topLevelCommentsById, selectCommentsById, (topLevelCommentsById, byId) => {
...
export const selectCommentForCommentId = createSelector((state, commentId) => commentId, selectCommentsById, (commentId, comments) => comments[commentId]);
export const selectFetchedCommentAncestors = (state: State) => selectState(state).fetchedCommentAncestors;
export const selectCommentAncestorsForId = createSelector((state, commentId) => commentId, selectFetchedCommentAncestors, (commentId, fetchedAncestors) => fetchedAncestors && fetchedAncestors[commentId]);
...
export const selectCommentsDisabledSettingForChannelId = (state: State, channelId: ClaimId) => selectCommentsEnabledSettingForChannelId(state, channelId) === false;
 ...
 */
export const selectFeaturedChannelsForChannelId = createCachedSelector(selectSettingsForChannelId, channelSettings => {
...
export const selectCommentsForUri = createCachedSelector((state, uri) => uri, selectCommentsByClaimId, selectClaimIdForUri, ...Object.values(filterCommentsDepOnList), (uri, byClaimId, claimId, ...filterInputs) => {
  const comments = byClaimId && byClaimId[claimId];
  return filterComments(comments, claimId, filterInputs);
})((state, uri) => String(uri));
export const selectTopLevelCommentsForUri = createCachedSelector((state, uri) => uri, (state, uri, maxCount) => maxCount, selectTopLevelCommentsByClaimId, selectClaimIdForUri, ...Object.values(filterCommentsDepOnList), (uri, maxCount = -1, byClaimId, claimId, ...filterInputs) => {
  const comments = byClaimId && byClaimId[claimId];
...
export const selectRepliesForParentId = createCachedSelector((state, id) => id, state => selectState(state).repliesByParentId, selectCommentsById, ...Object.values(filterCommentsDepOnList), (id, repliesByParentId, commentsById, ...filterInputs) => {
  ...
  return comments ? comments.filter(comment => {
    ...
    if (channelClaim) {
      if (myClaimIds && myClaimIds.size > 0 || myChannelClaimIds && myChannelClaimIds.length > 0) {
        const claimIsMine = channelClaim.is_my_output || myChannelClaimIds.includes(channelClaim.claim_id) || myClaimIds.includes(channelClaim.claim_id);
    ...
    if (channelClaim) {
      const geoRestriction: GeoRestriction | null | undefined = getGeoRestrictionForClaim(channelClaim, locale, geoBlockList);
...
export const makeSelectTotalReplyPagesForParentId = (parentId: string) => createSelector(selectState, state => {
...
// Personal list
export const makeSelectChannelIsBlocked = (uri: string) => createSelector(selectModerationBlockList, blockedChannelUris => {
...
export const makeSelectChannelIsAdminBlocked = (uri: string) => createSelector(selectAdminBlockList, list => {
  return list ? list.includes(uri) : false;
});
export const makeSelectChannelIsModeratorBlockedForCreator = (uri: string, creatorUri: string) => createSelector(selectModeratorBlockList, selectModeratorBlockListDelegatorsMap, (blockList, delegatorsMap) => {
  if (!blockList) return false;
  return blockList.includes(uri) && delegatorsMap[uri] && delegatorsMap[uri].includes(creatorUri);
});
export const makeSelectIsTogglingForDelegator = (uri: string, creatorUri: string) => createSelector(selectTogglingForDelegatorMap, togglingForDelegatorMap => {
  return togglingForDelegatorMap[uri] && togglingForDelegatorMap[uri].includes(creatorUri);
});
export const makeSelectUriIsBlockingOrUnBlocking = (uri: string) => createSelector(selectBlockingByUri, selectUnBlockingByUri, (blockingByUri, unBlockingByUri) => {
...
export const selectChannelMentionData = createCachedSelector((state, uri) => uri, selectClaimIdsByUri, selectClaimsById, selectTopLevelCommentsForUri, selectSubscriptionUris, selectMentionSearchResults, selectMentionQuery, (uri, claimIdsByUri, claimsById, topLevelComments, subscriptionUris, searchUris, query) => {
