...
export const selectActiveChannelClaim = createSelector(state => selectClaimForId(state, selectActiveChannelId(state)), // i.e. 'byId[activeChannelId]' specifically, instead of just 'byId'.
state => selectUserEmail(state), selectDefaultChannelClaim, selectMyChannelClaims, (activeChannelClaim, userEmail, defaultChannel, myChannelClaims) => {
...
export const selectShouldShowAds = state => {
  return SHOW_ADS && selectAdBlockerFound(state) === false && hasLegacyOdyseePremium(state) === false && // here
