...
export const makeSelectChannelIsMuted = (uri: string) => createSelector(selectMutedChannels, (state: Array<string>) => {
  return state.includes(uri);
});
export const selectMutedAndBlockedChannelIds = createSelector(state => state.blocked.blockedChannels, state => state.comments.moderationBlockList, (mutedUris, blockedUris) => {
