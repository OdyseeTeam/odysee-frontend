...
export const makeSelectPendingAmountByUri = uri => createSelector(selectClaimIdsByUri, selectPendingSupportTransactions, (claimIdsByUri, pendingSupports) => {
...
export const selectTransactionItems = createSelector(selectTransactionsById, byId => {
  ...
  Object.keys(byId).forEach(txid => {
    ...
    append.push(...tx.update_info.map(item => Object.assign({}, tx, item, {
      type: TRANSACTIONS.UPDATE
    })));
    append.push(...tx.abandon_info.map(item => Object.assign({}, tx, item, {
      type: TRANSACTIONS.ABANDON
    })));
...
export const selectHasTransactions = createSelector(selectTransactionItems, transactions => transactions && transactions.length > 0);
...
export const selectFilteredTransactions = createSelector(selectTransactionItems, selectTransactionListFilter, (transactions, filter) => {
  return transactions.filter(transaction => {
...
export const selectTxoItemCount = createSelector(selectState, state => state.txoPage && state.txoPage.total_items || 1);
...
export const makeSelectFilteredTransactionsForPage = (page = 1) => createSelector(selectFilteredTransactions, filteredTransactions => {
...
export const selectFilteredTransactionCount = createSelector(selectFilteredTransactions, filteredTransactions => filteredTransactions.length);
