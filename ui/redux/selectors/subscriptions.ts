...
import { selectAllFetchingChannelClaims, makeSelectChannelForClaimUri, makeSelectClaimForUri, selectClaimForUri, selectClaimForClaimId } from "redux/selectors/claims";
...
export const selectSubscriptions = createSelector(selectState, state => state.subscriptions && state.subscriptions.sort((a, b) => a.channelName.localeCompare(b.channelName)));
export const selectSubscriptionUris = createSelector(selectSubscriptions, subscriptions => subscriptions && subscriptions.map(sub => sub.uri));
...
export const selectSuggestedChannels = createSelector(selectSubscriptions, selectSuggested, (userSubscriptions, suggested) => {
  if (!suggested) {
    return null;
  }
...
export const selectSubscriptionsBeingFetched = createSelector(selectSubscriptions, selectAllFetchingChannelClaims, (subscriptions, fetchingChannelClaims) => {
...
export const makeSelectChannelInSubscriptions = uri => createSelector(selectSubscriptions, subscriptions => subscriptions.some(sub => sub.uri === uri));
export const selectIsSubscribedForUri = createCachedSelector((state, uri) => uri, selectClaimForUri, selectSubscriptions, (uri, claim, subscriptions) => {
 ...
 */
export const selectIsSubscribedForClaimId = createCachedSelector(selectClaimForClaimId, // (state, claimId)
...
export const makeSelectNotificationsDisabled = uri => createSelector(selectFollowing, makeSelectChannelForClaimUri(uri, true), makeSelectClaimForUri(uri), (following, channelUri, claim) => {
  const followedChannels = Array.isArray(following) ? following : [];
