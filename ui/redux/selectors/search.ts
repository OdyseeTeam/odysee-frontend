...
import { selectClaimsByUri, selectClaimForClaimId, selectClaimForUri, makeSelectClaimForUri, makeSelectClaimForClaimId, selectClaimIsNsfwForUri, makeSelectPendingClaimForUri, selectIsUriResolving } from "redux/selectors/claims";
...
export const selectHasReachedMaxResultsLength: (state: State) => { [key in boolean]?: Array<boolean> } = state => selectState(state).hasReachedMaxResultsLength;
...
export const makeSelectSearchUrisForQuery = (query: string): (state: State) => Array<string> => createSelector(selectSearchResultByQuery, byQuery => {
  if (!query) return;
...
export const makeSelectHasReachedMaxResultsLength = (query: string): (state: State) => boolean => createSelector(selectHasReachedMaxResultsLength, hasReachedMaxResultsLength => {
  if (query) {
    query = query.replace(/^lbry:\/\//i, '').replace(/\//, ' ');
    const normalizedQuery = createNormalizedSearchKey(query);
 ...
 */
export const selectRecommendedContentRawForUri = createCachedSelector(selectClaimForUri, selectShowMatureContent, selectClaimIsNsfwForUri, // (state, uri)
selectSearchResultByQuery, selectLanguage, state => selectClientSetting(state, SETTINGS.SEARCH_IN_LANGUAGE), (claim, matureEnabled, isMature, searchUrisByQuery, languageSetting, searchInLanguage) => {
 ...
 */
const selectRecClaimsByIdForUri = createSelector(selectClaimsByUri, selectRecommendedContentRawForUri, // (state, uri)
 ...
 */
const selectRecommendedContentFilteredForUri = createCachedSelector(selectClaimForUri, selectRecommendedContentRawForUri, selectRecClaimsByIdForUri, selectMutedChannels, (claim, recommendationsRaw, recClaimsByUri, blockedChannels) => {
 ...
 */
export const selectRecommendedContentForUri = createCachedSelector(selectClaimForUri, selectHistory, selectRecommendedContentFilteredForUri, selectRecClaimsByIdForUri, state => state.claims.costInfosById, (claim, history, filteredRecUris, recClaimsByUri, costInfosById) => {
...
export const selectRecommendedMetaForClaimId = createCachedSelector(selectClaimForClaimId, selectShowMatureContent, selectSearchResultByQuery, selectLanguage, state => selectClientSetting(state, SETTINGS.SEARCH_IN_LANGUAGE), (claim, matureEnabled, searchUrisByQuery, languageSetting, searchInLanguage) => {
...
export const makeSelectWinningUriForQuery = (query: string) => {
  ...
  return createSelector(selectShowMatureContent, makeSelectPendingClaimForUri(uriFromQuery), makeSelectClaimForUri(uriFromQuery), makeSelectClaimForUri(channelUriFromQuery), (matureEnabled, pendingClaim, claim1, claim2) => {
    ...
    if (!claim1 && !claim2) {
      ...
      return matureEnabled ? claim1.repost_url || claim1.canonical_url : claim1Mature ? undefined : claim1.repost_url || claim1.canonical_url;
    }
    ...
    const returnBeforePending = Number(effectiveAmount1) > Number(effectiveAmount2) ? claim1.repost_url || claim1.canonical_url : claim2.canonical_url;
...
export const makeSelectUrlForClaimId = (claimId: string) => createSelector(makeSelectClaimForClaimId(claimId), claim => claim ? claim.canonical_url || claim.permanent_url : null);