...
import {
  ...
  selectResolvingUris,
  selectClaimsByUri,
  selectGenericClaimUploadMetadataForId,
...
export const selectCollectionClaimUploadParamsForId = (state: State, collectionId: string) => {
  ...
  if (activeChannelId === undefined) return undefined;

  if (isPrivate) {
    const collectionPublishParams: CollectionPublishParams = {
      bid: 0.0001,
      channel_id: activeChannelId,
      name: sanitizeName(collection.title || collection.name),
  ...
  const collectionClaimMetadata = selectGenericClaimUploadMetadataForId(state, collectionId);
  if (collectionClaimMetadata === undefined) return undefined;
