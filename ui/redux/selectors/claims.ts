...
import { isClaimNsfw, isClaimUnlisted, getClaimScheduledState, filterClaims, getChannelIdFromClaim, isStreamPlaceholderClaim, getThumbnailFromClaim, getClaimRepostedAmount, getNameFromClaim, getChannelFromClaim, getChannelTitleFromClaim, getChannelNameFromClaim, getChannelPermanentUrlFromClaim } from "util/claim";
...
export const selectClaimsById = createSelector(selectById, selectPendingClaimsById, (byId, pendingById) => Object.assign({}, byId, pendingById));
...
export const selectMyCollectionClaimsById = createSelector(selectResolvedCollectionsById, selectMyCollectionClaimIds, (resolvedCollectionsById, myCollectionClaimIds) => {
...
export const selectLatestClaimForUri = createSelector((state, uri) => uri, selectLatestByUri, (uri, latestByUri) => {
...
export const selectChannelPermanentUriForUri = (state: State, uri: string) => getChannelPermanentUrlFromClaim(selectClaimForUri(state, uri));
...
export const makeSelectClaimIsPending = (uri: string) => createSelector(selectClaimIdsByUri, selectPendingClaimsById, (idsByUri, pendingById) => {
...
export const makeSelectClaimIdIsPending = (claimId: string) => createSelector(selectPendingClaimsById, pendingById => {
  return Boolean(pendingById[claimId]);
});
...
export const selectClaimForUri = createCachedSelector(selectClaimIdsByUri, selectClaimsById, (state, uri) => uri, (state, uri, returnRepost = true) => returnRepost, (byUri, byId, uri, returnRepost) => {
...
export const selectChannelClaimIdForUri = (state: State, uri: string) => getChannelIdFromClaim(selectClaimForUri(state, uri));
export const selectChannelNameForUri = (state: State, uri: string) => getChannelNameFromClaim(selectClaimForUri(state, uri));
...
export const makeSelectClaimForUri = (uri: string, returnRepost: boolean = true) => createSelector(selectClaimIdsByUri, selectClaimsById, (byUri, byId) => {
  const validUri = isURIValid(uri, false);
...
export const selectAbandoningIds = createSelector(selectAbandoningById, abandoningById => Object.keys(abandoningById));
export const makeSelectAbandoningClaimById = (claimId: string) => createSelector(selectAbandoningIds, ids => ids.includes(claimId));
export const makeSelectIsAbandoningClaimForUri = (uri: string) => createSelector(selectClaimIdsByUri, selectAbandoningIds, (claimIdsByUri, abandoningById) => {
  const claimId = claimIdsByUri[normalizeURI(uri)];
  return abandoningById.indexOf(claimId) >= 0;
});
export const selectMyActiveClaims = createSelector(selectMyClaimIdsRaw, selectAbandoningIds, (myClaimIds, abandoningIds) => {
...
const selectNormalizedAndVerifiedUri = createCachedSelector((state, rawUri) => rawUri, rawUri => {
  try {
...
export const selectClaimIsMine = (state: State, claim: Claim | null | undefined) => {
  if (!claim) return claim;
...
export const selectClaimIsMineForId = (state: State, claimId: string) => selectClaimIsMine(state, selectClaimForClaimId(state, claimId));
...
export const makeSelectMyPurchasesForPage = (query: string | null | undefined, page: number = 1) => createSelector(selectMyPurchases, selectClaimsByUri, (myPurchases: Array<string>, claimsByUri: Record<string, Claim>) => {
  if (!myPurchases) {
    return undefined;
  }
...
export const selectClaimWasPurchasedForUri = createSelector(selectClaimForUri, claim => Boolean(claim?.purchase_receipt !== undefined));
export const selectAllFetchingChannelClaims = createSelector(selectState, state => state.fetchingChannelClaims || {});
export const makeSelectFetchingChannelClaims = (uri: string) => createSelector(selectAllFetchingChannelClaims, fetching => fetching && fetching[uri]);
export const makeSelectClaimsInChannelForPage = (uri: string, page?: number) => createSelector(selectClaimsById, selectAllClaimsByChannel, (byId, allClaims) => {
...
export const makeSelectTotalClaimsInChannelSearch = (uri: string) => createSelector(selectClaimsById, selectAllClaimsByChannel, (byId, allClaims) => {
  const byChannel = allClaims[uri] || {};
  return byChannel['itemCount'];
});
// THIS IS LEFT OVER FROM ONE_TAB CHANNEL CONTENT
export const makeSelectTotalPagesInChannelSearch = (uri: string) => createSelector(selectClaimsById, selectAllClaimsByChannel, (byId, allClaims) => {
  const byChannel = allClaims[uri] || {};
...
export const selectMetadataItemForClaimIdAndKey = (state: State, claimId: ClaimId, key: ChannelMetadataKey | StreamMetadataKey | CollectionMetadataKey) => selectMetadataForClaimId(state, claimId)[key];
export const selectTagNamesForClaimId = createSelector((state, claimId) => selectMetadataItemForClaimIdAndKey(state, claimId, 'tags'), tags => tags && tags.map(tag => tag.name || tag));
...
export const selectCollectionClaimPublishUpdateMetadataForId = (state: State, claimId: ClaimId) => {
  ...
  // $FlowFixMe
  const collectionPublishUpdateMetadata: CollectionPublishUpdateParams = { ...claimMetadata,
    claims: collectionClaimIds
  };
  return collectionPublishUpdateMetadata;
};
export const makeSelectMetadataForUri = (uri: string) => createSelector(makeSelectClaimForUri(uri), claim => {
  const metadata = claim && claim.value;
  return metadata || (claim === undefined ? undefined : null);
});
export const makeSelectMetadataItemForUri = (uri: string, key: string) => createSelector(makeSelectMetadataForUri(uri), (metadata: ChannelMetadata | StreamMetadata) => {
...
export const selectClaimReleaseInFutureForUri = (state: State, uri: string) => selectMomentReleaseTimeForUri(state, uri).isAfter();
export const selectClaimReleaseInPastForUri = (state: State, uri: string) => selectMomentReleaseTimeForUri(state, uri).isBefore();
export const selectDateForUri = createCachedSelector(selectClaimForUri, // input: (state, uri, ?returnRepost)
claim => {
  // $FlowIgnore
  const forceCreationTimestamp = claim?.value?.tags?.includes(TAG.VISIBILITY_TAGS.UNLISTED);
  const timestamp = claim && claim.value && (claim.value.release_time && !forceCreationTimestamp ? claim.value.release_time * 1000 : claim.meta && claim.meta.creation_timestamp ? claim.meta.creation_timestamp * 1000 : null);
...
export const selectTimestampsForUri = createCachedSelector(selectClaimForUri, // (state, uri, ?returnRepost)
claim => {
...
export const makeSelectAmountForUri = (uri: string) => createSelector(makeSelectClaimForUri(uri), claim => {
...
export const makeSelectEffectiveAmountForUri = (uri: string) => createSelector(makeSelectClaimForUri(uri, false), claim => {
  return claim && claim.meta && typeof claim.meta.effective_amount === 'string' && Number(claim.meta.effective_amount);
});
export const makeSelectContentTypeForUri = (uri: string) => createSelector(makeSelectClaimForUri(uri), claim => {
...
export const makeSelectFileExtensionForUri = (uri: string) => createSelector(makeSelectClaimForUri(uri), claim => {
  const source = claim && claim.value && claim.value.source;
...
export const makeSelectCoverForUri = (uri: string) => createSelector(makeSelectClaimForUri(uri), claim => {
...
export const makeSelectAvatarForUri = (uri: string) => createSelector(makeSelectClaimForUri(uri), claim => {
  if (claim && claim.value.cover) {
    ...
    const avatar = claim && claim.signing_channel && claim.signing_channel.value && claim.signing_channel.value.thumbnail && claim.signing_channel.value.thumbnail;
...
export const selectMyClaimsPageNumber = createSelector(selectState, state => state.claimListMinePage && state.claimListMinePage.items || [], state => state.txoPage && state.txoPage.page || 1);
...
export const selectMyClaims = createSelector(selectMyActiveClaims, selectClaimsById, selectAbandoningIds, (myClaimIds, byId, abandoningIds) => {
  const claims = [];
...
export const selectMyPublicationClaims = createSelector(selectMyClaims, myClaims => myClaims.filter(claim => claim && ['stream', 'repost'].includes(claim.value_type)));
export const selectMyStreamClaims = createSelector(selectMyClaims, myClaims => myClaims.filter(claim => claim && claim.value_type === 'stream'));
export const selectMyRepostClaims = createSelector(selectMyClaims, myClaims => myClaims.filter(claim => claim && claim.value_type === 'repost'));
export const selectMyUnlistedClaims = createSelector(selectMyClaims, myClaims => // $FlowFixMe
myClaims.filter(claim => claim && claim.value?.tags?.includes(TAG.VISIBILITY_TAGS.UNLISTED)));
export const selectMyPaidClaims = createSelector(selectMyClaims, myClaims => // $FlowFixMe
myClaims.filter(claim => claim && claim.value?.tags?.includes(TAG.PURCHASE_TAG, TAG.RENTAL_TAG)));
export const selectMyPaidClaimsLegacy = createSelector(selectMyClaims, myClaims => // $FlowFixMe
myClaims.filter(claim => claim && claim.value?.fee));
export const selectMyScheduledClaims = createSelector(selectMyClaims, myClaims => myClaims.filter(claim => {
  return claim && claim.value?.release_time > Math.floor(Date.now() / 1000) && // $FlowFixMe
  claim.value?.tags?.some(tag => Object.values(TAG.SCHEDULED_TAGS).includes(tag));
}));
export const selectMyClaimsWithoutChannels = createSelector(selectMyClaims, myClaims => myClaims.filter(claim => claim && !claim.name.match(/^@/)).sort((a, b) => a.timestamp - b.timestamp));
...
export const selectAllMyClaimsByOutpoint = createSelector(selectMyClaimsRaw, claims => new Set(claims && claims.length ? claims.map(claim => `${claim.txid}:${claim.nout}`) : null));
...
export const selectMyChannelClaims = createSelector(selectMyChannelClaimsById, myChannelClaimsById => myChannelClaimsById && Object.values(myChannelClaimsById));
export const selectMyChannelClaimIds = createSelector(selectMyChannelClaimsById, myChannelClaimsById => myChannelClaimsById && Object.keys(myChannelClaimsById));
...
export const selectMyChannelClaimUrls = createSelector(selectMyChannelClaims, myChannelClaims => myChannelClaims && myChannelClaims.map(({
  permanent_url
}) => permanent_url));
...
export const selectIsResolvingForId = (state: State, claimId: ClaimId) => new Set(selectResolvingUris(state)).has(claimId);
export const selectChannelClaimCounts = createSelector(selectState, state => state.channelClaimCounts || {});
export const makeSelectPendingClaimForUri = (uri: string) => createSelector(selectPendingClaimsById, pendingById => {
  let uriStreamName;
  let uriChannelName;
  ...
  const pendingClaims = (Object.values(pendingById) as any);
...
export const makeSelectTotalItemsForChannel = (uri: string) => createSelector(selectChannelClaimCounts, byUri => byUri && byUri[normalizeURI(uri)]);
export const makeSelectTotalPagesForChannel = (uri: string, pageSize: number = 10) => createSelector(selectChannelClaimCounts, byUri => byUri && byUri[uri] && Math.ceil(byUri[normalizeURI(uri)] / pageSize));
export const makeSelectNsfwCountFromUris = (uris: Array<string>) => createSelector(selectClaimsByUri, claims => uris.reduce((acc, uri) => {
...
export const makeSelectOmittedCountForChannel = (uri: string) => createSelector(makeSelectTotalItemsForChannel(uri), makeSelectTotalClaimsInChannelSearch(uri), (claimsInChannel, claimsInSearch) => {
  if (claimsInChannel && typeof claimsInSearch === 'number' && claimsInSearch >= 0) {
    return claimsInChannel - claimsInSearch;
  } else return 0;
});
export const selectClaimIsNsfwForUri = createCachedSelector(selectClaimForUri, // Eventually these will come from some list of tags that are considered adult
...
export const selectNameForClaimId = (state: State, claimId: ClaimId) => getNameFromClaim(selectClaimForClaimId(state, claimId));
export const selectChannelForClaimUri = createCachedSelector((state, uri, includePrefix) => includePrefix, selectClaimForUri, (includePrefix?: boolean, claim: Claim) => {
...
export const makeSelectChannelForClaimUri = (uri: string, includePrefix: boolean = false) => createSelector(makeSelectClaimForUri(uri), (claim: Claim | null | undefined) => {
  if (!claim || !claim.signing_channel || !claim.is_channel_signature_valid) {
    return null;
  }
...
export const makeSelectChannelPermUrlForClaimUri = (uri: string, includePrefix: boolean = false) => createSelector(makeSelectClaimForUri(uri), (claim: Claim | null | undefined) => {
...
export const makeSelectMyChannelPermUrlForName = (name: string) => createSelector(selectMyChannelClaims, claims => {
...
export const selectTagsForUri = createCachedSelector(selectMetadataForUri, (metadata: GenericMetadata | null | undefined) => {
...
export const selectCanReceiveTipsForUri = createCachedSelector(selectClaimForUri, (state: State, uri: string) => state, (claim: Claim | null | undefined, state: State) => {
...
export const selectPurchaseTagForUri = createCachedSelector(selectMetadataForUri, selectCostInfoForUri, selectCanReceiveTipsForUri, (metadata: GenericMetadata | null | undefined, costInfo: any | null | undefined, canReceiveTips: boolean | null | undefined) => {
  return parsePurchaseTag(metadata?.tags) || costInfo?.feeCurrency === 'USD' && costInfo?.cost || canReceiveTips && costInfo?.feeCurrency === 'LBC' && costInfo?.usdCost;
})((state, uri) => String(uri));
export const selectPreorderTagForUri = createCachedSelector(selectMetadataForUri, (metadata: GenericMetadata | null | undefined) => {
...
export const selectProtectedContentTagForUri = createSelector(selectMetadataForUri, (metadata: GenericMetadata | null | undefined) => metadata && new Set(metadata.tags).has(MEMBERS_ONLY_CONTENT_TAG));
export const selectedRestrictedCommentsChatTagForUri = createSelector(selectMetadataForUri, (metadata: GenericMetadata | null | undefined) => metadata && new Set(metadata.tags).has(RESTRICTED_CHAT_COMMENTS_TAG));
export const selectRentalTagForUri = createCachedSelector(selectMetadataForUri, (metadata: GenericMetadata | null | undefined) => {
  return parseRentalTag(metadata?.tags);
})((state, uri) => String(uri));
export const selectPreorderContentClaimIdForUri = createCachedSelector(selectMetadataForUri, (metadata: GenericMetadata | null | undefined) => {
...
export const selectIsFetchingClaimSearchForQuery = (state: State, query: string) => selectFetchingClaimSearchByQuery(state)[query] || false;
export const selectFetchingClaimSearch = createSelector(selectFetchingClaimSearchByQuery, fetchingClaimSearchByQuery => Boolean(Object.keys(fetchingClaimSearchByQuery).length));
export const selectClaimSearchByQuery = createSelector(selectState, state => state.claimSearchByQuery || {});
export const selectClaimSearchByQueryLastPageReached = createSelector(selectState, state => state.claimSearchByQueryLastPageReached || {});
...
export const makeSelectSupportsForUri = (uri: string) => createSelector(selectSupportsByOutpoint, makeSelectClaimForUri(uri), (byOutpoint, claim: StreamClaim | null | undefined) => {
...
export const makeSelectReflectingClaimForUri = (uri: string) => createSelector(selectClaimIdsByUri, selectReflectingById, (claimIdsByUri, reflectingById) => {
  const claimId = claimIdsByUri[normalizeURI(uri)];
  return reflectingById[claimId];
});
export const makeSelectMyStreamUrlsForPage = (page: number = 1) => createSelector(selectMyClaimUrisWithoutChannels, urls => {
  const start = (Number(page) - 1) * Number(CLAIM.PAGE_SIZE);
  const end = Number(page) * Number(CLAIM.PAGE_SIZE);
...
export const makeSelectTagInClaimOrChannelForUri = (uri: string, tag: string) => createSelector(makeSelectClaimForUri(uri), claim => {
...
export const makeSelectClaimHasSource = (uri: string) => createSelector(makeSelectClaimForUri(uri), claim => {
  if (!claim) {
    return false;
  }
...
export const selectIsMyChannelCountOverLimit = createSelector(selectMyChannelClaimIds, selectYoutubeChannels, (myClaimIds, ytChannels: Array<{
 ...
 */
export const selectGeoRestrictionForUri = createCachedSelector(selectClaimForUri, selectGeoBlockLists, selectUserLocale, (claim, geoBlockLists, locale: LocaleInfo) => {
...
export const selectPurchaseMadeForClaimId = (state: State, claimId: string) => {
  ...
  return purchasedClaims.some(p => (p.reference_claim_id === claimId || p.target_claim_id === claimId) && p.type === 'purchase');
};
...
export const selectIsFiatPaidForUri = (state: State, uri: string) => {
  ...
  if (claimId) {
    return Boolean(selectPurchaseMadeForClaimId(state, claimId)) || Boolean(selectValidRentalPurchaseForClaimId(state, claimId));
  }
};
