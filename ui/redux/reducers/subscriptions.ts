...
export default handleActions({
  ...
  [ACTIONS.CHANNEL_UNSUBSCRIBE]: (state: SubscriptionState, action): SubscriptionState => {
    ...
    const newSubscriptions = state.subscriptions.filter(subscription => !isURIEqual(subscription.uri, subscriptionToRemove.uri));
    const newFollowing = state.following.filter(subscription => !isURIEqual(subscription.uri, subscriptionToRemove.uri));
    return { ...state,
      subscriptions: newSubscriptions,
      following: newFollowing
    };
  },
  [ACTIONS.FETCH_SUBSCRIPTIONS_START]: (state: SubscriptionState): SubscriptionState => ({ ...state,
    loading: true
  }),
  [ACTIONS.FETCH_SUBSCRIPTIONS_FAIL]: (state: SubscriptionState): SubscriptionState => ({ ...state,
    loading: false
  }),
  [ACTIONS.FETCH_SUBSCRIPTIONS_SUCCESS]: (state: SubscriptionState, action): SubscriptionState => ({ ...state,
    loading: false,
  ...
  [ACTIONS.FETCH_LAST_ACTIVE_SUBS_FAIL]: (state: SubscriptionState, action): SubscriptionState => ({ ...state,
    lastActiveSubscriptions: null
  }),
  [ACTIONS.SET_VIEW_MODE]: (state: SubscriptionState, action): SubscriptionState => ({ ...state,
    viewMode: action.data
  }),
  [ACTIONS.USER_STATE_POPULATE]: (state: SubscriptionState, action: {
    data: {
      subscriptions: Array<string> | null | undefined;
      following: Array<Subscription> | null | undefined;
    };
  }) => {
    const {
      subscriptions,
    ...
    if (!Array.isArray(following)) {
      newFollowing = newSubscriptions.slice().map(({
        uri
      }) => {
        return {
          uri,
