...
const defaultState: CollectionState = {
  ...
  savedIds: [],
  // -- local --
  isFetchingMyCollectionClaims: false,
...
const collectionsReducer = handleActions(
  {
    [ACTIONS.COLLECTION_LIST_MINE_STARTED]: (state) => ({ ...state, isFetchingMyCollectionClaims: true }),
    [ACTIONS.COLLECTION_LIST_MINE_COMPLETE]: (state) => ({ ...state, isFetchingMyCollectionClaims: false }),
    ...
    [ACTIONS.COLLECTION_ITEMS_RESOLVE_STARTED]: (state, action) => {
      ...
      const newResolving = new Set(state.collectionItemsFetchingIds);
      ids.forEach((id) => newResolving.add(id));

      return { ...state, collectionItemsFetchingIds: Array.from(newResolving) };
    },
    [ACTIONS.COLLECTION_ITEMS_RESOLVE_SUCCESS]: (state, action) => {
      ...
      if (failedCollectionIds && Object.keys(failedCollectionIds).length) {
        failedCollectionIds.forEach((failedId) => newResolving.delete(failedId));
      }
      ...
      return {
        ...
        updated: newUpdated,
        collectionItemsFetchingIds: Array.from(newResolving),
    ...
    [ACTIONS.COLLECTION_ITEMS_RESOLVE_FAILED]: (state, action) => {
      ...
      const newResolving = new Set(state.collectionItemsFetchingIds);
      ids.forEach((id) => newResolving.delete(id));
