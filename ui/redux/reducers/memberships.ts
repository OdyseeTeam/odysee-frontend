...
export type MembershipsState = {
  membershipMineByCreatorId: MembershipSubscribedDataByCreatorId | null | undefined;
  membershipMineFetching: boolean;
  membershipListByCreatorId: Record<string, MembershipSubs>;
  membershipListFetchingIds: ClaimIds;
  channelMembershipsByCreatorId: ChannelMembershipsByCreatorId;
  fetchingIdsByCreatorId: Record<string, ClaimIds>;
  legacyOdyseePremiumById: Record<string, string | null>;
  fetchingOdyseePremiumIds: Array<string>;
  pendingBuyIds: ClaimIds;
  membershipBuyError: string;
  pendingCancelIds: ClaimIds;
  myMembershipTiers: MembershipSubs | null | undefined;
  pendingDeleteIds: Array<string>;
  protectedContentClaimsByCreatorId: Record<string, any>;
  mySupportersList: SupportersList | null | undefined;
  membershipOdyseePerks: Array<MembershipOdyseePerk>;
  listingAllMyTiers: boolean | null | undefined;
...
reducers[ACTIONS.USER_LEGACY_ODYSEE_PREMIUM_CHECK_STARTED] = (state, action) => {
  ...
  const {
    fetchingOdyseePremiumIds: currentFetching
  } = state;
...
reducers[ACTIONS.USER_LEGACY_ODYSEE_PREMIUM_CHECK_SUCCESS] = (state, action) => {
  const {
    membershipsById
  } = action.data; // [{ [claimId: string]: 'Premium' | null}]
  ...
  const fetchingSet = new Set(currentFetching);
  Object.entries(membershipsById).forEach(([channelId, premiumValue]) => {
    newLegacyOdyseePremiumById[channelId] = premiumValue;
...
reducers[ACTIONS.CHANNEL_MEMBERSHIP_CHECK_STARTED] = (state, action) => {
  ...
  const {
    fetchingIdsByCreatorId: currentFetching
  } = state;
  return { ...state,
    fetchingIdsByCreatorId: { ...currentFetching,
      [channel]: ids
...
reducers[ACTIONS.CHANNEL_MEMBERSHIP_CHECK_COMPLETED] = (state, action) => {
  ...
  const newFetched = currentFetched[channelId] ? { ...currentFetched[channelId],
    ...newData
  } : membershipsById;
  ...
  return { ...state,
    ...
    channelMembershipsByCreatorId: { ...currentFetched,
      [channelId]: newFetched
    }
  };
};
...
reducers[ACTIONS.SET_MEMBERSHIP_BUY_STARTED] = (state, action) => {
  ...
  return { ...state,
    pendingBuyIds: Array.from(newPendingBuyIds),
    membershipBuyError: ''
  };
};
...
reducers[ACTIONS.SET_MEMBERSHIP_CANCEL_STARTED] = (state, action) => {
  ...
  return { ...state,
    pendingCancelIds: Array.from(newPendingCancelIds)
  };
};
...
reducers[ACTIONS.DELETE_MEMBERSHIP_STARTED] = (state, action) => {
  ...
  return { ...state,
    pendingDeleteIds: Array.from(newPendingDeleteIds)
  };
};
...
reducers[ACTIONS.GET_MEMBERSHIP_MINE_START] = (state, action) => ({ ...state,
  membershipMineFetching: true
});
...
reducers[ACTIONS.GET_MEMBERSHIP_MINE_DATA_SUCCESS] = (state, action) => {
  ...
  return { ...state,
    membershipMineByCreatorId: newMembershipMineByCreatorId,
    membershipMineFetching: false
  };
};

reducers[ACTIONS.GET_MEMBERSHIP_MINE_DATA_FAIL] = (state, action) => ({ ...state,
...
reducers[ACTIONS.MEMBERSHIP_PERK_LIST_COMPLETE] = (state, action) => ({ ...state,
  membershipOdyseePerks: action.data
});
...
reducers[ACTIONS.GET_CLAIM_MEMBERSHIP_TIERS_START] = (state, action) => {
  ...
  return { ...state,
    claimMembershipTiersFetchingIds: newClaimMembershipTiersFetchingIds
  };
};
  ...
  if (response && response.length > 0) {
    response.forEach((membershipContent: MembershipContentResponseItem) => {
      ...
      if (!creatorContentMemberships) {
        Object.assign(newProtectedContentClaims, {
          [creatorId]: newCreatorContentMemberships
        });
      }
      ...
      if (!contentClaimMemberships) {
        Object.assign(newProtectedContentClaims[creatorId], {
          [claimId]: newContentClaimMemberships
        });
      }
...
reducers[ACTIONS.GET_CLAIM_MEMBERSHIP_TIERS_FAIL] = (state, action) => {
  ...
  const newClaimMembershipTiersFetchingIds = state.claimMembershipTiersFetchingIds.filter(fetchingId => !claimIds.has(fetchingId));
  return { ...state,
    claimMembershipTiersFetchingIds: newClaimMembershipTiersFetchingIds
  };
};
...
reducers[ACTIONS.MEMBERSHIP_TX_INCOMING_STARTED] = state => {
  return { ...state,
    membershipPaymentsIncomingFetching: true,
    membershipPaymentsIncomingError: ''
  };
};
...
reducers[ACTIONS.MEMBERSHIP_TX_INCOMING_SUCCESSFUL] = (state, action) => {
  return { ...state,
    membershipPaymentsIncoming: action.data,
    membershipPaymentsIncomingFetching: false
  };
};
...
reducers[ACTIONS.MEMBERSHIP_TX_INCOMING_FAILED] = (state, action) => {
  return { ...state,
    membershipPaymentsIncomingFetching: false,
    membershipPaymentsIncomingError: action.data
  };
};

reducers[ACTIONS.MEMBERSHIP_TX_OUTGOING_STARTED] = state => {
  return { ...state,
    membershipPaymentsOutgoingFetching: true,
    membershipPaymentsOutgoingError: ''
  };
};
...
reducers[ACTIONS.MEMBERSHIP_TX_OUTGOING_SUCCESSFUL] = (state, action) => {
  return { ...state,
    membershipPaymentsOutgoingFetching: false,
    membershipPaymentsOutgoing: action.data
  };
};
...
reducers[ACTIONS.LIST_ALL_MY_MEMBERSHIPS_START] = (state, action) => ({ ...state,
  listingAllMyTiers: true
});

reducers[ACTIONS.LIST_ALL_MY_MEMBERSHIPS_COMPLETE] = (state, action) => ({ ...state,
  listingAllMyTiers: false
}); // -----------------------------------------------------------------------------------------------
// -----------------------------------------------------------------------------------------------
