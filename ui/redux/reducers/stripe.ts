...
reducers[ACTIONS.STRIPE_ACCOUNT_STATUS_START] = (state, action) => ({ ...state,
  accountStatusFetching: true
});

reducers[ACTIONS.STRIPE_ACCOUNT_STATUS_COMPLETE] = (state, action) => ({ ...state,
  accountStatusFetching: false,
  accountStatus: action.data?.stripe,
  arweaveStatus: action.data?.arweave
});

reducers[ACTIONS.AR_ADDR_DEFAULT_STARTED] = state => ({ ...state,
  arAccountUpdating: true,
  arAccountUpdatingError: undefined
});

reducers[ACTIONS.AR_ADDR_DEFAULT_SUCCESS] = state => ({ ...state,
  arAccountUpdating: false
});

reducers[ACTIONS.AR_ADDR_DEFAULT_ERROR] = (state, action) => ({ ...state,
  arAccountUpdating: false,
  arAccountUpdatingError: action.data
});

// Register
reducers[ACTIONS.AR_ADDR_REGISTER_STARTED] = (state, action) => ({ ...state,
  arAccountRegisteringId: action.data,
  arAccountRegisteringError: undefined
});

reducers[ACTIONS.AR_ADDR_REGISTER_SUCCESS] = (state, action) => ({ ...state,
  arAccountRegisteringId: undefined
});

reducers[ACTIONS.AR_ADDR_REGISTER_ERROR] = (state, action) => ({ ...state,
  arAccountRegisteringId: undefined,
  arAccountRegisteringError: action.data
});

reducers[ACTIONS.AR_ADDR_REGISTER_CLEAR] = (state, action) => ({ ...state,
  arAccountRegisteringId: undefined,
  arAccountRegisteringError: undefined
});

// Update
reducers[ACTIONS.AR_ADDR_UPDATE_STARTED] = (state, action) => ({ ...state,
  arAccountUpdatingId: action.data,
  arAccountUpdatingError: undefined
});

reducers[ACTIONS.AR_ADDR_UPDATE_SUCCESS] = (state, action) => ({ ...state,
  arAccountUpdatingId: undefined
});

reducers[ACTIONS.AR_ADDR_UPDATE_ERROR] = (state, action) => ({ ...state,
  arAccountUpdatingId: undefined,
  arAccountUpdatingError: action.data
});

// Link
reducers[ACTIONS.SET_ACCOUNT_LINK] = (state, action) => ({ ...state,
  accountLinkResponse: action.data
});

reducers[ACTIONS.SET_ACCOUNT_TRANSACTIONS] = (state, action) => ({ ...state,
  accountTransactions: action.data
});

reducers[ACTIONS.SET_ACCOUNT_PAYMENT_HISTORY] = (state, action) => ({ ...state,
  accountPaymentHistory: action.data
});

reducers[ACTIONS.SET_CUSTOMER_STATUS_STARTED] = (state, action) => ({ ...state,
  customerStatusFetching: true
});

reducers[ACTIONS.SET_CUSTOMER_STATUS] = (state, action) => ({ ...state,
  customerStatus: action.data,
  customerStatusFetching: false
});

reducers[ACTIONS.SET_CUSTOMER_SETUP_RESPONSE] = (state, action) => ({ ...state,
  customerSetupResponse: action.data
});
