...
reducers[ACTIONS.ARCONNECT_FETCHBALANCE] = (state, action) => ({ ...state, fetching: true });
...
reducers[ACTIONS.AR_SEND_STARTED] = (state) => ({ ...state, sending: true, error: null });
...
reducers[ACTIONS.AR_SEND_ERROR] = (state, action) => ({ ...state, sending: false, error: action.data.error });
