...
import { tusLockAndNotify, tusUnlockAndNotify, tusRemoveAndNotify, tusClearRemovedUploads, tusClearLockedUploads } from "util/tus";
...
export const publishReducer = handleActions({
    ...
    switch (visibility) {
      ...
      case 'scheduled':
        if (releaseTime) {
          ...
          if (isEditing) {
            assert(state.claimToEdit?.value?.release_time, 'scheduled claim without release_time'); // -- No need to enforce elapsed date when editing --
  ...
  [ACTIONS.CLEAR_PUBLISH]: (state: PublishState, action): PublishState => ({ ...defaultState,
    type: state.type,
    uri: undefined,
  ...
  [ACTIONS.PUBLISH_START]: (state: PublishState): PublishState => ({ ...state,
    publishing: true,
    publishSuccess: false
  }),
  [ACTIONS.PUBLISH_FAIL]: (state: PublishState): PublishState => ({ ...state,
    publishing: false
  }),
  [ACTIONS.PUBLISH_SUCCESS]: (state: PublishState): PublishState => ({ ...state,
    publishing: false,
    publishSuccess: true
  }),
  ...
  [ACTIONS.UPDATE_UPLOAD_ADD]: (state: PublishState, action) => {
    ...
    currentUploads[params.guid] = {
      file,
      fileFingerprint: file ? serializeFileObj(file) : undefined,
      // TODO: get hash instead?
  ...
  [ACTIONS.REHYDRATE]: (state: PublishState, action) => {
    if (action && action.payload && action.payload.publish) {
      ...
      const newPublish = { ...action.payload.publish,
        filePath: undefined,
        // File is not serializable, so can't rehydrate.
