...
import { makeSelectPostFormValue, selectPostFormValues, selectMyClaimForUri } from 'redux/selectors/publishPost';
...
export const doPublishDesktop = (filePath: string, preview?: boolean) => (dispatch: Dispatch, getState: () => {}) => {
  ...
  const editingUri = makeSelectPostFormValue('editingURI')(state) || '';
  const remoteUrl = makeSelectPostFormValue('remoteFileUrl')(state);
...
export const doPrepareEdit = (claim: StreamClaim, uri: string, fileInfo: FileListItem, fs: any) => (
  ...
  const postData: UpdatePostFormData = {
    name,
    bid: Number(amount),
...
export const doPublish = (success: Function, fail: Function, preview: Function, payload: any) => (
  ...
  const publishData = selectPostFormValues(state);
