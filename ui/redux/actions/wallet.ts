...
import { selectBalance, selectTotalBalance, selectPendingSupportTransactions, selectTxoPageParams, selectPendingOtherTransactions, selectPendingConsolidateTxid, selectPendingMassClaimTxid, selectIsFetchingAccounts } from "redux/selectors/wallet";
import { resolveApiMessage } from "util/api-message";
import { creditsToString } from "util/format-credits";
import { dispatchToast } from "util/toast-wrappers";
import { selectMyClaimsRaw, selectClaimsById, selectClaimForUri, selectPreorderTagForUri, selectPurchaseTagForUri, selectRentalTagForUri } from "redux/selectors/claims";
...
export function doFetchAccountList(page = 1, pageSize = 99999) {
  return (dispatch, getState) => {
    ...
    dispatch({
      type: ACTIONS.FETCH_ACCOUNT_LIST_STARTED
    });

    const callback = response => {
      dispatch({
        type: ACTIONS.FETCH_ACCOUNT_LIST_COMPLETED,
        data: response.items
      });
    };
    ...
    const failure = () => {
      dispatch({
        type: ACTIONS.FETCH_ACCOUNT_LIST_FAILED
      });
    };
...
export function doFetchTxoPage() {
  return (dispatch, getState) => {
    ...
    Lbry.txo_list(queryParams).then(res => {
...
export function doFetchUtxoCounts() {
  return async dispatch => {
    ...
    let resultSets = await Promise.all([Lbry.txo_list({
      type: 'other',
      is_not_spent: true,
      page: 1,
      page_size: 1
  ...
  return (dispatch, getState) => {
    ...
    const error = err => {
      ...
      if (errMsg.endsWith(ERRORS.SDK_FETCH_TIMEOUT)) {
        ...
        switch (purpose) {
          case 'comment':
            msg = __('The transaction timed out, but may have been completed. Please wait a few minutes, then check your wallet transactions before attempting to retry. Note that due to current limitations, we are unable to re-link the tip sent to a new comment.');
            break;

          default:
            msg = __('The transaction timed out, but may have been completed. Please wait a few minutes, then check your wallet transactions before attempting to retry.');
            break;
        }
...
export function doSupportAbandonForClaim(claimId, claimType, keep, preview) {
  return dispatch => {
    ...
    return Lbry.support_abandon(params).then(res => {
      if (!preview) {
...
export function doUpdateBlockHeight() {
  return dispatch => Lbry.status().then(status => {
    if (status.wallet) {
      dispatch({
...
export const doCheckPendingTxs = () => (dispatch, getState) => {
  ...
  const checkTxList = () => {
    ...
    entries.forEach(([claim, data]) => {
      promises.push(Lbry.transaction_show({
        txid: data.txid
      }));
      types.add(data.type);
    });
    ...
    if (pendingConsolidateTxes.length) {
      pendingConsolidateTxes.forEach(txid => promises.push(Lbry.transaction_show({
        txid
      })));
    }
    ...
    Promise.all(promises).then(txShows => {
      ...
      if (noLongerPendingConsolidate.length) {
        ...
        dispatch({
          type: ACTIONS.PENDING_CONSOLIDATED_TXOS_UPDATED,
          data: {
            txids: noLongerPendingConsolidate,
            remove: true
          }
        });
      }
...
export const doSendCashTip = (tipParams, anonymous, userParams, claimId, stripeEnvironment, preferredCurrency, successCallback) => dispatch => {
  Lbryio.call('customer', 'tip', {
    // round to fix issues with floating point numbers
    amount: Math.round(100 * tipParams.tipAmount),
    // convert from dollars to cents
    creator_channel_name: tipParams.tipChannelName,
...
export const doPurchaseClaimForUri = ({
  ...
  if (isV2) {
    try {
      const tipResponse = await Lbryio.call('customer', 'new_transaction', {
        // round to fix issues with floating point numbers
        amount: Math.round(100 * amountToUse),
        // convert from dollars to cents
        creator_channel_name: tipChannelName,
        creator_channel_claim_id: channelClaimId,
        ...(activeChannelClaim ? {
          tipper_channel_name: activeChannelClaim.name,
          tipper_channel_claim_id: activeChannelClaim.claim_id
        } : {
          anonymous: true
        }),
      ...
      const {
        transactionId
      } = await sendWinstons(arweaveTipData.address, String(transaction_amount), tags);
      await Lbryio.call('customer', 'new_transaction', {
        // round to fix issues with floating point numbers
        amount: Math.round(100 * amountToUse),
        // convert from dollars to cents
        creator_channel_name: tipChannelName,
  ...
  return Lbryio.call('customer', 'new_transaction', {
    // round to fix issues with floating point numbers
    amount: Math.round(100 * amountToUse),
    // convert from dollars to cents
    creator_channel_name: tipChannelName,
