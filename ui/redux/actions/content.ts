...
export function doSetRecsysEntries(entries: Record<ClaimId, RecsysEntry>) {
  return (dispatch: Dispatch) => {
    dispatch({
...
export function doFetchUriAccessKey(uri: string) {
  return async (dispatch: Dispatch, getState: GetState): Promise<UriAccessKey | null | undefined> => {
    const state = getState();
    const claim = selectClaimForUri(state, uri);
