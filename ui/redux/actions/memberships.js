...
export const doFetchOdyseeMembershipForChannelIds = (channelIds: ClaimIds) => async (dispatch: Dispatch) =>
  dispatch(doFetchChannelMembershipsForChannelIds(ODYSEE_CHANNEL_ID, channelIds));

export const doMembershipList = (params: { channel_name: string, channel_id: string }) => async (dispatch: Dispatch) =>
  ...
  await Lbryio.call('membership', 'mine', { environment: stripeEnvironment }, 'post')
    .then((response) => {
      ...
      for (const membership of response) {
        ...
        if (membershipChannel === ODYSEE_CHANNEL_NAME) {
