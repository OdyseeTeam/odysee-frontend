...
import { selectSyncHash, selectLastSyncHash, selectGetSyncIsPending, selectSetSyncIsPending, selectSyncIsLocked } from "redux/selectors/sync";
...
export const doGetSyncDesktop = (cb?: (arg0: any, arg1: any) => void, password?: string) => (dispatch: Dispatch, getState: GetState) => {
  const state = getState();
...
export function doGetSync(passedPassword?: string, callback?: (arg0: any, arg1: boolean | null | undefined) => void) {
  ...
  // @if TARGET='web'
  const xAuth = Lbry.getApiRequestHeaders() && Object.keys(Lbry.getApiRequestHeaders()).includes(X_LBRY_AUTH_TOKEN) ? Lbry.getApiRequestHeaders()[X_LBRY_AUTH_TOKEN] : '';
  ...
  return (dispatch: Dispatch, getState: GetState) => {
    ...
    const data = {};
    return Lbry.wallet_status().then(status => {
      if (status.is_locked) {
      ...
      const badPasswordError = syncAttemptError && syncAttemptError.data && syncAttemptError.data.name === BAD_PASSWORD_ERROR_NAME;
      const tooBigDataError = Boolean( // $FlowIgnore
      syncAttemptError?.message?.match(/rpc call sync_apply\(\) on.*?status code: 413. could not decode body to rpc response: invalid character/));
...
export function doResetSync() {
  return (dispatch: Dispatch): Promise<any> => new Promise(resolve => {
    dispatch({
...
export function doSyncEncryptAndDecrypt(oldPassword: string, newPassword: string, encrypt: boolean) {
  return (dispatch: Dispatch) => {
    const data = {};
    return Lbry.sync_hash().then(hash => Lbryio.call('sync', 'get', {
      hash
...
async function syncSharedPreferenceWrite(dispatch: Dispatch, getState: GetState, previousSyncHash: string | null | undefined, currentSyncHash: string | null | undefined) {
  const state = getState();
  const syncEnabled = selectClientSetting(state, SETTINGS.ENABLE_SYNC);
  const hasVerifiedEmail = selectUserVerifiedEmail(state);
  const syncLocked = selectSyncIsLocked(state);
...
export function doPreferenceSet(key: string, value: any, version: string, success: (...args: Array<any>) => any, fail: (...args: Array<any>) => any) {
...
export function doPreferenceGet(key: string, success: (...args: Array<any>) => any, fail?: (...args: Array<any>) => any) {
  return (dispatch: Dispatch) => {
    const options = {
      key
    };
    return Lbry.preference_get(options).then(result => {
      if (result) {
