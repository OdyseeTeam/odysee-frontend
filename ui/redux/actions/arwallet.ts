...
import { ARCONNECT_FAILURE, ARCONNECT_STARTED, ARCONNECT_SUCCESS, ARCONNECT_DISCONNECT, ARCONNECT_FETCHBALANCE, ARSETEXCHANGERATE, AR_TIP_STATUS_STARTED, AR_TIP_STATUS_SUCCESS, AR_TIP_STATUS_ERROR, WANDER_AUTH, AR_SEND_STARTED, AR_SEND_ERROR, AR_SEND_SUCCESS } from "constants/action_types";
// @ts-ignore
...
export const WALLET_PERMISSIONS = ['ACCESS_ADDRESS', 'ACCESS_PUBLIC_KEY', 'SIGN_TRANSACTION', 'DISPATCH', 'SIGNATURE', 'ENCRYPT', 'DECRYPT', 'ACCESS_TOKENS'];
...
export function doArInit() {
  return async (dispatch: Dispatch, getState: GetState) => {
    try {
      ...
      dispatch({
        type: ARSETEXCHANGERATE,
        data: Number(arExchangeRate)
      });
    } catch (e) {
      console.error(e);
    }
  ...
  return async (dispatch: Dispatch, getState: GetState) => {
    ...
    if (window.arweaveWallet) {
      try {
        ...
        dispatch({
          type: ARCONNECT_FAILURE,
          data: {
            error: e?.message || 'Error connecting to Arconnect.'
          }
        });
      }
    } else {
      dispatch({
        type: ARCONNECT_FAILURE,
        data: {
          error: 'Arconnect not found, install the extension.'
        }
      });
    }
...
export const doArTip = (tipParams: LocalTipParams, anonymous: boolean, userParams: UserParams, claimId: string, stripeEnvironment: string) => {
  return async (dispatch: Dispatch, getState: GetState) => {
    dispatch({
      type: AR_TIP_STATUS_STARTED,
      data: {
        claimId: claimId
      }
    });
    ...
    try {
      if (!window.arweaveWallet) {
        dispatch({
          ...
          data: {
            claimId: claimId,
            error: 'error: no wallet connection'
          }
        });
        return;
      }
      ...
      if (activeAddress !== senderAddress) {
        dispatch({
          ...
          data: {
            claimId: claimId,
            error: 'error: wallet address not registered'
          }
        });
        return;
      }
      ...
      if (!isRetry) {
        ...
        'customer', 'tip', {
          ...
          amount: Math.round(TWO_PLACES_TO_PENNIES * tipParams.tipAmountTwoPlaces),
      ...
      if (tipParams.currency === 'AR') {
        try {
          ...
          const {
            transactionId: txid
          } = await sendWinstons(tipParams.recipientAddress, transactionAmountString, tags);
          transactionId = txid;
        } catch (error) {
          console.error(error);
          dispatch({
            ...
            data: {
              claimId: claimId,
              error: 'error: arweave transaction failed'
            }
          });
          return {
        ...
        transactionId = await message({
          ...
          tags: [{
            ...
            name: 'Quantity',
            value: String(tipParams.tipAmountTwoPlaces * USD_TO_USDC)
          }, // test/fix
          {
            name: 'Recipient',
            value: tipParams.recipientAddress
          }, // get address
          {
            name: 'Tip_Type',
            value: 'tip'
      ...
      await Lbryio.call('customer', 'tip', {
        // round to fix issues with floating point numbers
        amount: Math.round(TWO_PLACES_TO_PENNIES * tipParams.tipAmountTwoPlaces),
        // convert from dollars to cents
        creator_channel_name: tipParams.tipChannelName,
  ...
  return async (dispatch: Dispatch, getState: GetState) => {
    ...
    if (Object.keys(currentState.arwallet?.tippingStatusById).length === 1) {
      dispatch({
        type: AR_TIP_STATUS_ERROR,
        data: {
          claimId: Object.keys(currentState.arwallet?.tippingStatusById)[0],
          error: 'Modal closed'
        }
      });
    }
...
export const sendWinstons = async (address: string, amountInWinstons: string, tags: Array<{
  name: string;
  value: string;
}>) => {
  let txResponse: {
  ...
  return async (dispatch: Dispatch) => {
    ...
    try {
      ...
      dispatch({
        type: AR_SEND_ERROR,
        data: {
          error: e.message || 'Failed to send AR'
        }
      });
      return {
        error: e.message || 'Failed to send AR'
