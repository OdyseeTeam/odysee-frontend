...
import { selectEmailToVerify, selectPhoneToVerify, selectUserCountryCode, selectUser, selectSetReferrerPending, selectReferrer } from "redux/selectors/user";
...
export function doInstallNew(appVersion, callbackForUsersWhoAreSharingData, domain) {
  ...
  Lbry.status().then(status => {
    ...
    Lbry.version().then(version => {
      ...
      analytics.log(`'install/new' skipped due to: (${e.message})`, {
        fingerprint: ['doInstallNew']
      });
    });
  });
}
...
export function doUserHasPremium() {
  return async (dispatch, getState) => {
    ...
    dispatch({
      type: ACTIONS.USER_ODYSEE_PREMIUM_CHECK_STARTED
    });

    try {
      const resById = await Lbryio.call('user', 'has_premium', {
        channel_claim_ids: channelClaim
      }, 'post');
      ...
      dispatch({
        type: ACTIONS.USER_LEGACY_ODYSEE_PREMIUM_CHECK_SUCCESS,
        data: {
          membershipsById: membershipsById
        }
      });
      dispatch({
        type: ACTIONS.USER_ODYSEE_PREMIUM_CHECK_SUCCESS,
        data: resById[channelClaim]
      });
    } catch (e) {
      dispatch({
        type: ACTIONS.USER_ODYSEE_PREMIUM_CHECK_FAILURE
      });
    }
...
export function doChannelsHavePremium(channelClaimIds) {
  return async dispatch => {
    dispatch({
      type: ACTIONS.USER_LEGACY_ODYSEE_PREMIUM_CHECK_STARTED,
      data: {
    ...
    try {
      const resById = await Lbryio.call('user', 'has_premium', {
        channel_claim_ids: channelClaimIdsCsv
      }, 'post');
      const channelIds = Object.keys(resById);
      ...
      dispatch({
        type: ACTIONS.USER_LEGACY_ODYSEE_PREMIUM_CHECK_FAILURE,
        data: {
          channelIds: channelClaimIds
        }
      });
...
export function doAuthenticate(appVersion, shareUsageData = true, callbackForUsersWhoAreSharingData, callInstall = true) {
  return dispatch => {
    dispatch({
...
export function doUserDeleteAccount() {
  return async dispatch => {
    ...
    const failure = error => {
      ...
      if (error.message === 'a delete operation is already in progress') {// Don't throw error
      } else {
        throw error;
      }
    };
...
export const doUserSetReferrerForUri = referrerPermanentUri => async (dispatch, getState) => {
  ...
  dispatch({
    type: ACTIONS.USER_SET_REFERRER_START
  });
  const referrerCode = referrerPermanentUri.replace('lbry://', '');
  return await Lbryio.call('user', 'referral', {
...
export function doFetchUserLocale(isRetry = false) {
  return dispatch => {
    fetch(LOCALE_API).then(async res => {
      let json = {};
