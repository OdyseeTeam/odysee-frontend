...
import { doAbandonClaim, doResolveUris, doResolveClaimId, doResolveClaimIds, doCheckPendingClaims } from "redux/actions/claims";
import { selectClaimForClaimId, selectClaimForId, makeSelectMetadataItemForUri, selectHasClaimForId, selectResolvingIds, selectResolvingUris, selectFailedToResolveUris, selectFailedToResolveIds, selectClaimIsPendingForId, selectClaimIdsByUri, selectClaimsById } from "redux/selectors/claims";
import { selectCollectionForId, selectResolvedCollectionForId, selectCollectionHasItemsResolvedForId, selectHasPrivateCollectionForId, selectIsCollectionPrivateForId, selectUrlsForCollectionId, selectCollectionSavedForId, selectAreCollectionItemsFetchingForId, selectCollectionKeyForId, selectCollectionForIdClaimForUriItem, selectAreThumbnailClaimsFetchingForCollectionIds, selectCollectionAutoPublishForId, selectCollectionIsPublishingForId, selectCollectionHasEditsForId, selectCollectionHasUnsavedEditsForId, selectCollectionIsMine } from "redux/selectors/collections";
...
export const doFetchCollectionListMine = (options: CollectionListOptions = {
  resolve: true,
  page: 1,
  page_size: 50
...
export function doCollectionPublish(options: CollectionPublishCreateParams, collectionId: string, cb?: () => void) {
  return (dispatch: Dispatch, getState: GetState): Promise<any> => {
    ...
    if (isPrivate) {
      const publishParams: CollectionPublishCreateParams = { ...params,
        name: options.name,
        // -- avoid duplicates --
      ...
      const updateParams: CollectionPublishUpdateParams = { ...params,
    ...
    return new Promise((resolve, reject) => {
      ...
      function failure(error) {
        ...
        if (scriptSizeError) {
          ...
          customMessage = __('Playlist exceeds size limits.') + ' ' + (itemsToDelete > 1 ? __('Please remove %itemsToDelete% items', {
            itemsToDelete
          }) : __('Please remove 1 item')) + ' ' + (extraBytes > 1 ? __('or %extraBytes% characters of text.', {
            extraBytes
...
const doAutoPublishCollectionIfNeeded = (collectionId: string, triggerNow: boolean = false) => (dispatch: Dispatch, getState: GetState): Promise<void> => {
  const state = getState();
  ...
  if (!triggerNow && !isAutoPublishEnabled || isPrivate || !isMine || isPending || isPublishing || !hasEdits && !hasUnsavedEdits) {
  ...
  const runPublish = () => // $FlowFixMe (selector already shapes params)
...
export const doSetCollectionAutoPublish = (collectionId: string, enabled: boolean) => (dispatch: Dispatch) => {
  ...
  dispatch(doToast({
    message: enabled ? __('Auto-publish enabled. New playlist edits will publish in the background.') : __('Auto-publish disabled. You can still publish manually.')
  }));
  if (enabled) dispatch(doAutoPublishCollectionIfNeeded(collectionId));
};
export const doRetryCollectionPublish = (collectionId: string) => (dispatch: Dispatch) => dispatch(doAutoPublishCollectionIfNeeded(collectionId, true));
export const doLocalCollectionCreate = (params: CollectionLocalCreateParams, cb?: (id: any) => void) => (dispatch: Dispatch, getState: GetState) => {
  const {
    items,
    sourceId
  } = params;
  ...
  if (sourceId) {
    ...
    const sourceDescription = sourceCollection.description || makeSelectMetadataItemForUri(sourceCollectionClaim?.canonical_url, 'description')(state);
...
export const doCollectionDelete = (collectionId: string, collectionKey: string | null | undefined = undefined, keepPrivate: boolean = false) => (dispatch: Dispatch, getState: GetState) => {
  const state = getState();
...
const doFetchCollectionItems = (items: Array<any>, pageSize?: number) => async (dispatch: Dispatch, getState: GetState) => {
  ...
  try {
    ...
    if (itemsWereFetching) {
      ...
      if (items.some(item => failedItems.includes(item))) {// itemsWereFetching stays true
...
export const doFetchItemsInCollection = (params: {
  ...
  if (!isPrivate && hasClaim === undefined) {
    await dispatch(doResolveClaimId(collectionId, true, {
  ...
  if (isPrivate) {
    ...
    const newPrivateCollection = { ...collection,
      items: newItems,
      key: collectionKey
    };
    return dispatch({
    ...
    const newStoreCollectionClaim = { ...collection,
...
export const doFetchThumbnailClaimsForCollectionIds = (params: {
  collectionIds: Array<any>;
  pageSize?: number;
}) => async (dispatch: Dispatch, getState: GetState) => {
  let state = getState();
  const {
  ...
  return await dispatch(doFetchCollectionItems(Array.from(allClaimIds), pageSize)).finally(() => dispatch({
    type: ACTIONS.COLLECTION_THUMBNAIL_CLAIMS_RESOLVE_COMPLETE,
    data: collectionIdsStr
  }));
};
export const doSortCollectionByKey = (collectionId: string, sortByKey: string, sortOrder: string) => async (dispatch: Dispatch, getState: GetState) => {
  let state = getState();
...
export const doCollectionEdit = (collectionId: string, params: CollectionEditParams) => async (dispatch: Dispatch, getState: GetState) => {
  let state = getState();
  ...
  const fallbackItems = collection && collection.items ? collection.items.filter(item => typeof item === 'string') : [];
  ...
  return new Promise(success => {
    dispatch({
      ...
      data: {
        collectionKey: isPreview ? COLS.KEYS.UNSAVED_CHANGES : isPublic ? COLS.KEYS.EDITED : selectCollectionKeyForId(state, collectionId),
        collection: { ...collection,
          items: newItems,
...
export const doClearQueueList = () => (dispatch: Dispatch, getState: GetState) => dispatch({
