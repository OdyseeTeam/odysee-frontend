...
import { selectMembershipMineFetching, selectIsMembershipListFetchingForId, selectFetchingIdsForMembershipChannelId, selectIsListingAllMyTiers, selectIsClaimMembershipTierFetchingForId, selectMembershipTiersForCreatorId, selectChannelMembershipsForCreatorId } from "redux/selectors/memberships";
...
export const doFetchChannelMembershipsForChannelIds = (channelId: string, channelIds: ClaimIds) => async (dispatch: Dispatch, getState: GetState) => {
  ...
  const channelsToFetch = dedupedChannelIds.filter(dedupedChannelId => {
    ...
    const alreadyFetched = creatorMemberships && (creatorMemberships[dedupedChannelId] || creatorMemberships[dedupedChannelId] === null);
  ...
  dispatch({
    type: ACTIONS.CHANNEL_MEMBERSHIP_CHECK_STARTED,
    data: {
      channel: channelId,
      ids: channelsToFetch
    }
  });
  dispatch(doChannelsHavePremium(channelsToFetch));
  return await Lbryio.call('membership_v2', 'check', {
    ...
    type: ACTIONS.CHANNEL_MEMBERSHIP_CHECK_FAILED,
    data: {
      channelId,
      error: e
    }
...
export const doFetchOdyseeMembershipForChannelIds = (channelIds: ClaimIds) => async (dispatch: Dispatch) => dispatch(doFetchChannelMembershipsForChannelIds(ODYSEE_CHANNEL.ID, channelIds));
// list available memberships for a channel id
export const doMembershipList = (params: MembershipListParams, forceUpdate: boolean | null | undefined) => async (dispatch: Dispatch, getState: GetState) => {
  const {
  ...
  dispatch({
    type: ACTIONS.MEMBERSHIP_LIST_START,
    data: channelId
  });
  return await Lbryio.call('membership_v2', 'list', { ...params
  }, 'post').then((response: MembershipSubs) => dispatch({
    type: ACTIONS.MEMBERSHIP_LIST_COMPLETE,
    data: {
      channelId,
      list: response
    }
...
export const doMembershipMine = () => async (dispatch: Dispatch, getState: GetState) => {
  ...
  dispatch({
    type: ACTIONS.GET_MEMBERSHIP_MINE_START
  });
  return await Lbryio.call('membership_v2/subscription', 'list', {}, 'post') // { membership,subscription, perks, payments, current_price }
  .then((response: MembershipSubs) => dispatch({
    type: ACTIONS.GET_MEMBERSHIP_MINE_DATA_SUCCESS,
    data: response
  })).catch(err => {
    dispatch({
      type: ACTIONS.GET_MEMBERSHIP_MINE_DATA_FAIL,
      data: err
    });
  });
};
...
export const doMembershipBuy = (membershipParams: MembershipBuyParams) => async (dispatch: Dispatch, getState: GetState) => {
  ...
  const {
    tippedChannelId,
    subscriberChannelId,
    // current url channel id
    priceId,
    // memberships.membershipList~.<channelid>[0].prices[n].id
    membershipId
  } = membershipParams;
  ...
  if (!priceId) return;
  dispatch({
    type: ACTIONS.SET_MEMBERSHIP_BUY_STARTED,
    data: membershipId
  });
  let subscribeToken = '';
  let payeeAddress = '';
  ...
  try {
    ...
    await Lbryio.call('membership_v2', 'subscribe', { ...subscribeApiParams
    }, 'post').then((response: MembershipSubscribeResponse) => {
      const {
        token,
        payee_address,
        price
    ...
    if (currencyType === 'AR') {
      ...
      const {
        transactionId: txid,
        error
      } = await sendWinstons(payeeAddress, cryptoAmount, tags);
    ...
    await dispatch(doMembershipMine());
    dispatch({
      type: ACTIONS.SET_MEMBERSHIP_BUY_SUCCESFUL,
      data: membershipId
    });
  } catch (error) {
    dispatch({
      type: ACTIONS.SET_MEMBERSHIP_BUY_FAILED,
      data: {
        id: membershipId,
...
export const doMembershipBuyClear = () => (dispatch: Dispatch) => {
  dispatch({
    type: ACTIONS.SET_MEMBERSHIP_BUY_CLEAR
  });
};
export const doMembershipCancelForMembershipId = (membershipId: number, revert?: boolean) => async (dispatch: Dispatch) => {
  dispatch({
    type: ACTIONS.SET_MEMBERSHIP_CANCEL_STARTED,
    data: membershipId
  });
  return await Lbryio.call('membership_v2/subscription', 'cancel', {
    membership_id: membershipId,
    revert
  }, 'post').then(response => {
    dispatch({
      type: ACTIONS.SET_MEMBERSHIP_CANCEL_SUCCESFUL,
      data: membershipId
    });
    ...
    if (e.message === 'this subscription is no longer active') {
      ...
      dispatch({
        type: ACTIONS.SET_MEMBERSHIP_CANCEL_FAILED,
        data: membershipId
      });
      throw new Error(e);
    }
...
export const doMembershipFetchIncomingPayments = () => async (dispatch: Dispatch) => {
  // start
  dispatch({
    type: ACTIONS.MEMBERSHIP_TX_INCOMING_STARTED
  });
...
export const doMembershipFetchOutgoingPayments = () => async (dispatch: Dispatch) => {
  // start
  dispatch({
    type: ACTIONS.MEMBERSHIP_TX_OUTGOING_STARTED
  });
...
export const doMembershipAddTier = (params: MembershipAddTierParams) => async (dispatch: Dispatch) => await Lbryio.call('membership_v2', 'create', { ...params
}, 'post').then((response: MembershipCreateResponse) => ({
  response: response
})).catch(e => {
  return {
    error: e?.message || e
...
export const doMembershipUpdateTier = (params: MembershipUpdateTierParams) => async (dispatch: Dispatch) => await Lbryio.call('membership_v2', 'update', { ...params
}, 'post').then((response: MembershipCreateResponse) => ({
  response: response
})).catch(e => {
  return {
...
export const doGetMembershipPerks = (params: MembershipListParams) => async (dispatch: Dispatch) => await Lbryio.call('membership_perk', 'list', { ...params,
  environment: stripeEnvironment
}, 'post').then((response: MembershipDetails) => dispatch({
  type: ACTIONS.MEMBERSHIP_PERK_LIST_COMPLETE,
  data: response
})).catch(e => e);
export const doOpenCancelationModalForMembership = (membershipSub: MembershipSub) => (dispatch: Dispatch, getState: GetState) => {
  const {
    membership,
    subscription
  } = membershipSub;
  ...
  const {
    name: channelName
  } = selectClaimForId(state, membership.channel_claim_id);
  ...
  if (isCanceled) {
    return dispatch(doOpenModal(MODALS.CONFIRM, {
      title: __('Confirm Restoring %membership_name% Membership', {
      ...
      onConfirm: (closeModal, setIsBusy) => {
        ...
        dispatch(doMembershipCancelForMembershipId(membership.id, true)).then(() => {
          setIsBusy(false);
          dispatch(doToast({
            message: __('Your membership was successfully restored.')
          }));
          closeModal();
        });
      }
  ...
  return dispatch(doOpenModal(MODALS.CONFIRM, {
    title: __('Confirm %membership_name% Cancellation', {
      membership_name: membership.name
    }),
    subtitle: __('Are you sure you want to cancel your %creator_name%\'s "%membership_name%" membership? ' + 'You will still have all your features until %end_date% at which point your purchase will not be renewed ' + 'and you will lose access to your membership features and perks.', {
    ...
    onConfirm: (closeModal, setIsBusy) => {
      ...
      dispatch(doMembershipCancelForMembershipId(membership.id)).then(() => {
        setIsBusy(false);
        dispatch(doToast({
          message: __('Your membership was successfully cancelled and will no longer be renewed.')
        }));
        closeModal();
      });
    }
...
export const doDeactivateMembershipForId = (membershipId: number) => async (dispatch: Dispatch) => {
  dispatch({
    type: ACTIONS.DELETE_MEMBERSHIP_STARTED,
    data: membershipId
  });
  await Lbryio.call('membership_v2', 'status_set', {
    membership_id: membershipId,
    enabled: false
  }, 'post').then(response => {
    dispatch({
...
export const doGetMembershipTiersForChannelClaimId = (channelClaimId: string) => async (dispatch: Dispatch) => {
  dispatch({
    type: ACTIONS.GET_MEMBERSHIP_TIERS_FOR_CHANNEL_STARTED,
    data: channelClaimId
  });
  ...
  await Lbryio.call('membership', 'content', {
    ...
    dispatch({
      type: ACTIONS.GET_MEMBERSHIP_TIERS_FOR_CHANNEL_SUCCESS,
      data: response
    });
    return response;
  }).catch(e => {
    dispatch({
      type: ACTIONS.GET_MEMBERSHIP_TIERS_FOR_CHANNEL_FAILED,
      data: channelClaimId
    });
    return e;
  });
};
export const doMembershipContentForStreamClaimIds = (contentClaimIds: ClaimIds) => async (dispatch: Dispatch, getState: GetState) => {
  const state = getState();
  ...
  dispatch({
    type: ACTIONS.GET_CLAIM_MEMBERSHIP_TIERS_START,
    data: idsToFetch
  });
  await Lbryio.call('membership_v2/member_content', 'resolve', {
    claim_ids: claimIdsCsv
  }, 'post').then((response: MembershipContentResponse) => {
    dispatch({
      type: ACTIONS.GET_CLAIM_MEMBERSHIP_TIERS_SUCCESS,
      data: {
        idsToFetch,
        response
      }
    ...
    dispatch({
      type: ACTIONS.GET_CLAIM_MEMBERSHIP_TIERS_FAIL,
      data: idsToFetch
    });
    return e;
  });
};
export const doMembershipContentforStreamClaimId = (contentClaimId: string) => async (dispatch: Dispatch, getState: GetState) => {
  const state = getState();
...
export const doSaveMembershipRestrictionsForContent = (channelClaimId: string, contentClaimId: string, contentClaimName: string, memberRestrictionTierIds: Array<number>, pendingClaim: boolean | null | undefined) => async (dispatch: Dispatch) => {
  dispatch({
    type: ACTIONS.SET_MEMBERSHIP_TIERS_FOR_CONTENT_STARTED,
    data: {
  ...
  await Lbryio.call('membership_v2/member_content', 'modify', {
    claim_id: contentClaimId,
    membership_ids: memberRestrictionTierIds.slice().sort((a, b) => a - b).join(','),
    ...
    if (memberRestrictionTierIds?.length > 0) {
      dispatch(doToast({
        isError: true,
        message: __(__('Failed to apply membership tiers. Access to the content may still be restricted.') + ' Error: ' + (e?.message || e) + '\n' + __('If the issue persists, please reach out to help@odysee.com'))
      }));
    }
...
export const doMembershipClearData = () => async (dispatch: Dispatch) => await Lbryio.call('membership', 'clear', {
  environment: 'test'
}, 'post').then(() => dispatch(doMembershipMine()));
// TODO get (paginate?) historical only_active = false
export const doGetMembershipSupportersList = () => async (dispatch: Dispatch) => Lbryio.call('membership_v2', 'subscribers', {
  only_active: true
}, 'post').then((response: SupportersList) => {
  dispatch({
    type: ACTIONS.GET_MEMBERSHIP_SUPPORTERS_LIST_COMPLETE,
    data: response
  });
}).catch(() => dispatch({
  type: ACTIONS.GET_MEMBERSHIP_SUPPORTERS_LIST_COMPLETE,
...
export const doListAllMyMembershipTiers = () => async (dispatch: Dispatch, getState: GetState) => {
  ...
  dispatch({
    type: ACTIONS.LIST_ALL_MY_MEMBERSHIPS_START
  });
  ...
  myChannelClaims.map((channelClaim, index) => {
    pendingPromises[index] = dispatch(doMembershipList({
      channel_claim_id: channelClaim.claim_id
    }));
  });
  return await Promise.all(pendingPromises).then(responseList => {
    dispatch({
      type: ACTIONS.LIST_ALL_MY_MEMBERSHIPS_COMPLETE
    });
    return responseList;
  }).catch(e => {
    dispatch({
