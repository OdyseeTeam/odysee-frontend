...
import { selectNotifications, selectNotificationsFiltered, selectNotificationCategories } from "redux/selectors/notifications";
  ...
  return async (dispatch: Dispatch) => {
    ...
    try {
      return Lbryio.call('notification', 'list', params).then(response => {
        ...
        if (resolve) {
          const channelsToResolve = notifications.filter((notification: WebNotification) => {
            if (notification.notification_parameters.dynamic && notification.notification_parameters.dynamic.comment_author || notification.notification_rule === RULE.NEW_CONTENT) {
              return true;
            } else {
              return false;
            }
      ...
      dispatch({
        type: ACTIONS.NOTIFICATION_LIST_FAILED,
        data: {
          error
        }
      });
...
export function doReadNotifications(notificationsIds: Array<number>) {
  return (dispatch: Dispatch, getState: GetState) => {
    ...
    return Lbryio.call('notification', 'edit', {
      notification_ids: ids.join(','),
      is_read: true
    }).then(() => {
      dispatch({
        type: ACTIONS.NOTIFICATION_READ_COMPLETED,
        data: {
          notificationIds: ids
        }
      });
      ...
      dispatch({
        type: ACTIONS.NOTIFICATION_READ_FAILED,
        data: {
          error
        }
      });
...
export function doSeeNotifications(notificationIds: Array<string>) {
  return (dispatch: Dispatch) => {
    ...
    return Lbryio.call('notification', 'edit', {
      ...
      dispatch({
        type: ACTIONS.NOTIFICATION_SEEN_FAILED,
        data: {
          error
        }
      });
...
export function doDeleteNotification(notificationId: number) {
  return (dispatch: Dispatch) => {
    Lbryio.call('notification', 'delete', {
      ...
      dispatch({
        type: ACTIONS.NOTIFICATION_DELETE_COMPLETED,
        data: {
          notificationId
        }
      });
      ...
      dispatch(doToast({
        isError: true,
        message: __('Unable to delete this right now. Please try again later.')
      }));
    });
  };
}
