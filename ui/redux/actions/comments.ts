...
import { selectClaimForUri, selectClaimsById, selectClaimsByUri, selectMyChannelClaims, selectClaimForClaimId, selectProtectedContentTagForUri } from "redux/selectors/claims";
import { doResolveUris, doClaimSearch, doResolveClaimIds } from "redux/actions/claims";
import { doToast, doSeeNotifications } from "redux/actions/notifications";
import { selectMyReactsForComment, selectOthersReactsForComment, selectPendingCommentReacts, selectModerationBlockList, selectModerationDelegatorsById, selectMyCommentedChannelIdsForId, selectLivestreamChatMembersOnlyForChannelId, selectMembersOnlyCommentsForChannelId, selectSettingsForChannelId } from "redux/selectors/comments";
...
const MENTION_REGEX = /(?:^| |\n)@[^\s=&#$@%?:;/"<>%{}|^~[]*(?::[\w]+)?/gm;
export function doCommentList(uri: string, parentId: string | null | undefined, page: number = 1, pageSize: number = 99999, sortBy: number | null | undefined = SORT_BY.NEWEST, isLivestream?: boolean) {
  return async (dispatch: Dispatch, getState: GetState) => {
    ...
    if (isProtected) {
      if (!myChannelClaims) {
        return dispatch({
          type: ACTIONS.COMMENT_LIST_FAILED,
          data: __('Failed to fetch channel list.')
        });
      }
      ...
      if (!myChannelClaim) {
        return dispatch({
          type: ACTIONS.COMMENT_LIST_FAILED,
          data: __('You do not own this channel.')
        });
      }
    ...
    return Comments.comment_list({
      ...
      ...(isProtected ? {
        ...
        requestor_channel_id: activeChannelId,
        // typo (requestor vs requester) is on backend atm
...
export function doCommentListOwn(channelId: string, page: number = 1, pageSize: number = 10, sortBy: number = SORT_BY.NEWEST_NO_PINS) {
      ...
      dispatch(doClaimSearch({
        ...
        dispatch({
          ...
          data: {
            ...
            totalFilteredItems: total_filtered_items,
            totalPages: total_pages,
            uri: channelClaim.canonical_url,
            // hijack Discussion Page ¹
  ...
  return (dispatch: Dispatch, getState: GetState) => {
    ...
    return Comments.comment_by_id({
      ...
      dispatch({
        ...
        data: {
          comment: item || items,
          // Requested a change to rename it to 'item'. This covers both.
          ancestors: ancestors
        }
      });
      return result;
...
export function doFetchMyCommentedChannels(claimId: string | null | undefined) {
  return (dispatch: Dispatch, getState: GetState) => {
    ...
    return Promise.all(myChannelClaims.map(x => ChannelSign.sign(x.claim_id, x.name, true))).then(signatures => {
      ...
      // $FlowFixMe
      return Promise.allSettled(params.map(p => Comments.comment_list(p))).then(response => {
...
export function doCommentReactList(commentIds: Array<string>) {
  return async (dispatch: Dispatch, getState: GetState) => {
    ...
    return Comments.reaction_list(params).then((result: ReactionListResponse) => {
      const {
...
function doFetchAllReactionsForId(commentIds: Array<string>, channelClaims: Array<Claim> | null | undefined) {
  ...
  return Promise.all(channelClaims.map(x => ChannelSign.sign(x.claim_id, x.name, true))).then(channelSignatures => {
    const params = [];
...
async function getReactedChannelNames(commentId: string, myChannelClaims: Array<Claim> | null | undefined) {
...
export function doCommentReact(commentId: string, type: string) {
  return async (dispatch: Dispatch, getState: GetState) => {
    ...
    dispatch({
      ...
      data: {
        myReactions: {
          [reactKey]: myReactsObj
        },
        othersReactions: {
          [reactKey]: othersReacts
        }
      }
    });
    ...
    if (checkIfAlreadyReacted) {
      ...
      if (!reactedChannelNames) {
        ...
        dispatch(doToast({
          message: __('Unable to react. Please try again later.'),
          isError: true
        }));
        rejectReaction = true;
    ...
    new Promise((res, rej) => rejectReaction ? rej('') : res(true)).then(() => {
      ...
      const myRevertedReactsObj = myReacts.filter(el => el !== type).reduce((acc, el) => {
        acc[el] = 1;
        return acc;
      }, {});
      dispatch({
...
export function doCommentCreate(uri: string, livestream: boolean, params: CommentSubmitParams) {
  return async (dispatch: Dispatch, getState: GetState) => {
    ...
    if (myCommentedChannelIds === undefined) {
      dispatchToast(dispatch, __('Failed to perform action.'), __('Please wait a while before re-submitting, or try refreshing the page.'), 'long');
      return;
    }
    ...
    if (previousCommenterChannel && previousCommenterChannel.claim_id !== activeChannelClaim.claim_id && previousCommenterChannel.last_comment_timestamp >= channelSwitchCutoffTimestamp && myCommentedChannelIds && !myCommentedChannelIds.includes(activeChannelClaim.claim_id)) {
      dispatchToast(dispatch, __('Commenting from multiple channels is not allowed.'), previousCommenterChannel.name, 'long');
      return;
    }
    ...
    if (myCommentedChannelIds && myCommentedChannelIds.length) {
      if (!myCommentedChannelIds.includes(activeChannelClaim.claim_id)) {
        ...
        dispatchToast(dispatch, __('Commenting from multiple channels is not allowed.'), commentedChannelNames.join(' • '), 'long');
        return;
      }
    }
    ...
    if (mentionMatches.length > 0) {
      ...
      if (mentionUrls.length > 0) {
        await dispatch(doResolveUris(mentionUrls, true)).then(response => {
...
export function doCommentPin(commentId: string, claimId: string, remove: boolean) {
  return async (dispatch: Dispatch, getState: GetState) => {
    ...
    return Comments.comment_pin(params).then((result: CommentPinResponse) => {
      dispatch({
 ...
 */
export function doCommentAbandon(commentId: string, deleterClaim?: Claim, deleterIsModOrAdmin?: boolean, creatorClaim?: Claim) {
  return async (dispatch: Dispatch, getState: GetState) => {
    if (!deleterClaim) {
      const state = getState();
  ...
  if (comment === '') {
    ...
    return async (dispatch: Dispatch, getState: GetState) => {
      ...
      if (!activeChannelClaim) {
        return dispatch(doToast({
          isError: true,
          message: __('No active channel selected.')
        }));
      }
...
// ChannelSign
// ****************************************************************************
type ChannelSignCache = Record<ChannelId, Record<string, {
    ...
    try {
      if (useCache) {
        if (ChannelSign._isCacheValid(channelClaimId, data)) {
          ...
          // Store
          ChannelSign._cache[channelClaimId] = { ...ChannelSign._cache[channelClaimId],
            [data]: {
...
function doCommentModToggleBlock(unblock: boolean, commenterUri: string, creatorUri: string, blockerIds: Array<string>, // [] = use all my channels
blockLevel: string, timeoutSec: number | null | undefined, showLink: boolean = false, offendingCommentId: string | null | undefined = undefined) {
  return async (dispatch: Dispatch, getState: GetState) => {
    const state = getState();
    ...
    const {
      channelName,
      channelClaimId
    } = parseURI(commenterUri);
    const {
      channelName: creatorName,
      channelClaimId: creatorId
    } = safeParseURI(creatorUri);
        ...
        case BLOCK_LEVEL.MODERATOR:
          {
            ...
            blockerChannelClaims = [blockerChannelClaims.find(x => {
        ...
        case BLOCK_LEVEL.ADMIN:
          {
            ...
            const delegatorsById = selectModerationDelegatorsById(state);
            blockerChannelClaims = [blockerChannelClaims.find(x => delegatorsById[x.claim_id] && delegatorsById[x.claim_id].global)];
          }
          break;
      }
    } else {
    ...
    return Promise.all(blockerChannelClaims.map(x => channelSignName(x.claim_id, x.name, true))).then(response => {
      channelSignatures = response;
      // $FlowFixMe
      return Promise.allSettled(channelSignatures.filter(x => x !== undefined && x !== null).map(signatureData => commentAction({
        // $FlowFixMe
        ...
        dispatch(doToast({
          message: unblock ? __('Channel unblocked!') : __('Channel "%channel%" blocked.', {
            channel: commenterNameForAction
          }),
 ...
 * @returns {function(Dispatch): *}
 */
export function doCommentModBlock(commenterUri: string, offendingCommentId: string | null | undefined, timeoutSec: number | null | undefined, showLink: boolean = true) {
  return (dispatch: Dispatch) => {
    return dispatch(doCommentModToggleBlock(false, commenterUri, '', [], BLOCK_LEVEL.SELF, timeoutSec, showLink, offendingCommentId));
  };
}
 ...
 * @returns {function(Dispatch): *}
 */
export function doCommentModBlockAsAdmin(commenterUri: string, offendingCommentId: string | null | undefined, blockerId: string | null | undefined, timeoutSec: number | null | undefined) {
  return (dispatch: Dispatch) => {
    return dispatch(doCommentModToggleBlock(false, commenterUri, '', blockerId ? [blockerId] : [], BLOCK_LEVEL.ADMIN, timeoutSec, false, offendingCommentId));
  };
}
 ...
 * @returns {function(Dispatch): *}
 */
export function doCommentModBlockAsModerator(commenterUri: string, offendingCommentId: string | null | undefined, creatorUri: string, blockerId: string | null | undefined, timeoutSec: number | null | undefined) {
  return (dispatch: Dispatch) => {
    return dispatch(doCommentModToggleBlock(false, commenterUri, creatorUri, blockerId ? [blockerId] : [], BLOCK_LEVEL.MODERATOR, timeoutSec, false, offendingCommentId));
  };
}
...
export function doCommentModUnBlockAsModerator(commenterUri: string, creatorUri: string, blockerId: string) {
  return (dispatch: Dispatch) => {
    return dispatch(doCommentModToggleBlock(true, commenterUri, creatorUri, blockerId ? [blockerId] : [], BLOCK_LEVEL.MODERATOR));
  };
}
export function doFetchModBlockedList() {
  return async (dispatch: Dispatch, getState: GetState) => {
    ...
    let channelSignatures = [];
    return Promise.all(myChannels.map(channel => channelSignName(channel.claim_id, channel.name, true))).then(response => {
      channelSignatures = response;
      // $FlowFixMe
      return Promise.allSettled(channelSignatures.filter(x => x !== undefined && x !== null).map(signatureData => Comments.moderation_block_list({
        mod_channel_id: signatureData.claim_id,
        mod_channel_name: signatureData.name,
        signature: signatureData.signature,
        signing_ts: signatureData.signing_ts
        ...
        for (let i = 0; i < blockListsPerChannel.length; ++i) {
          ...
          await storeList(delegated_blocked_channels, moderatorBlockList, moderatorTimeoutMap, moderatorBlockListDelegatorsMap);
        }

        dispatch({
          type: ACTIONS.COMMENT_MODERATION_BLOCK_LIST_COMPLETED,
          data: {
            personalBlockList: personalBlockList.length > 0 ? personalBlockList.sort((a, b) => new Date(a.blockedAt) - new Date(b.blockedAt)).map(blockedChannel => blockedChannel.channelUri) : null,
            adminBlockList: adminBlockList.length > 0 ? adminBlockList.sort((a, b) => new Date(a.blockedAt) - new Date(b.blockedAt)).map(blockedChannel => blockedChannel.channelUri) : null,
            moderatorBlockList: moderatorBlockList.length > 0 ? moderatorBlockList.sort((a, b) => new Date(a.blockedAt) - new Date(b.blockedAt)).map(blockedChannel => blockedChannel.channelUri) : null,
...
export const doUpdateBlockListForPublishedChannel = (channelClaim: ChannelClaim) => {
  return async (dispatch: Dispatch, getState: GetState) => {
    ...
    return Promise.all(blockedUris.map(uri => {
      const {
        channelName,
        channelClaimId
      } = parseURI(uri);
...
export function doCommentModAddDelegate(modChannelId: string, modChannelName: string, creatorChannelClaim: ChannelClaim, showToast: boolean = false) {
  return async (dispatch: Dispatch, getState: GetState) => {
    ...
    return Comments.moderation_add_delegate({
      ...
      dispatch({
        ...
        data: {
          newDelegates: res?.Delegates,
          creatorChannelId: creatorChannelClaim.claim_id
        }
      });
...
export function doCommentModRemoveDelegate(modChannelId: string, modChannelName: string, creatorChannelClaim: ChannelClaim, showToast: boolean = false) {
  return async (dispatch: Dispatch, getState: GetState) => {
    const signature = await ChannelSign.sign(creatorChannelClaim.claim_id, creatorChannelClaim.name, false);
    ...
    return Comments.moderation_remove_delegate({
      ...
      dispatch({
        ...
        data: {
          removedDelegateId: modChannelId,
          creatorChannelId: creatorChannelClaim.claim_id
        }
      });
...
export function doCommentModListDelegatesForMyChannels() {
  return async (dispatch: Dispatch, getState: GetState) => {
    ...
    return Promise.all(myChannels.map(channel => channelSignName(channel.claim_id, channel.name, true))).then(response => {
      channelSignatures = response;
      // $FlowFixMe
      return Promise.allSettled(channelSignatures.filter(x => x !== undefined && x !== null).map(signatureData => Comments.moderation_list_delegates({
...
export function doFetchCommentModAmIList(channelClaim: ChannelClaim) {
  return async (dispatch: Dispatch, getState: GetState) => {
    ...
    return Promise.all(myChannels.map(channel => channelSignName(channel.claim_id, channel.name, true))).then(response => {
      channelSignatures = response;
      // $FlowFixMe
      return Promise.allSettled(channelSignatures.filter(x => x !== undefined && x !== null).map(signatureData => Comments.moderation_am_i({
        channel_name: signatureData.name,
        channel_id: signatureData.claim_id,
        signature: signatureData.signature,
        signing_ts: signatureData.signing_ts
...
export const doDeleteChannelSection = (channelId: string, sectionId: string) => {
  return async (dispatch: Dispatch, getState: GetState) => {
    ...
    const sections: Sections | null | undefined = channelSettings && channelSettings.channel_sections;
...
export const doToggleMembersOnlyCommentsSettingForClaimId = (claimId: ClaimId) => async (dispatch: Dispatch, getState: GetState) => {
  const state = getState();
  const claim = selectClaimForClaimId(state, claimId);
  const {
    name: channelName,
    claim_id: channelId
  } = getChannelFromClaim(claim) || {};
...
export const doToggleLiveChatMembersOnlySettingForClaimId = (claimId: ClaimId) => async (dispatch: Dispatch, getState: GetState) => {
  const state = getState();
  const claim = selectClaimForClaimId(state, claimId);
  const {
    name: channelName,
  ...
  return Comments.setting_update({
    ...
    data: {
      responseData: {
        LivestreamChatMembersOnly: value
      },
      creatorId: channelId
    }
  })).catch(err => {
...
export const doFetchBlockedWords = () => {
  return async (dispatch: Dispatch, getState: GetState) => {
    ...
    if (myChannels) {
      for (const channelClaim of myChannels) {
        try {
          ...
          channelSignatures.push({ ...channelSignature,
    ...
    return Promise.all(channelSignatures.map(signatureData => Comments.setting_list_blocked_words({
      channel_name: signatureData.name,
      channel_id: signatureData.claim_id,
      signature: signatureData.signature,
      signing_ts: signatureData.signing_ts
