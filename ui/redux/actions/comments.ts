...
export function doCommentList(
  ...
  sortBy: number | null | undefined = SORT_BY.NEWEST,
...
function doCommentModToggleBlock(
  ...
  offendingCommentId: string | null | undefined = undefined
) {
  return async (dispatch: Dispatch, getState: GetState) => {
    const state = getState();
    ...
    if (index > -1 && channelClaim) {
      ...
      dispatch(
        doUpdateCreatorSettings(channelClaim, {
          channel_sections: { ...sections, entries },
        })
      );
    } // TODO: errors?
  };
