...
import { selectAccountCheckIsFetchingForId, selectCustomerStatusFetching, selectAccountStatusFetching } from "redux/selectors/stripe";
...
export const doTipAccountCheckForUri = (uri: string) => async (dispatch: Dispatch, getState: GetState) => {
  ...
  return await Lbryio.call('account', 'check', {
    channel_claim_id: channelClaimId,
    channel_name: channelName,
    environment: stripeEnvironment,
    v2: true
  }, 'post').then(accountCheckResponse => dispatch({
...
export const doTipAccountStatus = () => async (dispatch: Dispatch, getState: GetState) => {
  ...
  return await Lbryio.call('account', 'status', {
    environment: stripeEnvironment,
    v2: true
  }, 'post').then((accountStatusResponse: StripeAccountStatus | AccountStatus) => {
    dispatch({
    ...
    dispatch(doToast({
      message: __('There was an error getting your account setup, please try again later'),
      isError: true
    }));
    dispatch({
      type: ACTIONS.STRIPE_ACCOUNT_STATUS_COMPLETE,
...
export const doTipAccountRemove = () => async (dispatch: Dispatch, getState: GetState) => {
  return await Lbryio.call('account', 'remove', {
    environment: stripeEnvironment
  }, 'post').then(() => dispatch(doTipAccountStatus())).catch(error => {
    dispatch(doToast({
...
export const doGetAndSetAccountLink = () => async (dispatch: Dispatch) => {
  ...
  return await Lbryio.call('account', 'link', {
    return_url: currentUrl,
    refresh_url: currentUrl,
    environment: stripeEnvironment
  }, 'post').then((accountLinkResponse: StripeAccountLink) => {
    dispatch({
      type: ACTIONS.SET_ACCOUNT_LINK,
      data: accountLinkResponse
    });
    return accountLinkResponse;
  });
};
export const doCustomerListPaymentHistory = () => async (dispatch: Dispatch) => await Lbryio.call('customer', 'list', {
  environment: stripeEnvironment
}, 'post').then((customerTransactionResponse: StripeTransactions) => {
  dispatch({
...
export const doCheckIfPurchasedClaimId = (claimId: string) => async (dispatch: Dispatch) => {
  ...
  return await Lbryio.call('customer', 'list', {
    environment: stripeEnvironment,
    claim_id_filter: claimId
  }, 'post').then(response => dispatch({
...
export const doCheckIfPurchasedClaimIds = (claimIds: ClaimIds) => {
  ...
  return (dispatch: Dispatch) => {
    ...
    return Lbryio.call('customer', 'list', params, 'post').then(response => dispatch({
      type: ACTIONS.CHECK_IF_PURCHASED_COMPLETED,
      data: response
    })).catch(e => dispatch({
      type: ACTIONS.CHECK_IF_PURCHASED_FAILED,
...
export const doCustomerPurchaseCost = (cost: number) => (dispatch: Dispatch): Promise<StripeCustomerPurchaseCostResponse> => {
  return Lbryio.call('customer', 'purchase_cost', {
    environment: stripeEnvironment,
    amount: cost
  });
};
export const doListAccountTransactions = () => async (dispatch: Dispatch) => await Lbryio.call('account', 'list', {
  environment: stripeEnvironment
}, 'post').then((accountListResponse: StripeTransactions) => {
...
export const doGetCustomerStatus = () => async (dispatch: Dispatch, getState: GetState) => {
  ...
  if (isFetching) return;
  dispatch({
    type: ACTIONS.SET_CUSTOMER_STATUS_STARTED
  });
  return await Lbryio.call('customer', 'status', {
    environment: stripeEnvironment
  }, 'post').then((customerStatusResponse: StripeCustomerStatus) => {
    dispatch({
    ...
    if (error === 'internal_apis_down') {
      dispatch(doToast({
        message: __(STRIPE.APIS_DOWN_ERROR_RESPONSE),
        isError: true
      }));
    } else if (error.message === 'user as customer is not setup yet') {// probably doesn't need to do a toast message for this
    } else {
      // probably an error from stripe
      dispatch(doToast({
        message: __(STRIPE.CARD_SETUP_ERROR_RESPONSE),
        isError: true
      }));
    }
...
export const doCustomerSetup = () => async (dispatch: Dispatch) => await Lbryio.call('customer', 'setup', {
  environment: stripeEnvironment
}, 'post').then((customerSetupResponse: StripeCustomerSetupResponse) => {
  dispatch({
    type: ACTIONS.SET_CUSTOMER_SETUP_RESPONSE,
    data: customerSetupResponse
  });
  return customerSetupResponse;
});
export const doCustomerRemove = () => async (dispatch: Dispatch) => await Lbryio.call('customer', 'remove', {
  environment: stripeEnvironment
}, 'post').then(() => dispatch(doGetCustomerStatus())).catch(error => {
  dispatch(doToast({
    message: error.message || error,
    isError: true
  }));
...
export const doRemoveCardForPaymentMethodId = (paymentMethodId: string) => async (dispatch: Dispatch) => await Lbryio.call('customer', 'detach', {
  environment: stripeEnvironment,
  payment_method_id: paymentMethodId
}, 'post').then(() => dispatch(doGetCustomerStatus()));
...
const updateAddressStatus = async (id: string, status: string) => {
  try {
    const res = await Lbryio.call('arweave/address', 'update', {
      id,
      status
...
export const doRegisterArweaveAddress = (address: string, makeDefault: boolean) => async (dispatch: Dispatch) => {
  dispatch({
    type: ACTIONS.AR_ADDR_REGISTER_STARTED,
    data: address
  });
  ...
  try {
    ...
    dispatch({
      type: ACTIONS.AR_ADDR_REGISTER_SUCCESS,
      data: address
    });
  } catch (e) {
    console.error(e);
    ...
    if (e.message === 'address already exists for another user') {
      ...
      dispatch({
        type: ACTIONS.AR_ADDR_REGISTER_ERROR,
        data: e.message
      });
      dispatch({
        type: ACTIONS.ARCONNECT_FAILURE,
        data: {
          error: e.message
        }
      });
    ...
    dispatch(doOpenModal(MODALS.ERROR, {
      error: e.message || e
    }));
    dispatch({
      type: ACTIONS.AR_ADDR_REGISTER_ERROR,
...
export const doUpdateArweaveAddressStatus = (id: string, status: "active" | "inactive") => async (dispatch: Dispatch) => {
  ...
  try {
    ...
    await dispatch(doTipAccountStatus());
    dispatch({
      type: ACTIONS.AR_ADDR_UPDATE_SUCCESS,
      data: id
    });
  } catch (e) {
    console.error(e);
    dispatch({
      type: ACTIONS.AR_ADDR_UPDATE_ERROR,
      data: e?.message || e
    });
  }
};
export const doUpdateArweaveAddressDefault = (id: string) => async (dispatch: Dispatch) => {
  dispatch({
    type: ACTIONS.AR_ADDR_DEFAULT_STARTED,
    data: id
  });
  ...
  try {
    ...
    await dispatch(doTipAccountStatus());
    dispatch({
      type: ACTIONS.AR_ADDR_DEFAULT_SUCCESS,
      data: id
    });
  } catch (e) {
    console.error(e);
    dispatch({
      type: ACTIONS.AR_ADDR_DEFAULT_ERROR,
      data: e?.message || e
    });
  }
};