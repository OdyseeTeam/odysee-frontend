...
import { selectIsLiveFetchingForId, selectActiveLivestreamsFetchingForQuery, selectActiveLivestreamsLastFetchedDateForQuery, selectActiveLivestreamsLastFetchedFailCountForQuery } from "redux/selectors/livestream";
...
const doFetchActiveLivestreamsForQuery = (livestreamInfoByCreatorId: LivestreamInfoByCreatorIds, query: {
  any_languages: Array<string> | null | undefined;
} = {
  any_languages: null
  ...
  const activeLivestreamClaims = await dispatch(doClaimSearch({
    page: 1,
    page_size: 50,
    has_no_source: true,
...
export const doFetchAllActiveLivestreamsForQuery = (query: {
  any_languages: Array<string> | null | undefined;
} = {
  any_languages: null
  ...
  return Livestream.call('livestream', 'all').then(async (response: LivestreamAllResponse) => {
    const livestreamInfoByCreatorId: LivestreamInfoByCreatorIds = transformNewLivestreamData(response);
    const activeLivestreamResolvedByCreatorId = await dispatch(doFetchActiveLivestreamsForQuery(livestreamInfoByCreatorId, query));
    dispatch({
      type: ACTIONS.FETCH_ACTIVE_LIVESTREAMS_SUCCESS,
      data: { ...completedParams,
        livestreamInfoByCreatorId: activeLivestreamResolvedByCreatorId
      }
    });
  }).catch(() => dispatch({
    type: ACTIONS.FETCH_ACTIVE_LIVESTREAMS_FAIL,
    data: completedParams
  }));
};
export const doSetIsLivePollingForChannelId = (channelId: string, isPolling: boolean) => (dispatch: Dispatch) => dispatch({
  type: ACTIONS.SET_IS_LIVE_POLLING_FOR_ID,
  data: {
    channelId,
    isPolling
  }
