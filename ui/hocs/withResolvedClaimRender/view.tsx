...
const withResolvedClaimRender = (ClaimRenderComponent: FunctionalComponentParam) => {
  const ResolvedClaimRender = (props: Props) => {
    ...
    const resolveRequired = claim === undefined || claim && claim.value?.fee && claim.purchase_receipt === undefined && isAuthenticated;
    const isVisibilityRestricted = useIsVisibilityRestricted(claim, claimIsMine, isGlobalMod, uriAccessKey, verifyClaimSignature);
    ...
    const resolveClaim = React.useCallback(() => doResolveUri(uri, false, true, isAuthenticated ? {
    ...
    if (!hasClaim) {
      ...
      return <Wrapper>
          <div className="main--empty">
            <Yrbl title={isChannel ? __('Channel Not Found') : __('No Content Found')} subtitle={<div className="section__actions">
                  {!isChannel && <Button button="primary" // label={__(isChannel ? 'Claim this handle' : 'Publish Something')} -- only support non-channels for now
            label={__('Publish Something')} onClick={() => doBeginPublish(PUBLISH_TYPES.FILE, streamName)} />}

                  {!isChannel && <Button button="secondary" label={__('Repost Something')} onClick={() => doOpenModal(MODALS.REPOST, {
              streamName
            })} />}
                </div>} />
          </div>
    ...
    if (claimIsRestricted && geoRestriction === undefined) {
      if (!gblAvailable) {
        return <Wrapper>
            <div className="main--empty">
              <Yrbl title={__('Oops! Something went wrong.')} subtitle={<>
    ...
    if (isVisibilityRestricted !== false) {
      ...
      return <Wrapper>
          <div className="main--empty">
            <Yrbl title={__(isChannel ? 'Channel unavailable' : 'Content unavailable')} subtitle={__('Reach out to the creator to obtain the full URL for access.')} type="sad" alwaysShow />
          </div>
        </Wrapper>;
    }
    ...
    if (claimIsRestricted && isChannel) {
      if (geoRestriction) {
        return <Wrapper>
            <div className="main--empty">
              <Yrbl title={__(isChannel ? 'Channel unavailable' : 'Content unavailable')} subtitle={geoRestriction.message ? __(geoRestriction.message) : ''} type="sad" alwaysShow />
            </div>
          </Wrapper>;
      }
    }
    ...
    if (claimIsRestricted && !isChannel) {
      ...
      if (isClaimBlackListed) {
        return <Wrapper>
            <div className="main--empty">
              <Card title={uri} subtitle={__('In response to a complaint we received under the US Digital Millennium Copyright Act, we have blocked access to this content from our applications. Content may also be blocked due to DMCA Red Flag rules which are obvious copyright violations we come across, are discussed in public channels, or reported to us.')} actions={<div className="section__actions">
      ...
      if (isClaimFiltered) {
        return <Wrapper>
            <div className="main--empty">
              <Card title={uri} subtitle={__('This content violates the terms and conditions of Odysee and has been filtered.')} />
            </div>
          </Wrapper>;
      }
    }

    return <React.Suspense fallback={<LoadingSpinner text={__('Loading...')} />}>
