...
export default function PaidContentOvelay(props: Props) {
  ...
  const {
    icon: fiatIconToUse,
    symbol: fiatSymbol
  } = STRIPE.CURRENCY[preferredCurrency];
  ...
  const ButtonPurchase = React.useMemo(() => {
    return ({
      ...
      return <Button className={'purchase-button' + (sdkFeeRequired ? ' purchase-button--fee' : '')} icon={sdkFeeRequired && !purchaseTag ? ICONS.LBC : fiatIconToUse} button="primary" label={label} requiresAuth {...clickProps} />;
    };
  ...
  return <div className="paid-content-overlay">
      <div className="paid-content-overlay__body">
        <div className="paid-content-prompt paid-content-prompt--overlay">
          ...
          {preorderTag && <>
              {preorderContentClaim ? <Button iconColor="red" className={'preorder-button'} button="primary" label={__('Click here to view your purchased content')} navigate={`/${preorderContentClaim.canonical_url.replace('lbry://', '')}`} /> : <ButtonPurchase label={__('Preorder now for %fiatSymbol%%preorderTag%', {
            fiatSymbol,
            preorderTag
          })} />}
            </>}
