...
export class ShortsPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    // ── Feed container ───────────────────────────────────────────────────────
    this.feedContainer = page.locator('.shorts-feed, [class*="shorts__feed"], [class*="shorts-container"]').first();
    ...
    this.activeSlide = page
      .locator('.shorts__slide--active, [class*="shorts__item--active"], [class*="short-slide--active"]')
      .or(this.slides.first()); // fallback: first slide when no active class exists yet

    // ── Video ────────────────────────────────────────────────────────────────
    this.activePlayer = this.activeSlide.locator('.video-js, [class*="videojs"], .shorts-video-player').first();
    ...
    // ── Actions panel ─────────────────────────────────────────────────────────
    this.actionsPanel = page.locator('.shorts-actions, [class*="shorts-side-panel"], [class*="shortsActions"]').first();
    ...
    this.tipButton = this.actionsPanel
      ...
      .first();
    this.shareButton = this.actionsPanel.locator('[aria-label*="Share"], [class*="share"]').first();
    ...
    this.nextButton = page
      .locator('[aria-label*="Next"], [class*="shorts__next"], [class*="next-short"], button[class*="down"]')
      .first();
    this.prevButton = page
      .locator('[aria-label*="Previous"], [class*="shorts__prev"], [class*="prev-short"], button[class*="up"]')
      .first();
  ...
  async waitForPlayback(timeout = 10_000): Promise<void> {
    await expect.poll(async () => this.isPlaying(), { timeout, message: 'Expected video to start playing' }).toBe(true);
  }
  ...
  async getVideoDuration(): Promise<number | null> {
    const duration = await this.activeVideo.evaluate((video: HTMLVideoElement) => video.duration);
