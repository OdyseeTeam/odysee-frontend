...
export const selectBanStateForUri = createCachedSelector(selectClaimForUri, selectBlackListedData, selectFilteredData, selectMutedChannels, selectModerationBlockList, (claim, blackListedData, filteredData, mutedChannelUris, personalBlocklist) => {
  if (!claim) {
  ...
  if (blackListedData) {
    if (channelClaim && blackListedData[channelClaim.claim_id || channelClaim.channel_id] || blackListedData[claim.claim_id]) {
  ...
  if (filteredData) {
    if (channelClaim && filteredData[channelClaim.claim_id || channelClaim.channel_id] || filteredData[claim.claim_id]) {
