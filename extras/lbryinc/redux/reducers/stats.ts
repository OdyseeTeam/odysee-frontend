...
export type StatsState = {
  ...
  viewCountById: Record<ClaimId, number>;
  fetchingSubCount: boolean;
  subCountError?: Error;
  subCountById: Record<ClaimId, number>;
  subCountLastFetchedById: Record<ClaimId, number>;
};
...
export const statsReducer = handleActions({
  [ACTIONS.FETCH_VIEW_COUNT_STARTED]: state => ({ ...state,
    fetchingViewCount: true
  }),
  [ACTIONS.FETCH_VIEW_COUNT_FAILED]: (state, action) => ({ ...state,
  ...
  [ACTIONS.FETCH_SUB_COUNT_STARTED]: state => ({ ...state,
    fetchingSubCount: true
  }),
  [ACTIONS.FETCH_SUB_COUNT_FAILED]: (state, action) => ({ ...state,
