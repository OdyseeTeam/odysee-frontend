...
export function makeUploadRequest(
  ...
  if (file && typeof file !== 'string') {
    const blobFile = file as File | Blob;
    const fileName = (blobFile as File).name || 'file.md';
    if (blobFile instanceof Blob && !(blobFile instanceof File)) {
      body.append('file', blobFile, fileName);
    } else {
  ...
  return new Promise<any>((resolve, reject) => {
    ...
    xhr.onerror = () => {
      window.store.dispatch(doUpdateUploadProgress({ guid, status: { error: 'error' } }));
