...
export function resolveFileToUpload(params: FileUploadSdkParams): Promise<File | string> {
  return new Promise((resolve, reject) => {
    if (params.preview) {
      ...
      resolve(new File([dummyContent], 'dummy.md', {
        type: 'text/markdown'
      }));
    } else {
...
export function requestUploadToken(authToken: string, remoteUrl?: string): Promise<TokenRequestResponse> {
  return new Promise((resolve, reject) => {
    ...
    }).then(convertResponseToJson).then(json => validateJson(json, 'upload_token_created', p => p.token && p.location)).then(payload => resolve({
      token: payload.token,
      location: payload.location
    })).catch(err => reject(v4Error(err, {
      step: 'getToken'
    })));
  });
}
...
export function startRemoteUrl(uploadToken: TokenRequestResponse, remoteUrl: string): Promise<SdkFilePath> {
  return new Promise((resolve, reject) => {
    ...
    }).then(convertResponseToJson).then(json => validateJson(json, 'url_created', p => p.upload_id)).then(payload => resolve(`${uploadToken.location}${payload.upload_id}`)).catch(err => reject(v4Error(err, {
      step: 'startRemoteUrl'
    })));
  });
}
...
export function startTus(file: File | string, uploadUrl: string | null | undefined, // Previous upload URL if resuming
uploadLocation: string, // Upload URL if not resuming
uploadToken: string, cb: TusCallbacks): Promise<tus.Upload> {
  return new Promise((resolve, reject) => {
    assert(uploadUrl || uploadLocation, 'Either uploadUrl or uploadLocation must be provided');
    // --- Create tus session ---
    const tusSession = new tus.Upload(file, { ...(uploadUrl ? {
        uploadUrl: uploadUrl
      } : {
      ...
      onShouldRetry: (err, retryAttempt, options) => {
        ...
        const shouldRetry = !inStatusCategory(status, 400) || status === STATUS_CONFLICT || status === STATUS_LOCKED || status === STATUS_NOT_FOUND;
      ...
      onError: err => {
        ...
        cb.onError();
        reject(v4Error(beautifyTusError(err), {
          step: 'tus',
          inputs: {
            filename,
            filetype,
...
export function createClaim(authToken: string, sdkFilePath: SdkFilePath | null | undefined, params: any, cb: CreateClaimCallbacks): Promise<PublishId> {
  return new Promise((resolve, reject) => {
    ...
    }).then(convertResponseToJson).then(json => validateJson(json, 'query_created', p => p.query_id)).then(payload => {
  ...
  return new Promise((resolve, reject) => {
    fetch(`${LBRY_WEB_PUBLISH_API_V4}/${queryId}`, {
      ...
      switch (response.status) {
        ...
        case 200:
          convertResponseToJson(response).then(json => {
            if (json && json.result && !json.error) {
              ...
              resolve({
                status: 'error',
                error: json?.error ? v4Error(`${json?.error?.message || json?.error}`, {
                  response: json
...
function convertResponseToJson(response: Response): Promise<any> {
  return new Promise((resolve, reject) => {
    if (response.ok) {
      ...
      response.text().then(text => reject(new Error(text))).catch(() => reject(new Error(response.statusText)));
    }
  });
}
...
function validateJson(json: any, okStatus: string, validator: (payload: any) => boolean): Promise<any> {
  return new Promise((resolve, reject) => {
    if (json && json.status === okStatus && json.payload && validator(json.payload)) {
      ...
      if (json?.error) {
        reject(v4Error(`${json?.error?.message || json?.error}`, {
          response: json
        }));
      } else {
        reject(v4Error('Invalid response from server', {
          response: json
        }));
      }
    }
...
function v4Error(flop: Error | string, cause: {}): Error {
  if (typeof flop === 'string') {
    ...
    const mergedCause = { ...(flop.cause ? flop.cause : {}),
      ...cause
    };
