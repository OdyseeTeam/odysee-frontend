...
function insertToHead(fullHtml, htmlToInsert, buildRev = '') {
  ...
  if (beginIndex > -1 && finalIndex > -1 && finalIndex > beginIndex) {
    ...
    return `${fullHtml.slice(0, beginIndex)}` + `${htmlToInsert || buildOgMetadata()}\n` + `${DOUBLE_TAB}` + `${uiScript}` + `${fullHtml.slice(finalIndex + FINAL_STR.length)}`;
  }
}
...
async function buildClaimOgMetadata(uri, claim, overrideOptions = {}, referrerQuery) {
  ...
  const claimDescription = value && value.description && value.description.length > 0 ? escapeHtmlProperty(truncateDescription(value.description)) : `View on ${SITE_NAME}: ${claimTitle}`;
  const claimLanguage = value && value.languages && value.languages.length > 0 ? escapeHtmlProperty(value.languages[0]) : 'en_US';
  let imageThumbnail;
  ...
  let claimThumbnail = escapeHtmlProperty(thumbnail) || getThumbnailCardCdnUrl(imageThumbnail) || OG_IMAGE_URL || `${URL}/public/v2-og.png`;
...
function buildSearchPageHead(html, requestPath, queryStr) {
  const searchPageMetadata = buildOgMetadata({ ...(queryStr ? {
...
async function resolveClaimOrRedirect(ctx, urlOrClaimId, ignoreRedirect = false) {
  ...
  if (isClaimId) {
    try {
      ...
      const response = await Lbry.claim_search({
        claim_ids: [claimId]
      });
...
async function getHtml(ctx) {
  ...
  if (requestPath === '/' || requestPath.length === 0) {
    ...
    let ogMetadata = buildBasicOgMetadata({
      baseUrl: ctx.origin,
