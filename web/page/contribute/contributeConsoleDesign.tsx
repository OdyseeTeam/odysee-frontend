...
import { CONTRIBUTION_TITLE, CONTRIBUTION_INTRO, CONTRIBUTION_BODY, WHY_POINTS, TRACKS, PROCESS_STEPS, QUALITY_NOTE, REPOSITORIES } from "./content";
const TRACK_SLUGS = {};
...
const HELP_LINES = ['Available commands:', '', '  help             Show this message', '  about            About this page', '  why              Why contribute?', '  tracks           List contribution tracks', '  show <track>     Details for a track (e.g. show tv-apps)', '  steps            How the process works', '  repos            GitHub repositories', '  open <repo>      Open a repo (e.g. open frontend)', '  links            Quick links', '  clear            Clear output'];
...
function runCommand(rawInput) {
  ...
  switch (cmd) {
    case 'help':
    case '?':
      return HELP_LINES.map(line => line === '' ? {
        type: 'blank'
      } : {
        type: 'text',
        value: line
...
const ContributeConsoleDesign = () => {
  ...
  return <div className="contribute-container contribute-console-design">
      <div className="a43-screen">
        ...
        {WHY_POINTS.map(point => <p key={point} className="a43-text a43-quote">
            {'> '}
            {point}
          </p>)}
        ...
        <div className="a43-track-list">
          {TRACKS.map(track => <div key={track.title} className="a43-track-entry">
        ...
        <div className="a43-steps-block">
          {PROCESS_STEPS.map((step, index) => <p key={step} className="a43-text">
        ...
        <div className="a43-repos">
          {REPOSITORIES.map(repo => <a key={repo.label} href={repo.href} className="a43-repo">
              {'{ '}
        ...
        {entries.map((entry, i) => <div key={i} className="a43-entry">
            <div className="a43-line a43-prompt">
              <span className="a43-caret">$</span>
            ...
            if (line.type === 'heading') {
              return <p key={j} className="a43-out-heading">
                      {line.value}
                    </p>;
            }

            if (line.type === 'quote') {
              return <p key={j} className="a43-text a43-quote">
                      {'> '}
                      {line.value}
                    </p>;
            }
            ...
            if (line.type === 'link') {
              return <a key={j} className="a43-out-link" href={line.href} target="_blank" rel="noopener noreferrer">
                      {line.value}
                    </a>;
            }

            if (line.type === 'error') {
              return <p key={j} className="a43-out-error">
                      {line.value}
                    </p>;
            }

            if (line.type === 'hint') {
              return <p key={j} className="a43-out-hint">
                      {line.value}
                    </p>;
            }

            return <p key={j} className="a43-out-text">
                    {line.value}
                  </p>;
          })}
            </div>
        ...
        <form className="a43-input-row" onSubmit={handleSubmit}>
          <span className="a43-caret">$</span>
          <input ref={inputRef} type="text" className="a43-input" value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKeyDown} placeholder="type a command..." spellCheck={false} autoComplete="off" />
        </form>
        <div ref={endRef} />
      </div>
    </div>;
