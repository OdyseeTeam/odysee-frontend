...
function runCommand(rawInput) {
  ...
  switch (cmd) {
    case 'help':
    case '?':
      return HELP_LINES.map((line) => (line === '' ? { type: 'blank' } : { type: 'text', value: line }));

    case 'about':
      return [
        { type: 'heading', value: CONTRIBUTION_TITLE },
        { type: 'text', value: CONTRIBUTION_INTRO },
        { type: 'text', value: CONTRIBUTION_BODY },
      ];

    case 'why':
      return WHY_POINTS.map((point) => ({ type: 'quote', value: point }));
    ...
    case 'tracks':
      return [
        ...
        { type: 'blank' },
        { type: 'hint', value: 'Use "show <name>" for details, e.g. show tv-apps' },
      ];

    case 'show': {
      if (!arg) return [{ type: 'error', value: 'Usage: show <track-name>. Try "tracks" first.' }];
      const track = TRACK_SLUGS[arg];
      if (!track) {
        return [{ type: 'error', value: `Unknown track "${arg}". Available: ${Object.keys(TRACK_SLUGS).join(', ')}` }];
      }

      return [
        { type: 'heading', value: track.title },
        { type: 'text', value: track.summary },
        ...track.items.map((item) => ({ type: 'text', value: `  - ${item}` })),
      ];
    }
    ...
    case 'how':
      return [
        ...PROCESS_STEPS.map((step, index) => ({ type: 'text', value: `  ${index + 1}. ${step}` })),
        { type: 'blank' },
        { type: 'hint', value: QUALITY_NOTE },
      ];
    ...
    case 'repos':
      return [
        ...
        { type: 'blank' },
        { type: 'hint', value: 'Use "open <name>" to visit, e.g. open frontend' },
      ];

    case 'open': {
      if (!arg) return [{ type: 'error', value: 'Usage: open <repo>. Try "repos" first.' }];
      const repo = REPO_SLUGS[arg];
      if (!repo) {
        return [{ type: 'error', value: `Unknown repo "${arg}". Available: ${Object.keys(REPO_SLUGS).join(', ')}` }];
      }
      if (typeof window !== 'undefined') window.open(repo.href, '_blank');
      return [{ type: 'text', value: `Opening ${repo.label}...` }];
    }

    case 'links':
      return [
        { type: 'link', value: '  GitHub - Help Wanted issues', href: REPOSITORIES[0].href },
        { type: 'link', value: '  Discord - Chat with contributors', href: 'https://chat.odysee.com' },
        { type: 'link', value: '  llms.txt - Architecture docs for AI agents', href: 'https://llms.odysee.com' },
      ];
    ...
    default:
      return [
        ...
        { type: 'hint', value: 'Type "help" for available commands.' },
      ];
  }
}
...
const ContributeConsoleDesign = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (output && output !== 'CLEAR') {
      setEntries((prev) => [...prev, { cmd: raw, lines: output }]);
    }
    setInput('');
  };
