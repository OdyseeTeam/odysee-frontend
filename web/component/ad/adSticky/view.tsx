...
const AdSticky = memo(function AdSticky(props: Props) {
  ...
  const stickyContainer = useRef<HTMLDivElement | null | undefined>(null);
  ...
  function callback(mutationList) {
    if (provider === 'revcontent') {
      ...
      // $FlowIgnore
      stickyContainer.current?.firstElementChild?.querySelectorAll('iframe:not(:first-child)').forEach(iframe => iframe.remove());
    }
  }

  function shouldShowAdsForPath(pathname, isContentClaim, isChannelClaim, authenticated) {
    const pathIsCategory = Object.values(homepageData.categories || {}).some(x => // $FlowIgnore
  ...
  useEffect(() => {
    if (stickyContainer && stickyContainer.current) {
      observer.observe(stickyContainer.current, {
        attributes: true
      });
  ...
  if (shouldShowAds && provider === 'revcontent') {
    return <div id="sticky-d-rc" ref={stickyContainer} className={classnames({
  ...
  if (shouldShowAds && provider === 'rumble' && !isHidden) {
    return random === 0 ? <div id="rmbl-sticky" ref={stickyContainer} className={classnames({
      ...
      </div> : <div id="rmbl-sticky" ref={stickyContainer} className={classnames('rmbl-sticky--157', {
      'show-rmbl-sticky': isActive && !adBlockerFound && !isHidden,
      FILE: isContentClaim
    })}>
        <div className="rmbl-sticky">
