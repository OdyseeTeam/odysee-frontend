...
function AdErrorBoundary(props: Props) {
  ...
  const handleError = React.useCallback(err => assert(false, `Suppressed uncaught Ad error from "${type}"`, err), [type]);
  return <Sentry.ErrorBoundary fallback={null} beforeCapture={beforeCapture} onError={handleError}>
      {children}
